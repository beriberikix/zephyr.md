---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/kconfig/tips.html
original_path: build/kconfig/tips.html
---
...
The official Kconfig documentation is [kconfig-language.rst](https://www.kernel.org/doc/html/latest/kbuild/kconfig-language.html)
and [kconfig-macro-language.rst](https://www.kernel.org/doc/html/latest/kbuild/kconfig-macro-language.html).

## [What to turn into Kconfig options](#id1)
...
[Interactive Kconfig interfaces](menuconfig.md#menuconfig)), or by manually editing configuration files. Conversely, a
...
See the [optional prompts](#optional-prompts) section for a way to deal with settings that are
fixed on some machines and configurable on other machines.

## [What not to turn into Kconfig options](#id2)
...
[devicetree](../dts/index.md#dt-guide) instead.

In particular, avoid adding new Kconfig options of the following types:

### [Options that specify a device in the system by name](#id3)
...
controlled by. See [Device drivers that depend on other devices](../dts/howtos.md#dt-drivers-that-depend) for alternatives.
...
[Applications that depend on board-specific devices](../dts/howtos.md#dt-apps-that-depend) for alternatives.

### [Options that specify fixed hardware configuration](#id4)
...
[GPIO](../dts/api/api.md#devicetree-gpio-api) devicetree API from C. Similar advice applies to
other cases where devicetree.h provides [Hardware specific APIs](../dts/api/api.md#devicetree-hw-api) for referring
to other nodes in the system. Search the source code for drivers using these
APIs for examples.

An application-specific devicetree [binding](../dts/bindings.md#dt-bindings) to identify
board specific properties may be appropriate. See
[tests/drivers/gpio/gpio\_basic\_api](https://github.com/zephyrproject-rtos/zephyr/blob/main/tests/drivers/gpio/gpio_basic_api) for an example.

For applications, see [Blinky](../../samples/basic/blinky/README.md#blinky "Blink an LED forever using the GPIO API.") for a devicetree-based alternative.

## [`select` statements](#id5)
...
for simple helper symbols, as covered below in [Using select for helper symbols](#good-select-use).
...
### [Using `select` for helper symbols](#id8)
...
## [(Lack of) conditional includes](#id10)
...
conditionally includes the file `Kconfig.other`:
...
`FOO` in `Kconfig.other`, because the `DEP` dependency has already
...
## [“Stuck” symbols in menuconfig and guiconfig](#id11)
...
remember that Zephyr creates an initial configuration in `zephyr/.config`
in the build directory by merging configuration files (including e.g.
`prj.conf`). This configuration file exists before
...
- If `STACK_SIZE` should usually be user-configurable, but needs to be set to
  ...
  enabled, as described in [optional prompts](#optional-prompts):
...
## [Assignments to promptless symbols in configuration files](#id12)
...
(`zephyr/.config`), seeing a bunch of assignments to hidden symbols,
...
`zephyr/.config` are ignored by Kconfig, like for other configuration
files.

To understand why `zephyr/.config` still includes assignments to hidden
symbols, it helps to realize that `zephyr/.config` serves two separate
purposes:

1. It holds the saved configuration, and
2. it holds configuration output. `zephyr/.config` is parsed by the CMake
   files to let them query configuration settings, for example.

The assignments to hidden symbols in `zephyr/.config` are just
...
[menuconfig and guiconfig interfaces](menuconfig.md#menuconfig), could be considered
closer to just a saved configuration, without the full configuration output.

## [`depends on` and `string`/`int`/`hex` symbols](#id13)
...
## [Commas in macro arguments](#id15)
...
## [Checking changes in menuconfig/guiconfig](#id16)

When adding new symbols or making other changes to Kconfig files, it is a good
idea to look up the symbols in [menuconfig or guiconfig](menuconfig.md#menuconfig)
...
- Are there any duplicated dependencies?
  ...
  This can be checked by selecting a symbol and pressing `?` to view the
...
## [Checking changes with `scripts/kconfig/lint.py`](#id17)

After you make Kconfig changes, you can use the
[scripts/kconfig/lint.py](https://github.com/zephyrproject-rtos/zephyr/blob/main/scripts/kconfig/lint.py) script to check for some potential
...
## [Style recommendations and shorthands](#id18)

This section gives some style recommendations and explains some common Kconfig
shorthands.

### [Factoring out common dependencies](#id19)
...
### [Common Kconfig shorthands](#id21)
...
See [Coding Style Guidelines](../../contribute/style/index.md#coding-style) for style guidelines.

## [Lesser-known/used Kconfig features](#id24)

This section lists some more obscure Kconfig behaviors and features that might
still come in handy.

### [The `imply` statement](#id25)
...
See the [optional prompts](#optional-prompts) section for the meaning of the conditions on the
prompts.
...
## [Other resources](#id29)

The *Intro to symbol values* section in the [Kconfiglib docstring](https://github.com/ulfalizer/Kconfiglib/blob/master/kconfiglib.py) goes
