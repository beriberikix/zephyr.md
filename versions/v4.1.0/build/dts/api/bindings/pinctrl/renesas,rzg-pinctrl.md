---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/pinctrl/renesas,rzg-pinctrl.html
...
| Name | Type | Details |
| --- | --- | --- |
| `pinmux` | `array` | ```text Pinmux configuration node. Values are constructed from GPIO port number, pin number, and alternate function configuration number using the RZG_PINMUX() helper macro in pinctrl_rzg.h ``` |
| `pins` | `array` | ```text Special Purpose pins configuration node. Values are define in pinctrl_rzg.h. Ex: BSP_IO_XSPI_IO0,BSP_IO_I3C_SCL,... ``` |
