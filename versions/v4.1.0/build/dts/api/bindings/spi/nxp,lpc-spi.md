---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/spi/nxp,lpc-spi.html
original_path: build/dts/api/bindings/spi/nxp,lpc-spi.html
---
...
[drivers/spi/spi\_mcux\_flexcomm.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/spi/spi_mcux_flexcomm.c).
...
| Name | Type | Details |
| --- | --- | --- |
| `pre-delay` | `int` | ```text Delay in nanoseconds inserted between chip select assert to the first clock edge. If not set, no additional delay is inserted. ``` |
| `post-delay` | `int` | ```text Delay in nanoseconds inserted between the last clock edge to the chip select deassert. If not set, no additional delay is inserted. ``` |
| `frame-delay` | `int` | ```text Delay in nanoseconds inserted between data frames when chip select is asserted and the EOF flag is set. If not set, no additional delay is inserted. ``` |
| `transfer-delay` | `int` | ```text Delay in nanoseconds inserted between transfers when chip select is deasserted. If not set, no additional delay is inserted. ``` |
| `def-char` | `int` | ```text Default character clocked out when the TX buffer pointer is NULL. Applies to SPI master and slave configurations. ``` |
...
may apply to the “nxp,lpc-spi” compatible.
