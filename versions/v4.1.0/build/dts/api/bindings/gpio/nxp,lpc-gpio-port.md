---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/gpio/nxp,lpc-gpio-port.html
original_path: build/dts/api/bindings/gpio/nxp,lpc-gpio-port.html
---
...
[drivers/gpio/gpio\_mcux\_lpc.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/gpio/gpio_mcux_lpc.c).

## Description

```text
LPC GPIO port device.
```
...
These property descriptions apply to “nxp,lpc-gpio-port”
...
| `int-source` | `string` | ```text Interrupt source for the gpio port. For ports that can use the PINT as an interrupt source for their pins (typically ports 0 and 1), this can be set to PINT. Otherwise, the property should be set to "int-a" or "int-b" if interrupt support is desired, and the appropriate IRQ number should set for the device. ```  Default value: `none`  Legal values: `'pint'`, `'int-a'`, `'int-b'`, `'none'` |
...
may apply to the “nxp,lpc-gpio-port” compatible.
