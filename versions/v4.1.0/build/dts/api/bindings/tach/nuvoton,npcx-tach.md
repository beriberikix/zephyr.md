---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/tach/nuvoton,npcx-tach.html
original_path: build/dts/api/bindings/tach/nuvoton,npcx-tach.html
---
...
[drivers/sensor/nuvoton/nuvoton\_tach\_npcx/tach\_nuvoton\_npcx.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/sensor/nuvoton/nuvoton_tach_npcx/tach_nuvoton_npcx.c).
...
| `sample-clk` | `int` | ```text sampling clock frequency of tachometer. Please notice that it must be fixed to 32768 if bus in clocks property is NPCX_CLOCK_BUS_LFCLK. ``` |
| `port` | `int` | ```text selected port of tachometer (port-A is 0 and port-B is 1) ``` |
| `pulses-per-round` | `int` | ```text number of pulses (holes) per round of tachometer's input (encoder) ``` |
...
may apply to the “nuvoton,npcx-tach” compatible.
