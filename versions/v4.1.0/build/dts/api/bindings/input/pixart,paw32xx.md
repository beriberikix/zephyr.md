---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/input/pixart,paw32xx.html
original_path: build/dts/api/bindings/input/pixart,paw32xx.html
---

# pixart,paw32xx (on spi bus)
...
[drivers/input/input\_paw32xx.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/input/input_paw32xx.c).

## Description

```text
PAW32xx ultra low power wireless mouse chip
```
...
| `res-cpi` | `int` | ```text CPI resolution for the sensor. This can also be changed in runtime using the paw32xx_set_resolution() API. ``` |
| `invert-x` | `boolean` | ```text Invert X axis values. ``` |
| `invert-y` | `boolean` | ```text Invert Y axis values. ``` |
| `force-awake` | `boolean` | ```text Initialize the sensor in "force awake" mode. This can also be enabled or disabled in runtime by the application using the paw32xx_force_awake() API. ``` |
...
may apply to the “pixart,paw32xx” compatible.
