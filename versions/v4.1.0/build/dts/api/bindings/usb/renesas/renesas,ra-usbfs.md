---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/usb/renesas/renesas,ra-usbfs.html
original_path: build/dts/api/bindings/usb/renesas/renesas,ra-usbfs.html
---
...
```text
Renesas RA USB full-speed controller
```
...
| Name | Type | Details |
| --- | --- | --- |
| `phys-clock` | `phandles` | ```text USBFS physical clock. ```  This property is **required**. |
...
may apply to the “renesas,ra-usbfs” compatible.

| Name | Type | Details |
| --- | --- | --- |
| `interrupts` | `array` | ```text IRQ number and priority to use for USBFS. ```  This property is **required**.  See [Important properties](../../../../intro-syntax-structure.md#dt-important-props) for more information. |
| `interrupt-names` | `string-array` | ```text Interrupts must be given corresponding names so that the shim driver can recognize them. ```  This property is **required**.  Legal values: `'usbfs-i'`, `'usbfs-r'` |
