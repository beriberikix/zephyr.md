---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/usb/renesas/renesas,ra-usbhs.html
original_path: build/dts/api/bindings/usb/renesas/renesas,ra-usbhs.html
---
...
```text
Renesas RA USB high-speed controller
```
...
| Name | Type | Details |
| --- | --- | --- |
| `phys-clock` | `phandles` | ```text USBHS physical clocks. Should be provided in case internal clock source is set in phys node. ``` |
| `phys` | `phandle` | ```text PHY provider specifier ``` |
...
may apply to the “renesas,ra-usbhs” compatible.

| Name | Type | Details |
| --- | --- | --- |
| `interrupts` | `array` | ```text IRQ number and priority to use for USBHS. ```  This property is **required**.  See [Important properties](../../../../intro-syntax-structure.md#dt-important-props) for more information. |
| `interrupt-names` | `string-array` | ```text Interrupts must be given corresponding names so that the shim driver can recognize them. ```  This property is **required**.  Legal values: `'usbhs-ir'` |
