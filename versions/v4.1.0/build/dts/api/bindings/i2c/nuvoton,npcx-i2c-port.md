---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/i2c/nuvoton,npcx-i2c-port.html
original_path: build/dts/api/bindings/i2c/nuvoton,npcx-i2c-port.html
---
...
[drivers/i2c/i2c\_npcx\_port.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/i2c/i2c_npcx_port.c).
...
| Name | Type | Details |
| --- | --- | --- |
| `port` | `int` | ```text Index of i2c port This property is used to differentiate between multiple I2C ports controlled by the same or different I2C controllers. Bit[7:4] module id (controller). Bit[3:0] port id. Use the macro NPCX_I2C_CTRL_PORT(ctrl, port) to set this property. ```  This property is **required**. |
| `controller` | `phandle` | ```text i2c controller to handle signals from port pads ```  This property is **required**. |
...
| `clock-frequency` | `int` | ```text Initial clock frequency in Hz ```  This property is **required**. |
...
may apply to the “nuvoton,npcx-i2c-port” compatible.
