---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/wifi/infineon,airoc-wifi-spi.html
original_path: build/dts/api/bindings/wifi/infineon,airoc-wifi-spi.html
---

# infineon,airoc-wifi (on spi bus)
...
[drivers/wifi/infineon](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/wifi/infineon).
...
| Name | Type | Details |
| --- | --- | --- |
| `wifi-host-wake-gpios` | `phandle-array` | ```text Host wake-up gpio. Signal from the AIROC Wi-Fi device to the host indicating that the device requires attention. ```  This property is **required**. |
| `bus-select-gpios` | `phandle-array` | ```text Select bus mode.  This gpio must be held low before wifi-reg-on-gpios goes high to select SPI bus mode. ```  This property is **required**. |
| `spi-half-duplex` | `boolean` | ```text Use half-duplex communication;  if not present, full- duplex operation is assumed. ``` |
| `spi-word-size` | `int` | ```text SPI word size in bits.  Since 8-bit word size is the most common, that is selected as the default. ```  Default value: `8`  Legal values: `8`, `16`, `32` |
| `spi-data-irq-shared` | `boolean` | ```text SPI data and IRQ share the same GPIO. ``` |
