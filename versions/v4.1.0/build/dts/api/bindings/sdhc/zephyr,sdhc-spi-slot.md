---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/sdhc/zephyr,sdhc-spi-slot.html
original_path: build/dts/api/bindings/sdhc/zephyr,sdhc-spi-slot.html
---
...
[drivers/sdhc/sdhc\_spi.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/sdhc/sdhc_spi.c).
...
| Name | Type | Details |
| --- | --- | --- |
| `power-delay-ms` | `int` | ```text Time in ms for SPI SDHC to delay when toggling power to the SD card. This delay gives the card time to power up or down fully. It can be increased if you observe issues initializing your card. ```  Default value: `1` |
| `spi-clock-mode-cpol` | `boolean` | ```text Clock polarity to use for SPI SDHC. Some cards respond properly only when the clock goes low when not active. ``` |
| `spi-clock-mode-cpha` | `boolean` | ```text Clock phase: this dictates when is the data captured, and depends on the clock's polarity. When mode-cpol is set and this option as well, capture will occur on low to high transition and high to low if this option is not set (default). ``` |
| `pwr-gpios` | `phandle-array` | ```text Power pin This pin defaults to active high when consumed by the SPI SDHC driver. It can be used to toggle card power via an external control circuit ``` |
...
may apply to the “zephyr,sdhc-spi-slot” compatible.
