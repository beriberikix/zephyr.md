---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/samples/subsys/usb/testusb/README.html
original_path: samples/subsys/usb/testusb/README.html
---
...
](https://github.com/zephyrproject-rtos/zephyr/blob/main//samples/subsys/usb/testusb/README.rst/..)
...
1. Load the `usbtest` Linux kernel module on the Linux Host.
   ...
   ```
2. Use the `testusb` tool in `linux/tools/usb` inside Linux kernel source directory
   to start the tests.
   ...
   ```
3. To run all the tests the Zephyr’s VID / PID should be inserted to USB
   driver id table. The method for loading the `usbtest` driver for our
   device is described here: [https://lwn.net/Articles/160944/](https://lwn.net/Articles/160944/).
   ...
   ```
4. Use the `testusb` tool in `linux/tools/usb` inside Linux kernel source directory
   to start the tests.
