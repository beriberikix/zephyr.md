---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/samples/subsys/mgmt/mcumgr/smp_svr/README.html
original_path: samples/subsys/mgmt/mcumgr/smp_svr/README.html
---
...
](https://github.com/zephyrproject-rtos/zephyr/blob/main//samples/subsys/mgmt/mcumgr/smp_svr/README.rst/..)
...
For more information about MCUmgr and SMP, please see [Device Management](../../../../../services/device_mgmt/index.md#device-mgmt).
...
- The MCUboot bootloader is required for `img_mgmt` to function
  ...
  MCUboot can be found in [MCUboot](../../../../../services/device_mgmt/dfu.md#mcuboot).
...
it. There are various tools available which are listed on the [Tools/libraries](../../../../../services/device_mgmt/mcumgr.md#mcumgr-tools-libraries) page
...
[Using the controller with BlueZ](../../../../bluetooth/hci_uart/README.md#bluetooth-hci-uart-bluez).
...
BluetoothSerialUSB CDC\_ACMShellUDP

To build the bluetooth sample:

```shell
west build -b nrf52dk/nrf52832 --sysbuild samples/subsys/mgmt/mcumgr/smp_svr -- -DEXTRA_CONF_FILE="overlay-bt.conf"
```

To build the serial sample with file-system and shell management support:

```shell
west build -b frdm_k64f --sysbuild samples/subsys/mgmt/mcumgr/smp_svr -- -DEXTRA_CONF_FILE="overlay-serial.conf;overlay-fs.conf;overlay-shell-mgmt.conf"
```

To build the serial sample with USB CDC\_ACM backend:

```shell
west build -b nrf52840dk/nrf52840 --sysbuild samples/subsys/mgmt/mcumgr/smp_svr -- -DEXTRA_CONF_FILE="overlay-cdc.conf" -DEXTRA_DTC_OVERLAY_FILE="usb.overlay"
```

To build the shell sample:

```shell
west build -b frdm_k64f --sysbuild samples/subsys/mgmt/mcumgr/smp_svr -- -DEXTRA_CONF_FILE="overlay-shell.conf"
```
...
```shell
west build -b frdm_k64f --sysbuild samples/subsys/mgmt/mcumgr/smp_svr -- -DEXTRA_CONF_FILE="overlay-udp.conf"
```

## Flashing the sample image

The original application will be built for slot-0, see [Flash map](../../../../../services/storage/flash_map/flash_map.md#flash-map-api) for details on flash
...
- Build an MCUboot enabled application using sysbuild, see [Building the sample application](#smp-svr-sample-build)
...
upload `image` ID to use to update that slot, see [Slot info](../../../../../services/device_mgmt/smp_groups/smp_group_1.md#mcumgr-smp-group-1-slot-info) for
details.
...
[File Systems](../../../../../services/file_system/index.md#file-system-api), this is useful with e.g. FS log backend, when files are stored in
