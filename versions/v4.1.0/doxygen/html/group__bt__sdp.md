---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/doxygen/html/group__bt__sdp.html
...
Service Search: The SDP Client generates an SDP\_SERVICE\_SEARCH\_REQ to locate service records that match the service search pattern (params->uuid) given as the first parameter of the PDU. Service Attribute: The SDP Client generates an SDP\_SERVICE\_ATTR\_REQ to retrieve specified attribute values from a specific service record (params->handle). Service Search Attribute: The SDP Client generates an SDP\_SERVICE\_SEARCH\_ATTR\_REQ to retrieve specified attribute values that match the service search pattern (params->uuid) given as the first parameter of the PDU.
