---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/develop/modules.html
original_path: develop/modules.html
---
...
[module licensing requirements](#modules-licensing) and
[contribution guidelines](#modules-contributing). They should also have a
...
references to optional [binary blobs](../contribute/bin_blobs.md#bin-blobs).
...
Zephyr modules, described in this page, are not the same as [west projects](west/basics.md#west-workspace). In fact, modules [do not require west](#modules-without-west) at all. However, when using modules [with west](#modules-using-west), then the build system uses west in order to find modules.

In summary:

Modules are repositories that contain a `zephyr/module.yml` file, so that
the Zephyr build system can pull in the source code from the repository.
[West projects](west/manifest.md#west-manifests-projects) are entries in the `projects:`
...
need to be modules.
Modules are found by the Zephyr build system either via [west itself](#modules-using-west), or via the [ZEPHYR\_MODULES CMake variable](#modules-without-west).
...
- The module repository codebase shall include a *module.yml* file in a
  `zephyr/` folder at the root of the repository.
...
- regularly synchronize the module repository with its upstream
  ...
  [Synchronizing with upstream](#modules-synchronization)
...
- list any known security vulnerability issues, present in the
  ...
  [Maintainers](../project/project_roles.md#project-roles).
...
[Submitting changes to modules](#submitting-new-modules) describes in
...
**Issue Reporting:** [GitHub issues](https://github.com/zephyrproject-rtos/zephyr/issues) are intentionally disabled in module
repositories, in
...
> [Release Quality Criteria](../project/release_process.md#release-quality-criteria).
...
- licensing information as described in [Licensing requirements and policies](#modules-licensing).
...
The build system variable **ZEPHYR\_MODULES** is a [CMake list](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#lists) of
absolute paths to the directories containing Zephyr modules. These modules
contain `CMakeLists.txt` and `Kconfig` files describing how to
build and configure them, respectively. Module `CMakeLists.txt` files are
added to the build using CMake’s [add\_subdirectory()](https://cmake.org/cmake/help/latest/command/add_subdirectory.html) command, and the
`Kconfig` files are included in the build’s Kconfig menu tree.

If you have [west](west/index.md#west) installed, you don’t need to worry about how
this variable is defined unless you are adding a new module. The build system
knows how to use west to set **ZEPHYR\_MODULES**. You can add additional
modules to this list by setting the **EXTRA\_ZEPHYR\_MODULES** CMake
variable or by adding a **EXTRA\_ZEPHYR\_MODULES** line to `.zephyrrc`
(See the section on [Environment Variables](env_vars.md#env-vars) for more details). This can be
useful if you want to keep the list of modules found with west and also add
your own.

Note

If the module `FOO` is provided by [west](west/index.md#west) but also given with
`-DEXTRA_ZEPHYR_MODULES=/<path>/foo` then the module given by the command
line variable **EXTRA\_ZEPHYR\_MODULES** will take precedence.
This allows you to use a custom version of `FOO` when building and still
use other Zephyr modules provided by [west](west/index.md#west).
...
into the [submanifests](https://github.com/zephyrproject-rtos/zephyr/blob/main/submanifests) directory. See
[submanifests/README.txt](https://github.com/zephyrproject-rtos/zephyr/blob/main/submanifests/README.txt) for more details.

See [Basics](west/basics.md#west-basics) for more on west workspaces.
...
A module can be described using a file named `zephyr/module.yml`.
The format of `zephyr/module.yml` is described in the following:
...
The name should be specified in the `zephyr/module.yml` file. This will
...
containing the module’s `CMakeLists.txt` file.
...
As example, the Zephyr module located in `<workspace>/modules/bar` will
use `bar` as its module name if nothing is specified in
`zephyr/module.yml`.

### Module integration files (in-module)

Inclusion of build files, `CMakeLists.txt` and `Kconfig`, can be
described as:
...
`<cmake-directory>` contains the `CMakeLists.txt` to use. The
...
Here is an example `module.yml` file referring to
`CMakeLists.txt` and `Kconfig` files in the root directory of the
module:
...
[Sysbuild](../build/sysbuild/index.md#sysbuild) is the Zephyr build system that allows for building
...
sysbuild-specific build files, `CMakeLists.txt` and `Kconfig`, can
be described as:
...
Here is an example `module.yml` file referring to
`CMakeLists.txt` and `Kconfig` files in the `sysbuild` directory of
the module:
...
The module description file `zephyr/module.yml` can also be used to
specify that the build files, `CMakeLists.txt` and `Kconfig`, are
located in a [Module integration files (external)](#modules-module-ext-root).
...
CPE field must follow the CPE 2.3 schema provided by [NVD](https://csrc.nist.gov/projects/security-content-automation-protocol/specifications/cpe).
PURL field must follow the PURL specification provided by [Github](https://github.com/package-url/purl-spec/blob/master/PURL-SPECIFICATION.rst).

### Build system integration

When a module has a `module.yml` file, it will automatically be included into
...
absolute path to the directory containing the `CMakeLists.txt` file that
...
For example, to include the file `some/Kconfig` in module `foo`:
...
It is possible to append values to a Zephyr [CMake list](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#lists) variable from the module’s first
CMakeLists.txt file.
To do so, append the value to the list and then set the list in the PARENT\_SCOPE
...
containing the `CMakeLists.txt` file that is included into CMake build
...
In Kconfig, the variable may be used to find additional files to include.
For example, to include the file `some/Kconfig`:
...
It is possible to append values to a Zephyr [CMake list](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#lists) variable from the
module’s first CMakeLists.txt file.
To do so, append the value to the list and then set the list in the
PARENT\_SCOPE of the CMakeLists.txt file. For example, to append `bar` to the
...
module’s sysbuild `CMakeLists.txt` file must define function `foo_post_cmake()`.
...
module’s sysbuild `CMakeLists.txt` file.
...
The Zephyr repository contain `CMakeLists.txt` and `Kconfig` build
...
the CMake build system. The `MODULE_EXT_ROOT` accepts a [CMake list](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#lists) of roots as
argument.

A Zephyr module can automatically be added to the `MODULE_EXT_ROOT`
list using the module description file `zephyr/module.yml`, see
[Build settings](#modules-build-settings).
...
The `modules.cmake` file must contain the logic that specifies the
...
The module description file `zephyr/module.yml` can be used to specify
that the build files, `CMakeLists.txt` and `Kconfig`, are located
in a [Module integration files (external)](#modules-module-ext-root).
...
- `board_root`: Contains additional boards that are available to the build
  system. Additional boards must be located in a `<board_root>/boards`
  folder.
- `dts_root`: Contains additional dts files related to the architecture/soc
  families. Additional dts files must be located in a `<dts_root>/dts`
  folder.
- `snippet_root`: Contains additional snippets that are available for use.
  These snippets must be defined in `snippet.yml` files underneath the
  `<snippet_root>/snippets` folder. For example, if you have
  `snippet_root: foo`, then you should place your module’s
  `snippet.yml` files in `<your-module>/foo/snippets` or any
  nested subdirectory.
- `soc_root`: Contains additional SoCs that are available to the build
  system. Additional SoCs must be located in a `<soc_root>/soc` folder.
- `arch_root`: Contains additional architectures that are available to the
  ...
  `<arch_root>/arch` folder.
- `module_ext_root`: Contains `CMakeLists.txt` and `Kconfig` files
  for Zephyr modules, see also [Module integration files (external)](#modules-module-ext-root).
- `sca_root`: Contains additional [SCA](sca/index.md#sca) tool implementations
  available to the build system. Each tool must be located in
  `<sca_root>/sca/<tool>` folder. The folder must contain a
  `sca.cmake`.

Example of a `module.yaml` file containing additional roots, and the
...
`zephyr/module.yml` file. Additionally, if a module defines out of tree
...
Zephyr supports fetching and using [binary blobs](../contribute/bin_blobs.md#bin-blobs), and their
metadata is contained entirely in `zephyr/module.yml`. This is because
...
Binary blobs are fetched using [west blobs](west/zephyr-cmds.md#west-blobs). If `west` is
[not used](#modules-without-west), they must be downloaded and
verified manually.

The `blobs` section in `zephyr/module.yml` consists of a sequence of
maps, each of which has the following entries:

- `path`: The path to the binary blob, relative to the `zephyr/blobs/`
  folder in the module repository
- `sha256`: [SHA-256](https://en.wikipedia.org/wiki/SHA-2) checksum of the
  binary blob file
- `type`: The [type of binary blob](../contribute/bin_blobs.md#bin-blobs-types). Currently limited
  to `img` or `lib`
...
Calling `west packages pip` lists [requirement files](https://pip.pypa.io/en/stable/reference/requirements-file-format/) for Zephyr and modules.
...
If a module has out of tree boards that require custom [runners](west/build-flash-debug.md#west-runner),
...
If west is installed and **ZEPHYR\_MODULES** is not already set, the
build system finds all the modules in your [west installation](../glossary.md#term-west-installation) and uses
those. It does this by running [west list](west/built-in.md#west-built-in-misc) to get
...
- If the project contains a file named `zephyr/module.yml`, then the
...
- Otherwise (i.e. if the project has no `zephyr/module.yml`), the
  build system looks for `zephyr/CMakeLists.txt` and
  `zephyr/Kconfig` files in the project. If both are present, the project
...
- If neither of those checks succeed, the project is not considered a module,
  and is not added to **ZEPHYR\_MODULES**.

#### Without West

If you don’t have west installed or don’t want the build system to use it to
find Zephyr modules, you can set **ZEPHYR\_MODULES** yourself using one
of the following options. Each of the directories in the list must contain
either a `zephyr/module.yml` file or the files
`zephyr/CMakeLists.txt` and `Kconfig`, as described in the previous
section.
...
3. In a separate CMake script which is pre-loaded to populate the CMake cache,
   like this:
...
If you don’t have west installed and don’t specify **ZEPHYR\_MODULES**
...
Please follow the process in [Submission and review process](../contribute/external.md#external-src-process) and obtain the TSC
...
[here](#modules-using-west), and then add a new entry to the
[west.yml](https://github.com/zephyrproject-rtos/zephyr/blob/main/west.yml) with the following information:
...
For example, to add *my\_module* to the manifest:
...
*my\_module* repository. Once the module changes are reviewed and merged, the
...
1. Submit the changes using a pull request to an existing repository following
   the [contribution guidelines](../contribute/guidelines.md#contribute-guidelines) and
   [expectations](../contribute/contributor_expectations.md#contributor-expectations).
2. Submit a pull request changing the entry referencing the module into the
   [west.yml](https://github.com/zephyrproject-rtos/zephyr/blob/main/west.yml) of the main Zephyr tree with the following
   information:
