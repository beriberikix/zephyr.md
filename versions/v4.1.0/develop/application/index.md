---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/develop/application/index.html
...
application. In the following example, the [hello\_world sample](../../samples/hello_world/README.md#hello_world "Print "Hello World" to the console.") is a Zephyr repository application:
...
Zephyr’s [Samples and Demos](../../samples/index.md#samples) as a starting point is likely to be easier.
...
- **BOARD**: Selects the board that the application’s build
  will use for the default configuration. See [Supported Boards and Shields](../../boards/index.md#boards) for
...
1. If you only want to build for one board, add the name of the board
   ...
   Refer to [Supported Boards and Shields](../../boards/index.md#boards) for more information on available boards.
...
2. If your application uses a configuration file or files other than
   the usual `prj.conf`, add lines setting the **CONF\_FILE**
   ...
   avoid setting **CONF\_FILE** in a single place. For example:
...
Looking at [existing samples](../../samples/index.md#samples) is a good way to get
started.
...
[dts/bindings/vendor-prefixes.txt](https://github.com/zephyrproject-rtos/zephyr/blob/main/dts/bindings/vendor-prefixes.txt) if submitting upstream to Zephyr, or be
...
`CMakeLists.txt` file, see [Configuring sysbuild internal state](../../build/sysbuild/index.md#sysbuild-var-override) for details.
