---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/develop/west/workspaces.html
original_path: develop/west/workspaces.html
---

# Workspaces

This page describes the *west workspace* concept introduced in
[Basics](basics.md#west-basics) in more detail.
...
specified for the project at the time [west update](built-in.md#west-update) was last run.
...
helpers built in to Git](https://git-scm.com/docs/gitcredentials). Since Git has credential storage built in, there is
...
credential helper. See the [git-credential-store](https://git-scm.com/docs/git-credential-store#_examples) manual page for details.
...
On GitHub, you can set up a [personal access token](https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token) to use in place of your
...
them in memory temporarily using [git-credential-cache](https://git-scm.com/docs/git-credential-cache) instead.
...
password, you can avoid entering it manually every time using [ssh-agent](https://www.ssh.com/ssh/agent).

On GitHub, see [Connecting to GitHub with SSH](https://docs.github.com/en/github/authenticating-to-github/connecting-to-github-with-ssh) for details on configuration
and key creation.
...
- The zephyr repository acts as the central repository and specifies
  its [Modules (External projects)](../modules.md#modules) in its `west.yml`
- Analogy with existing mechanisms: Git submodules with zephyr as the
  super-project

This is the default. See [Workspace concepts](basics.md#west-workspace) for how mainline Zephyr is an
...
- A repository containing a Zephyr application acts as the central repository
  and names other projects required to build it in its `west.yml`. This
...
Here is an example `application/west.yml` which uses
[Manifest Imports](manifest.md#west-manifest-import), available since west 0.7, to import Zephyr v2.5.0
...
`import:` in this way; see [Example 1.3: Downstream of a Zephyr release, with module fork](manifest.md#west-manifest-ex1-3) for an example.

Another way to do the same thing is to copy/paste `zephyr/west.yml`
to `application/west.yml`, adding an entry for the zephyr
...
(The `west-commands` is there for [Building, Flashing and Debugging](build-flash-debug.md#west-build-flash-debug) and other
Zephyr-specific [Extensions](extensions.md#west-extensions). It’s not necessary when using
`import`.)
...
Zephyr’s [module](../modules.md#modules) versions are automatically determined from the
...
Here is an example T3 `manifest-repo/west.yml` which uses
[Manifest Imports](manifest.md#west-manifest-import), available since west 0.7, to import Zephyr
...
as shown [above](#west-t2) for the T2 topology, with the same caveats.

## Not supported: workspace topdir as .git repository

Some users have asked for support making the workspace [topdir](basics.md#west-workspace) a git repository, like this example:
