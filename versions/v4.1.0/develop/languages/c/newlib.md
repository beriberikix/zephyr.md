---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/develop/languages/c/newlib.html
original_path: develop/languages/c/newlib.html
---

# Newlib

[Newlib](https://sourceware.org/newlib/) is a complete C library implementation written for the embedded
systems. It is a separate open source project and is not included in source
code form with Zephyr. Instead, the [Zephyr SDK](../../toolchains/zephyr_sdk.md#toolchain-zephyr-sdk) includes a
precompiled library for each supported architecture (`libc.a` and
`libm.a`).

Note

Other 3rd-party toolchains, such as [GNU Arm Embedded](../../toolchains/gnu_arm_embedded.md#toolchain-gnuarmemb), also bundle
...
`lib/libc/newlib/libc-hooks.c` and translate the library internal system
calls to the equivalent Zephyr API calls.

## Types of Newlib

The Newlib included in the [Zephyr SDK](../../toolchains/zephyr_sdk.md#toolchain-zephyr-sdk) comes in two versions:
‘full’ and ‘nano’ variants.

### Full Newlib

The Newlib full variant (`libc.a` and `libm.a`) is the most capable
...
[`CONFIG_NEWLIB_LIBC`](../../../kconfig.md#CONFIG_NEWLIB_LIBC "CONFIG_NEWLIB_LIBC") and de-selecting the
[`CONFIG_NEWLIB_LIBC_NANO`](../../../kconfig.md#CONFIG_NEWLIB_LIBC_NANO "CONFIG_NEWLIB_LIBC_NANO") in the application configuration
file.

### Nano Newlib

The Newlib nano variant (`libc_nano.a` and `libm_nano.a`) is the
...
This variant can be enabled by selecting the
[`CONFIG_NEWLIB_LIBC`](../../../kconfig.md#CONFIG_NEWLIB_LIBC "CONFIG_NEWLIB_LIBC") and
[`CONFIG_NEWLIB_LIBC_NANO`](../../../kconfig.md#CONFIG_NEWLIB_LIBC_NANO "CONFIG_NEWLIB_LIBC_NANO") in the application configuration
file.
...
- Floating point format specifiers (e.g. `%f`) require
  [`CONFIG_NEWLIB_LIBC_FLOAT_PRINTF`](../../../kconfig.md#CONFIG_NEWLIB_LIBC_FLOAT_PRINTF "CONFIG_NEWLIB_LIBC_FLOAT_PRINTF") and
  [`CONFIG_NEWLIB_LIBC_FLOAT_SCANF`](../../../kconfig.md#CONFIG_NEWLIB_LIBC_FLOAT_SCANF "CONFIG_NEWLIB_LIBC_FLOAT_SCANF") to be enabled.
...
[`malloc()`](../../../doxygen/html/stdlib_8h.md#a9c36d0fe3ec4675cbffdc9b52f5fb399) and [`free()`](../../../doxygen/html/stdlib_8h.md#afbedc913aa4651b3c3b4b3aecd9b4711)).
...
in the Full Newlib (`libc.a` and `libm.a`) of the Zephyr SDK
...
(`libc_nano.a` and `libm_nano.a`).

The only interface between the Newlib dynamic memory management functions and
the Zephyr-side libc hooks is the `sbrk()` function, which is used by the
Newlib to manage the size of the memory pool reserved for its internal heap
allocator.

The `_sbrk()` hook function, implemented in `libc-hooks.c`, handles
...
- When MMU is enabled ([`CONFIG_MMU`](../../../kconfig.md#CONFIG_MMU "CONFIG_MMU") is selected), the amount of
  memory space reserved for the Newlib heap is set by the size of the free
  memory space returned by the [`k_mem_free_get()`](../../../doxygen/html/group__kernel__memory__management.md#gabb315b4994193147e9f51b0c3268bfcd) function or the
  [`CONFIG_NEWLIB_LIBC_MAX_MAPPED_REGION_SIZE`](../../../kconfig.md#CONFIG_NEWLIB_LIBC_MAX_MAPPED_REGION_SIZE "CONFIG_NEWLIB_LIBC_MAX_MAPPED_REGION_SIZE"), whichever is the
  smallest.
- When MPU is enabled and the MPU requires power-of-two partition size and
  address alignment ([`CONFIG_NEWLIB_LIBC_ALIGNED_HEAP_SIZE`](../../../kconfig.md#CONFIG_NEWLIB_LIBC_ALIGNED_HEAP_SIZE "CONFIG_NEWLIB_LIBC_ALIGNED_HEAP_SIZE") is
  set to a non-zero value), the amount of memory space reserved for the Newlib
  heap is set by the [`CONFIG_NEWLIB_LIBC_ALIGNED_HEAP_SIZE`](../../../kconfig.md#CONFIG_NEWLIB_LIBC_ALIGNED_HEAP_SIZE "CONFIG_NEWLIB_LIBC_ALIGNED_HEAP_SIZE").
