---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/develop/api/api_lifecycle.html
original_path: develop/api/api_lifecycle.html
---
...
![API Life Cycle](https://docs.zephyrproject.org/4.1.0/_images/api_lifecycle.png)

API Life Cycle

An up-to-date table of all APIs and their maturity level can be found in the
[API Overview](overview.md#api-overview) page.
...
to the community via the [Developer mailing list](https://lists.zephyrproject.org/g/devel).
...
version is up to one (0.1.z). (see [API Overview](overview.md#api-overview))
...
version is larger than one (0.y.z | y > 1 ). (see [API Overview](overview.md#api-overview))
...
1. A Pull Request must be opened that changes the corresponding entry in the
   [API Overview](overview.md#api-overview) table
2. An email must be sent to the `devel` mailing list announcing the API
   upgrade request
3. The Pull Request must be submitted for discussion in the next
   [Zephyr Architecture meeting](https://github.com/zephyrproject-rtos/zephyr/wiki/Architecture-Working-Group) where, barring any objections, the Pull Request
   will be merged
...
version is one or larger (x.y.z | x >= 1 ). (see [API Overview](overview.md#api-overview))
...
1. An [RFC issue](../../contribute/proposals_and_rfcs.md#rfcs) must be opened on GitHub with the following
   content:
...
3. The RFC issue must be submitted for discussion in the next [Zephyr
   Architecture meeting](https://github.com/zephyrproject-rtos/zephyr/wiki/Architecture-Working-Group)
4. An email must be sent to the `devel` mailing list with a subject identical
...
for it to be discussed and ultimately even voted on in the [Zephyr TSC meeting](https://github.com/zephyrproject-rtos/zephyr/wiki/Zephyr-Committee-and-Working-Group-Meetings#technical-steering-committee-tsc).
...
0 when major version is incremented. (see [API Overview](overview.md#api-overview))
...
- What is required when deprecating:
  ...
  - Add an entry in the corresponding release
    [GitHub issue](https://github.com/zephyrproject-rtos/zephyr/labels/deprecation_tracker)
