---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/services/tracing/index.html
...
your application and allows [Tracing Tools](#tools) running on a host to visualize the inner-working of
...
[`sys_trace_named_event()`](../../doxygen/html/group__subsys__tracing__apis__named.md#ga205bc648bdf0152f8652f04f15cf48af), which takes an event name as well as two
...
Zephyr includes support for [Percepio Tracealyzer](https://percepio.com/tracealyzer) that offers trace visualization for
...
[![Percepio Tracealyzer](https://docs.zephyrproject.org/4.1.0/_images/percepio_tracealyzer.png)
](https://docs.zephyrproject.org/4.1.0/_images/percepio_tracealyzer.png)
...
- Under Subsystems and OS Services ‣ Tracing Support ‣ Tracing Format, select
...
Modules ‣ percepio ‣ TraceRecorder ‣ Stream Port.
...
under Modules ‣ percepio ‣ TraceRecorder ‣ (Stream Port) Config.
...
- Under Subsystems and OS Services ‣ Tracing Support ‣ Tracing Format, select
  Percepio Tracealyzer
- Under Modules ‣ percepio ‣ TraceRecorder ‣ Recorder Start Mode, select
  Start
- Under Modules ‣ percepio ‣ TraceRecorder ‣ Stream Port, select
  Ring Buffer
- Under Modules ‣ percepio ‣ TraceRecorder ‣ Ring Buffer Config ‣ Buffer Size,
...
In menuconfig, see
Subsystems and OS Services ‣ Tracing Support ‣ Tracing Configuration.
...
specified. Open this file in Tracealyzer by selecting File ‣ Open ‣ Open File.
...
- Under Modules ‣ percepio ‣ TraceRecorder ‣ Stream Port, select
  RTT
- Under Modules ‣ percepio ‣ TraceRecorder ‣ RTT Config, set the size of the
  RTT “up” buffer in bytes.

The setting RTT buffer size up sets the size of the RTT transmission buffer. This is important
...
- Under Modules ‣ percepio ‣ TraceRecorder ‣ Stream Port, select
  ITM
- Under Modules ‣ percepio ‣ TraceRecorder ‣ ITM Config, set the ITM port to
  1.
...
The option Use internal buffer should typically remain disabled. It buffers the data in RAM
...
User Manual.
See Creating and Loading Traces ‣ Percepio TraceRecorder ‣ Using TraceRecorder v4.6 or later ‣ Stream ports (or search for ITM).
...
- Enable General Architecture Options ‣ Semihosting support for Arm and RISC-V targets
- Enable Subsystems and OS Services ‣ Tracing Support
- Under Subsystems and OS Services ‣ Tracing Support ‣ Tracing Format, select
  Percepio Tracealyzer
- Under Modules ‣ percepio ‣ TraceRecorder ‣ Recorder Start Mode, select
...
unless a different path is specified. Open this file in [Percepio Tracealyzer](https://percepio.com/tracealyzer) by selecting
File ‣ Open ‣ Open File.

#### Recorder Start Mode

You may have noticed the Recorder Start Mode option in the Tracealyzer examples above.
This decides when the tracing starts. With the option Start, the tracing begins directly
...
For streaming via RTT or ITM you may also use Start From Host or
Start Await Host. Both listens for start commands from the Tracealyzer application. The
latter option, Start Await Host, causes the TraceRecorder initialization to block until
...
Learn more about how to get started in the [Tracealyzer Getting Started Guides](https://percepio.com/tracealyzer/gettingstarted/).
...
To enable tracing support with [SEGGER SystemView](https://www.segger.com/products/development-tools/systemview/) add the
[SystemView RTT Tracing Snippet (rtt-tracing)](../../snippets/rtt-tracing/README.md#snippet-rtt-tracing) to your build command:
...
[`CONFIG_SEGGER_SYSVIEW_POST_MORTEM_MODE`](../../kconfig.md#CONFIG_SEGGER_SYSVIEW_POST_MORTEM_MODE "CONFIG_SEGGER_SYSVIEW_POST_MORTEM_MODE"). In this mode, a debugger can
...
[![SEGGER SystemView](https://docs.zephyrproject.org/4.1.0/_images/segger_systemview.png)
](https://docs.zephyrproject.org/4.1.0/_images/segger_systemview.png)
...
To get started, the simplest way is to use the CTF format with the [native\_sim](../../boards/native/native_sim/doc/index.md#native-sim)
