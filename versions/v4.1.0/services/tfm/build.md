---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/services/tfm/build.html
original_path: services/tfm/build.html
---
...
- tfm\_s - TF-M secure firmware
- tfm\_ns - TF-M non-secure app (only used by regression tests).
- bl2 - TF-M MCUboot, if enabled

For each of these, it creates .bin, .hex, .elf, and .axf files.

The TF-M build system also creates signed variants of tfm\_s and tfm\_ns, and a
...
The Zephyr build system usually signs both tfm\_s and the Zephyr non-secure app itself.
...
When [`CONFIG_TFM_BL2`](../../kconfig.md#CONFIG_TFM_BL2 "CONFIG_TFM_BL2") is set to `y`, TF-M uses a secure bootloader
...
using the [`CONFIG_TFM_KEY_FILE_S`](../../kconfig.md#CONFIG_TFM_KEY_FILE_S "CONFIG_TFM_KEY_FILE_S") and
[`CONFIG_TFM_KEY_FILE_NS`](../../kconfig.md#CONFIG_TFM_KEY_FILE_NS "CONFIG_TFM_KEY_FILE_NS") config flags.

To satisfy [PSA Certified Level 1](https://www.psacertified.org/security-certification/psa-certified-level-1/) requirements, **You MUST replace
...
[`CONFIG_TFM_KEY_FILE_S`](../../kconfig.md#CONFIG_TFM_KEY_FILE_S "CONFIG_TFM_KEY_FILE_S") and [`CONFIG_TFM_KEY_FILE_NS`](../../kconfig.md#CONFIG_TFM_KEY_FILE_NS "CONFIG_TFM_KEY_FILE_NS") config
flags.
...
file that contains all three binaries: bl2, tfm\_s, and the zephyr app. This
...
For more information on these tools look here: [Footprint and Memory Usage](../../develop/optimizations/tools.md#footprint-tools)

Use the `tfm_ram_report` to get the RAM report for TF-M secure firmware (tfm\_s).

Using west:

```shell
# From the root of the zephyr repository
west build -b mps2/an521/cpu0/ns samples/hello_world
...
cmake -Bbuild -GNinja -DBOARD=mps2/an521/cpu0/ns samples/hello_world

# Now run the build tool on the generated build system:
ninja -Cbuild tfm_ram_report
```

Use the `tfm_rom_report` to get the ROM report for TF-M secure firmware (tfm\_s).
