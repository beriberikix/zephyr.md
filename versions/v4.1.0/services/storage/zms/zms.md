---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/services/storage/zms/zms.html
...
| . | . | . |
| . | . | . |
| . | ID ATE\_b2 | ID ATE\_c2 |
| ID ATE\_a2 | ID ATE\_b1 | ID ATE\_c1 |
| ID ATE\_a1 | ID ATE\_b0 | ID ATE\_c0 |
| ID ATE\_a0 | GC\_done ATE | GC\_done ATE |
| Close ATE (cyc=1) | Close ATE (cyc=1) | Close ATE (cyc=1) |
| Empty ATE (cyc=1) | Empty ATE (cyc=2) | Empty ATE (cyc=2) |
...
See the [`zms_ate`](../../../doxygen/html/structzms__ate.md) structure.

Note

The CRC of the data is checked only when a full read of the data is made.
The CRC of the data is not checked for a partial read, as it is computed for the whole element.

Warning

Enabling the CRC feature on previously existing ZMS content that did not have it
...
The maximum single data length that can be written at once in a sector is 64K
...
For example for 4 sectors of 1024 bytes, free space for 8-byte length data is \(\frac{3 \times 944 \times 8}{16} = 1416 \, \text{ bytes}\).
...
See [Available space for user data](#available-space-for-user-data-key-value-pairs).
...
- If you are using devices that do not require an erase operation like RRAM or MRAM, [ZMS](#zms-api) is definitely the
...
- The total size of the storage partition should be set appropriately to achieve the best
  ...
  in the documentation. See [Available space for user data](#available-space-for-user-data-key-value-pairs).
