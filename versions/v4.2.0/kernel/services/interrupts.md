---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/kernel/services/interrupts.html
...
provides the [`k_is_in_isr()`](../../doxygen/html/group__isr__apis.md#ga8482b0dd2283d12677a9ebe321667d16) API to allow the routine to
...
[`CONFIG_ZERO_LATENCY_IRQS`](../../kconfig.md#CONFIG_ZERO_LATENCY_IRQS "CONFIG_ZERO_LATENCY_IRQS") to be enabled. Any interrupts
configured as zero-latency must also be declared as [direct ISRs](#direct-isrs) (and must not use the [`ISR_DIRECT_PM`](../../doxygen/html/group__isr__apis.md#ga3c1327551dfca7818975e3fbf1470227) in them), since
regular ISRs interact with the kernel. In addition to that, the flag
[`IRQ_ZERO_LATENCY`](../../doxygen/html/arch_2arm_2irq_8h.md#a1b5d8b88524f2fd81f32ed675c832a57) must be passed to the [`IRQ_DIRECT_CONNECT`](../../doxygen/html/group__isr__apis.md#gac6c8746ac28da6ce02b24714f4144ff3)
...
dynamic is possible on some architectures, see [Defining a ‘direct’ ISR](#direct-isrs).
...
[`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) or [`irq_connect_dynamic()`](../../doxygen/html/group__isr__apis.md#ga4e9915b92b09df49b99bc449f0cc31a1)), the interrupt line will
...
interrupts using [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) and [`irq_connect_dynamic()`](../../doxygen/html/group__isr__apis.md#ga4e9915b92b09df49b99bc449f0cc31a1) as
...
allow users to dynamically disconnect ISRs using [`irq_disconnect_dynamic()`](../../doxygen/html/group__isr__apis.md#gabdab7745edc1e15862e1772d8673fc00).
...
An ISR is defined at runtime by calling [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984). It must
then be enabled by calling [`irq_enable()`](../../doxygen/html/group__isr__apis.md#ga7ea700ee31e4ff036c997a554dbedfeb).
...
Since the [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) macro requires that all its parameters be
known at build time, in some cases this may not be acceptable. It is also
possible to install interrupts at runtime with
[`irq_connect_dynamic()`](../../doxygen/html/group__isr__apis.md#ga4e9915b92b09df49b99bc449f0cc31a1). It is used in exactly the same way as
[`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984):
...
- [Zero Latency Interrupts](#zlis) must always be declared as direct ISRs, since regular
  ISRs interact with the kernel

Zephyr supports so-called ‘direct’ interrupts, which are installed via
[`IRQ_DIRECT_CONNECT`](../../doxygen/html/group__isr__apis.md#gac6c8746ac28da6ce02b24714f4144ff3) and whose handlers are declared using
[`ISR_DIRECT_DECLARE`](../../doxygen/html/group__isr__apis.md#gaf86866cd07fd37f381d98866f8874ebf). These direct interrupts have some special
implementation requirements and a reduced feature set; see the definitions
of [`IRQ_DIRECT_CONNECT`](../../doxygen/html/group__isr__apis.md#gac6c8746ac28da6ce02b24714f4144ff3) and [`ISR_DIRECT_DECLARE`](../../doxygen/html/group__isr__apis.md#gaf86866cd07fd37f381d98866f8874ebf) for details.
...
`ARM_IRQ_DIRECT_DYNAMIC_CONNECT`, which can be used to declare a direct
...
The same restrictions regarding [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) described in [Defining a regular ISR](#defining-a-regular-isr)
...
If dynamic interrupts are enabled, [`irq_connect_dynamic()`](../../doxygen/html/group__isr__apis.md#ga4e9915b92b09df49b99bc449f0cc31a1) will allow sharing interrupts
...
The [`irq_disconnect_dynamic()`](../../doxygen/html/group__isr__apis.md#gabdab7745edc1e15862e1772d8673fc00) call will result in interrupt 24 becoming
unshared, meaning the system will act as if the first [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984)
...
The invocation of [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) will declare an instance of
...
directly in the file where [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) is used.
...
Any invocation of [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) will declare an instance of
struct \_isr\_list which is placed in a special .intList section:
...
[`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) also has a runtime component which programs the desired
...
1. For ‘direct’ interrupts declared with [`IRQ_DIRECT_CONNECT`](../../doxygen/html/group__isr__apis.md#gac6c8746ac28da6ce02b24714f4144ff3), the
   handler function will be placed here.
2. For regular interrupts declared with [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984), the address
...
[`CONFIG_GEN_IRQ_VECTOR_TABLE`](../../kconfig.md#CONFIG_GEN_IRQ_VECTOR_TABLE "CONFIG_GEN_IRQ_VECTOR_TABLE") option should be disabled.
...
interrupt vectors are created locally in place of invocation of [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) macro.
Then automatically generated linker scripts are used to place it in the right place in the memory.

This option requires enabling by the choose of [`CONFIG_ISR_TABLES_LOCAL_DECLARATION`](../../kconfig.md#CONFIG_ISR_TABLES_LOCAL_DECLARATION "CONFIG_ISR_TABLES_LOCAL_DECLARATION").
If this configuration is supported by the used architecture and toolchaing the
`CONFIG_ISR_TABLES_LOCAL_DECLARATION_SUPPORTED` is set.
See details of this option for the information about currently supported configurations.

Any invocation of [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) or [`IRQ_DIRECT_CONNECT`](../../doxygen/html/group__isr__apis.md#gac6c8746ac28da6ce02b24714f4144ff3) will declare an instance
...
Beside of the \_isr\_list\_sname the [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) macro generates an entry
...
The values created by [`IRQ_DIRECT_CONNECT`](../../doxygen/html/group__isr__apis.md#gac6c8746ac28da6ce02b24714f4144ff3) macro depends on the architecture.
...
Similar like for [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984), the created variable or function is placed
...
indexed by IRQ line is not used. Instead, the [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984) call
...
For interrupts declared with [`IRQ_DIRECT_CONNECT`](../../doxygen/html/group__isr__apis.md#gac6c8746ac28da6ce02b24714f4144ff3) the parameterless
...
configured vector in the IDT. This is used at runtime by [`IRQ_CONNECT`](../../doxygen/html/group__isr__apis.md#ga131739d1faf501a15590053817aba984)
