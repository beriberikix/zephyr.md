---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/samples/boards/boards.html
...
### [Ultra Low Power coprocessor (ULP)](espressif/ulp/index.md)

#### [Low-Power CPU (LP CORE)](espressif/ulp/lp_core/index.md)

- [Debug ULP](espressif/ulp/lp_core/debug_ulp/README.md#debug-ulp "Debug the LP Core in ESP32C6.")Debug the LP Core in ESP32C6.
- [Echo ULP](espressif/ulp/lp_core/echo_ulp/README.md#echo-ulp "Leverage Zephyr's UART API to use the LP UART on the ESP32-C6's LP core.")Leverage Zephyr's UART API to use the LP UART on the ESP32-C6's LP core.
- [Interrupt ULP](espressif/ulp/lp_core/interrupt_ulp/README.md#interrupt-ulp "HP Core interrupt LP Core.")HP Core interrupt LP Core.
...
- [Nordic IronSide SE firmware update](nordic/nrf_ironside/update/README.md#nrf_ironside_update "Update the Nordic IronSide SE firmware.")Update the Nordic IronSide SE firmware.
...
- [SPIS wake up](nordic/spis_wakeup/README.md#spis-wakeup "Reduce current consumption by handling the wake line while using an SPIS.")Reduce current consumption by handling the wake line while using an SPIS.
...
- [Audio output AMP sample.](nxp/adsp/rtxxx/amp_audio_loopback/README.md#amp_audio_loopback "AMP system example for NXP i.MX RTxxx platforms - audio loopback.")AMP system example for NXP i.MX RTxxx platforms - audio loopback.
- [Audio output AMP sample.](nxp/adsp/rtxxx/amp_audio_output/README.md#amp_audio_output "AMP system example for NXP i.MX RTxxx platforms - audio output.")AMP system example for NXP i.MX RTxxx platforms - audio output.
- [Blinky AMP sample](nxp/adsp/rtxxx/amp_blinky/README.md#amp_blinky "AMP system example for NXP i.MX RTxxx platforms - blinking LED.")AMP system example for NXP i.MX RTxxx platforms - blinking LED.
...
- [mbox API AMP sample](nxp/adsp/rtxxx/amp_mbox/README.md#amp_mbox "AMP system example for NXP i.MX RTxxx platforms - IPC using the mbox API.")AMP system example for NXP i.MX RTxxx platforms - IPC using the mbox API.
...
## [Renesas](renesas/index.md)

- [OpenAMP Linux Zephyr RPMsg](renesas/openamp_linux_zephyr/README.md#rz-openamp-linux-zephyr "Enable message exchange between two cores, with the application core running Linux and the real-time core running Zephyr, using the OpenAMP library.")Enable message exchange between two cores, with the application core running Linux
  and the real-time core running Zephyr, using the OpenAMP library.
- [Renesas comparator](renesas/comparator/README.md#renesas_comparator "Monitor the output of comparator.")Monitor the output of comparator.
- [Renesas ELC Sample](renesas/elc/README.md#renesas-elc "Integrating the Renesas ELC with PWM Functionality")Integrating the Renesas ELC with PWM Functionality
...
- [UART circular mode](st/uart/circular_dma/README.md#uart-circular-dma "Read data from the console and echo it back using a circular DMA configuration.")Read data from the console and echo it back using a circular DMA configuration.
