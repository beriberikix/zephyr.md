---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/samples/bluetooth/mtu_update/README.html
original_path: samples/bluetooth/mtu_update/README.html
---
...
](https://github.com/zephyrproject-rtos/zephyr/blob/main//samples/bluetooth/mtu_update/README.rst/..)
...
Set [`CONFIG_BT_L2CAP_TX_MTU`](../../../kconfig.md#CONFIG_BT_L2CAP_TX_MTU "CONFIG_BT_L2CAP_TX_MTU") to at least x

Set [`CONFIG_BT_BUF_ACL_RX_SIZE`](../../../kconfig.md#CONFIG_BT_BUF_ACL_RX_SIZE "CONFIG_BT_BUF_ACL_RX_SIZE") to at least x + L2CAP header
size (4 octets).

Set [`CONFIG_BT_BUF_ACL_RX_SIZE`](../../../kconfig.md#CONFIG_BT_BUF_ACL_RX_SIZE "CONFIG_BT_BUF_ACL_RX_SIZE") to at least x + L2CAP header +
SDU length field length (6 octets) if using
[`CONFIG_BT_EATT`](../../../kconfig.md#CONFIG_BT_EATT "CONFIG_BT_EATT").

Ensure the remote side supports the same MTUs.

> Question: I only want to *send* large packets. I don’t need to receive large
> ones.
> Do I still need to set [`CONFIG_BT_BUF_ACL_RX_SIZE`](../../../kconfig.md#CONFIG_BT_BUF_ACL_RX_SIZE "CONFIG_BT_BUF_ACL_RX_SIZE")?

Answer: Yes. [[1]](#mtu-exchange) The Bluetooth specification mandates a symmetric MTU for ATT.
...
According to the Bluetooth specification, [[2]](#mtu) MTU is the maximum size of
SDUs.
However, in Zephyr, we can assume that it also represents the maximum size of
the PDUs. Because, in Bluetooth LE, [[3]](#sud-encapsulation) unless we are using L2CAP dynamic
channels, SDUs are not segmented.
The Kconfig symbol used to configure the size of the TX MTU is
[`CONFIG_BT_L2CAP_TX_MTU`](../../../kconfig.md#CONFIG_BT_L2CAP_TX_MTU "CONFIG_BT_L2CAP_TX_MTU"). There is no Kconfig symbol to update
...
[`CONFIG_BT_BUF_ACL_RX_SIZE`](../../../kconfig.md#CONFIG_BT_BUF_ACL_RX_SIZE "CONFIG_BT_BUF_ACL_RX_SIZE").
The resulting L2CAP RX MTU will be the value of this Kconfig symbol minus the
L2CAP header size.

![Diagram of the MTUs and their corresponding Kconfig symbols](https://docs.zephyrproject.org/4.2.0/_images/mtu.svg)
...
This sample can be found under [samples/bluetooth/mtu\_update](https://github.com/zephyrproject-rtos/zephyr/blob/main/samples/bluetooth/mtu_update) in
the Zephyr tree.
...
Bluetooth Core Specification v. 5.3: Vol. 3, Part F, 3.4.2

[[2](#id2)]

Bluetooth Core Specification v. 5.3: Vol. 3, Part A, 5.1

[[3](#id3)]

Bluetooth Core Specification v. 5.3: Vol. 3, Part A, 7.3
