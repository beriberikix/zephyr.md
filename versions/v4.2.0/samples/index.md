---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/samples/index.html
...
- [Call Control Profile (CCP) Call Control Server](bluetooth/ccp_call_control_client/README.md#bluetooth_ccp_call_control_client "CCP Call Control Server sample that registers one or more TBS bearers and advertises the TBS UUID(s).")CCP Call Control Server sample that registers one or more TBS bearers and advertises the
  TBS UUID(s).
- [Call Control Profile (CCP) Call Control Server](bluetooth/ccp_call_control_server/README.md#bluetooth_ccp_call_control_server "CCP Call Control Server sample that registers one or more TBS bearers and advertises the TBS UUID(s).")CCP Call Control Server sample that registers one or more TBS bearers and advertises the
  TBS UUID(s).
- [Central](bluetooth/central/README.md#ble_central "Implement basic Bluetooth LE Central role functionality (scanning and connecting).")Implement basic Bluetooth LE Central role functionality
...
- [Channel Sounding](bluetooth/channel_sounding/README.md#ble_cs "Use Channel Sounding functionality.")Use Channel Sounding functionality.
...
- [Mesh](bluetooth/mesh/README.md#ble_mesh "Use basic Bluetooth Mesh functionality.")Use basic Bluetooth Mesh functionality.
...
- [ST Bluetooth LE Sensor Demo](bluetooth/st_ble_sensor/README.md#bluetooth_st_ble_sensor "Export vendor-specific GATT services over Bluetooth.")Export vendor-specific GATT services over Bluetooth.
...
### [Bluetooth Classic](bluetooth/classic/index.md)

- [Hands-free](bluetooth/classic/handsfree/README.md#bluetooth_handsfree "Use the Hands-Free Profile (HFP) APIs.")Use the Hands-Free Profile (HFP) APIs.
- [Hands-free Audio Gateway (AG)](bluetooth/classic/handsfree_ag/README.md#bluetooth_handsfree_ag "Use the Hands-Free Profile Audio Gateway (AG) APIs.")Use the Hands-Free Profile Audio Gateway (AG) APIs.
...
#### [Ultra Low Power coprocessor (ULP)](boards/espressif/ulp/index.md)

##### [Low-Power CPU (LP CORE)](boards/espressif/ulp/lp_core/index.md)

- [Debug ULP](boards/espressif/ulp/lp_core/debug_ulp/README.md#debug-ulp "Debug the LP Core in ESP32C6.")Debug the LP Core in ESP32C6.
- [Echo ULP](boards/espressif/ulp/lp_core/echo_ulp/README.md#echo-ulp "Leverage Zephyr's UART API to use the LP UART on the ESP32-C6's LP core.")Leverage Zephyr's UART API to use the LP UART on the ESP32-C6's LP core.
- [Interrupt ULP](boards/espressif/ulp/lp_core/interrupt_ulp/README.md#interrupt-ulp "HP Core interrupt LP Core.")HP Core interrupt LP Core.
...
- [Coresight STM benchmark](boards/nordic/coresight_stm/README.md#coresight_stm_sample)
...
- [Nordic IronSide SE firmware update](boards/nordic/nrf_ironside/update/README.md#nrf_ironside_update "Update the Nordic IronSide SE firmware.")Update the Nordic IronSide SE firmware.
...
- [SPIS wake up](boards/nordic/spis_wakeup/README.md#spis-wakeup "Reduce current consumption by handling the wake line while using an SPIS.")Reduce current consumption by handling the wake line while using an SPIS.
...
- [Audio output AMP sample.](boards/nxp/adsp/rtxxx/amp_audio_loopback/README.md#amp_audio_loopback "AMP system example for NXP i.MX RTxxx platforms - audio loopback.")AMP system example for NXP i.MX RTxxx platforms - audio loopback.
- [Audio output AMP sample.](boards/nxp/adsp/rtxxx/amp_audio_output/README.md#amp_audio_output "AMP system example for NXP i.MX RTxxx platforms - audio output.")AMP system example for NXP i.MX RTxxx platforms - audio output.
- [Blinky AMP sample](boards/nxp/adsp/rtxxx/amp_blinky/README.md#amp_blinky "AMP system example for NXP i.MX RTxxx platforms - blinking LED.")AMP system example for NXP i.MX RTxxx platforms - blinking LED.
...
- [mbox API AMP sample](boards/nxp/adsp/rtxxx/amp_mbox/README.md#amp_mbox "AMP system example for NXP i.MX RTxxx platforms - IPC using the mbox API.")AMP system example for NXP i.MX RTxxx platforms - IPC using the mbox API.
...
### [Renesas](boards/renesas/index.md)

- [OpenAMP Linux Zephyr RPMsg](boards/renesas/openamp_linux_zephyr/README.md#rz-openamp-linux-zephyr "Enable message exchange between two cores, with the application core running Linux and the real-time core running Zephyr, using the OpenAMP library.")Enable message exchange between two cores, with the application core running Linux
  and the real-time core running Zephyr, using the OpenAMP library.
- [Renesas comparator](boards/renesas/comparator/README.md#renesas_comparator "Monitor the output of comparator.")Monitor the output of comparator.
- [Renesas ELC Sample](boards/renesas/elc/README.md#renesas-elc "Integrating the Renesas ELC with PWM Functionality")Integrating the Renesas ELC with PWM Functionality
...
- [UART circular mode](boards/st/uart/circular_dma/README.md#uart-circular-dma "Read data from the console and echo it back using a circular DMA configuration.")Read data from the console and echo it back using a circular DMA configuration.
...
- [Min-Heap Data Structure](data_structures/min-heap/README.md#min-heap "Demonstrate usage of a min-heap implementation in a Zephyr application.")Demonstrate usage of a min-heap implementation in a Zephyr application.
...
- [Auxiliary digits display](drivers/auxdisplay_digits/README.md#auxdisplay_digits "Output increasing numbers to an auxiliary display.")Output increasing numbers to an auxiliary display.
...
- [Fuel Gauge](drivers/fuel_gauge/README.md#fuel_gauge "Use fuel gauge API to access fuel gauge properties and get charge information.")Use fuel gauge API to access fuel gauge properties and get charge information.
...
- [PSI5 interface](drivers/psi5/README.md#psi5 "Use the PSI5 (Peripheral Sensor Interface) driver.")Use the PSI5 (Peripheral Sensor Interface) driver.
- [Real-Time Clock (RTC)](drivers/rtc/README.md#rtc "Set and read the date/time from a Real-Time Clock.")Set and read the date/time from a Real-Time Clock.
- [SENT interface](drivers/sent/README.md#sent "Use the SENT (Single Edge Nibble Transmission) driver.")Use the SENT (Single Edge Nibble Transmission) driver.
...
- [I2C RTIO loopback](drivers/i2c/rtio_loopback/README.md#i2c-rtio-loopback "Perform I2C transfers between I2C controller and custom I2C target using RTIO.")Perform I2C transfers between I2C controller and custom I2C target using RTIO.
...
- [Ambiq MSPI timing scan](drivers/mspi/mspi_timing_scan/README.md#mspi-timing-scan "Find the appropriate timing for a given device on a given board.")Find the appropriate timing for a given device on a given board.
...
### [Stepper](drivers/stepper/index.md)

- [Stepper](drivers/stepper/generic/README.md#stepper "Rotate a stepper motor in 4 different modes.")Rotate a stepper motor in 4 different modes.
- [TMC50XX stepper](drivers/stepper/tmc50xx/README.md#tmc50xx "Rotate a TMC50XX stepper motor and change velocity at runtime.")Rotate a TMC50XX stepper motor and change velocity at runtime.
...
- [UART ASYNC API](drivers/uart/async_api/README.md#uart_async "Demonstrate the use of the asynchronous API")Demonstrate the use of the asynchronous API
...
- [LVGL Multi-display](modules/lvgl/multi_display/README.md#lvgl-multi-display "Run different LVGL demos on multiple displays.")Run different LVGL demos on multiple displays.
- [LVGL screen transparency](modules/lvgl/screen_transparency/README.md#lvgl-screen-transparency "Rendering to screens with transparency support using LVGL.")Rendering to screens with transparency support using LVGL.

### [PMCI](modules/pmci/index.md)

#### [MCTP](modules/pmci/mctp/mctp.md)

- [MCTP Endpoint Sample](modules/pmci/mctp/endpoint/README.md#mctp_endpoint_sample "Create an MCTP endpoint over UART.")Create an MCTP endpoint over UART.
- [MCTP Host Sample](modules/pmci/mctp/host/README.md#mctp_host_sample "Create an MCTP host over UART.")Create an MCTP host over UART.
...
- [Network packet filter](net/pkt_filter/README.md#net-pkt-filter "Install network packet filter hooks.")Install network packet filter hooks.
...
### [OpenThread](net/openthread/README.md)

- [OpenThread co-processor](net/openthread/coprocessor/README.md#openthread-coprocessor "Build a Thread border-router using OpenThread's co-processor designs.")Build a Thread border-router using OpenThread's co-processor designs.
- [OpenThread CoAP client and server application](net/openthread/coap/README.md#ot-coap "Build a Full Thread Device (FTD) CoAP server and client.")Build a Full Thread Device (FTD) CoAP server and client.
- [OpenThread shell](net/openthread/shell/README.md#openthread-shell "Test Thread and IEEE 802.15.4 using the OpenThread shell.")Test Thread and IEEE 802.15.4 using the OpenThread shell.
...
- [Barometric pressure and temperature sensor interrupt example](sensor/pressure_interrupt/README.md#pressure_interrupt "Manage interrupts from a barometric pressure and temperature sensor.")Manage interrupts from a barometric pressure and temperature sensor.
- [Barometric pressure and temperature sensor polling example](sensor/pressure_polling/README.md#pressure_polling "Get barometric pressure and temperature data from a sensor.")Get barometric pressure and temperature data from a sensor.
...
- [BMG160 3-axis gyroscope](sensor/bmg160/README.md#bmg160 "Get temperature, and angular velocity data from a BMG160 sensor.")Get temperature, and angular velocity data from a BMG160 sensor.
...
- [Generic 6DOF Motion Dataready](sensor/6dof_motion_drdy/README.md#6dof_motion_drdy "Get 6-Axis accelerometer and gyroscope data from a sensor (data ready interrupt mode).")Get 6-Axis accelerometer and gyroscope data from a sensor (data ready interrupt mode).
...
- [Generic device FIFO streaming](sensor/stream_fifo/README.md#stream_fifo "Get accelerometer/gyroscope/temperature FIFO data frames from a sensor using SENSOR_TRIG_FIFO_WATERMARK as a trigger.")Get accelerometer/gyroscope/temperature FIFO data frames from a sensor using
  SENSOR\_TRIG\_FIFO\_WATERMARK as a trigger.
- [Generic device sample streaming using Data Ready trigger](sensor/stream_drdy/README.md#stream_drdy "Get accelerometer data frames from a sensor using SENSOR_TRIG_DATA_READY.")Get accelerometer data frames from a sensor using SENSOR\_TRIG\_DATA\_READY.
...
- [Generic distance measurement](sensor/distance_polling/README.md#distance_polling "Measure distance to an object using a distance sensor")Measure distance to an object using a distance sensor
- [Generic Light Sensor Polling](sensor/light_polling/README.md#light_sensor_polling "Get illuminance data from a light sensor.")Get illuminance data from a light sensor.
...
- [Heart Rate Sensor](sensor/heart_rate/README.md#heart_rate "Get heart rate data from a sensor (polling mode).")Get heart rate data from a sensor (polling mode).
...
- [LPS22HH Temperature and Pressure Sensor](sensor/lps22hh/README.md#lps22hh "Get pressure and temperature data from an LPS22HH sensor (polling & trigger mode).")Get pressure and temperature data from an LPS22HH sensor (polling & trigger mode).
- [LPS22HH Temperature and Pressure Sensor (I3C)](sensor/lps22hh_i3c/README.md#lps22hh_i3c "Get pressure and temperature data from an LPS22HH sensor over I3C (polling & trigger mode).")Get pressure and temperature data from an LPS22HH sensor over I3C (polling &
  trigger mode).
...
- [PAJ7620 Gesture Sensor](sensor/paj7620_gesture/README.md#paj7620_gesture "Get hand gesture data from PAJ7620 sensor.")Get hand gesture data from PAJ7620 sensor.
...
- [Sensor Clock](sensor/clock/README.md#sensor_clock "Test and debug Sensor Clock functionality.")Test and debug Sensor Clock functionality.
...
- [TDK Advanced Pedometer and Event Detection (APEX)](sensor/tdk_apex/README.md#tdk_apex "Get TDK APEX event detection (trigger mode).")Get TDK APEX event detection (trigger mode).
...
- [VEML6031 High Accuracy Ambient Light Sensor](sensor/veml6031/README.md#veml6031 "Get ambient light data from a VEML4040 sensor (polling mode).")Get ambient light data from a VEML4040 sensor (polling mode).
...
- [nPM2100 EK](shields/npm2100_ek/doc/index.md#npm2100_ek "Interact with the nPM2100 PMIC using the EK buttons and the shell interface.")Interact with the nPM2100 PMIC using the EK buttons and the shell interface.
...
- [UUID](subsys/uuid/README.md#uuid "Manipulate UUID v4 and v5 compliant with IETF RFC 9562.")Manipulate UUID v4 and v5 compliant with IETF RFC 9562.
...
- [Fatfs filesystem fstab](subsys/fs/fatfs_fstab/README.md#fatfs-fstab "Define fatfs filesystems in the devicetree.")Define fatfs filesystems in the devicetree.
...
- [Bluetooth logging backend](subsys/logging/ble_backend/README.md#logging-ble-backend "Send log messages over Bluetooth using the Bluetooth logging backend.")Send log messages over Bluetooth using the Bluetooth logging backend.
...
### [Real Time I/O (RTIO)](subsys/rtio/rtio.md)

- [Sensor batch processing](subsys/rtio/sensor_batch_processing/README.md#sensor_batch_processing "Implement a sensor device using RTIO for asynchronous data processing.")Implement a sensor device using RTIO for asynchronous data processing.
...
- [Console over USB CDC ACM](subsys/usb/console-next/README.md#usbd-cdc-acm-console "Output "Hello World!" to the console over USB CDC ACM.")Output "Hello World!" to the console over USB CDC ACM.
...
- [USB CDC-ACM bridge](subsys/usb/cdc_acm_bridge/README.md#usb-cdc-acm-bridge "Use USB CDC-ACM driver to implement a serial port bridge.")Use USB CDC-ACM driver to implement a serial port bridge.
- [USB device testing application](subsys/usb/testusb/README.md#testusb-app "Test USB device drivers using a loopback function.")Test USB device drivers using a loopback function.
- [USB DFU](subsys/usb/dfu-next/README.md#dfu-next "Implement a basic USB DFU device")Implement a basic USB DFU device
...
- [USB MIDI2 device](subsys/usb/midi/README.md#usb-midi2-device "Implements a simple USB MIDI loopback and keyboard device.")Implements a simple USB MIDI loopback and keyboard device.
...
- [USB Video webcam](subsys/usb/uvc/README.md#uvc "Send video frames over USB.")Send video frames over USB.
...
- [WebUSB-next](subsys/usb/webusb-next/README.md#webusb-next "Receive and echo data from a web page using WebUSB API.")Receive and echo data from a web page using WebUSB API.
