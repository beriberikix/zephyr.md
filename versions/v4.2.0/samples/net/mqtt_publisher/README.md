---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/samples/net/mqtt_publisher/README.html
original_path: samples/net/mqtt_publisher/README.html
---
...
](https://github.com/zephyrproject-rtos/zephyr/blob/main//samples/net/mqtt_publisher/README.rst/..)
...
See the [MQTT v3.1.1 spec](https://docs.oasis-open.org/mqtt/mqtt/v3.1.1/mqtt-v3.1.1.html) and [MQTT v5.0 spec](https://docs.oasis-open.org/mqtt/mqtt/v5.0/mqtt-v5.0.html) for more information about
MQTT v3.1.1 and v5.0, respectively.

The source code of this sample application can be found at:
[samples/net/mqtt\_publisher](https://github.com/zephyrproject-rtos/zephyr/blob/main/samples/net/mqtt_publisher).
...
Open the `src/config.h` file and set the IP addresses according
to the LAN environment.
Alternatively, set the IP addresses in the `prj.conf` file.

The file `src/config.h` also contains some variables that may be changed:
...
be enabled by changing the default value of APP\_BLUEMIX\_TOPIC from 0 to 1:
...
of this sample application (i.e., [samples/net/mqtt\_publisher](https://github.com/zephyrproject-rtos/zephyr/blob/main/samples/net/mqtt_publisher)) and type:

```shell
west build -b frdm_k64f samples/net/mqtt_publisher
west flash
```
...
Alternatively, a [publicly available Mosquitto MQTT server/broker](https://test.mosquitto.org/) is available to quickly and easily
...
- In `src/test_certs.h`, set `ca_certificate[]` using the certificate
  contents (or set it to its filename if the socket offloading feature is
  enabled on your platform and [`CONFIG_TLS_CREDENTIAL_FILENAMES`](../../../kconfig.md#CONFIG_TLS_CREDENTIAL_FILENAMES "CONFIG_TLS_CREDENTIAL_FILENAMES") is
  set to `y`).
- In `src/config.h`, set SERVER\_ADDR to the IP address to connect to,
  i.e., the IP address of test.mosquitto.org `"37.187.106.16"`
- In `src/main.c`, set TLS\_SNI\_HOSTNAME to `"test.mosquitto.org"`
...
The MQTT logging backend uses the [`log_backend_mqtt_client_set()`](../../../doxygen/html/group__log__backend__mqtt.md#ga75e1b3fb99bb8211eaa684fcaa41882a) API to
register an MQTT client for publishing log messages. The backend only uses the
client’s [`mqtt_publish()`](../../../doxygen/html/group__mqtt__socket.md#ga57745efa1bf6fbdf7eb1b3f01623e4c7) function and does not manage the client’s
...
- [`CONFIG_LOG_BACKEND_MQTT_TOPIC_DEFAULT`](../../../kconfig.md#CONFIG_LOG_BACKEND_MQTT_TOPIC_DEFAULT "CONFIG_LOG_BACKEND_MQTT_TOPIC_DEFAULT"): Topic for publishing logs (default: “zephyr/logs”)
- [`CONFIG_LOG_BACKEND_MQTT_QOS`](../../../kconfig.md#CONFIG_LOG_BACKEND_MQTT_QOS "CONFIG_LOG_BACKEND_MQTT_QOS"): QoS level for log messages (default: 0)
- [`CONFIG_LOG_BACKEND_MQTT_RETAIN`](../../../kconfig.md#CONFIG_LOG_BACKEND_MQTT_RETAIN "CONFIG_LOG_BACKEND_MQTT_RETAIN"): Whether to retain log messages (default: disabled)
- [`CONFIG_LOG_BACKEND_MQTT_MAX_MSG_SIZE`](../../../kconfig.md#CONFIG_LOG_BACKEND_MQTT_MAX_MSG_SIZE "CONFIG_LOG_BACKEND_MQTT_MAX_MSG_SIZE"): Maximum log message size (default: 256 bytes)

### Running on cc3220sf\_launchxl

Offloading on cc3220sf\_launchxl also provides DHCP services, so the sample
uses dynamic IP addresses on this board.

By default, the sample is set up to connect to the broker at the address
specified by SERVER\_ADDR in config.h. If the broker is secured using TLS, users
...
and name it “ca\_cert.der”.

In addition, TLS\_SNI\_HOSTNAME in main.c should be defined to match the
Common Name (CN) in the certificate file in order for the TLS domain
name verification to succeed.

See the note on Provisioning and Fast Connect in [CC3220SF LaunchXL](../../../boards/ti/cc3220sf_launchxl/doc/index.md#cc3220sf_launchxl).
