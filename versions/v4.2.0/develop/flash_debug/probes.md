---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/develop/flash_debug/probes.html
original_path: develop/flash_debug/probes.html
---
...
[tracing program execution](../../services/tracing/index.md#tracing). For details on the related host
software supported by Zephyr, see [Flash & Debug Host Tools](host-tools.md#flash-debug-host-tools).
...
[OpenSDA](#opensda-onboard-debug-probe),
[LPC-Link2](#lpc-link2-onboard-debug-probe), or
[MCU-Link](#mcu-link-onboard-debug-probe), probes depending on
the microcontroller the debug probe firmware runs on.
ST boards have the [ST-LINK probe](#stlink-v21-onboard-debug-probe). Each
...
the [MCU-Link Onboard Debug Probe](#mcu-link-onboard-debug-probe), [LPC-LINK2 Onboard Debug Probe](#lpc-link2-onboard-debug-probe)
and [OpenSDA Onboard Debug Probe](#opensda-onboard-debug-probe). Each of these probes is implemented
...
- LPC55S69: [MCU-Link Onboard Debug Probe](#mcu-link-onboard-debug-probe)
- LPC4322: [LPC-LINK2 Onboard Debug Probe](#lpc-link2-onboard-debug-probe)
- MK20: [OpenSDA Onboard Debug Probe](#opensda-onboard-debug-probe)

For example, the [FRDM-K64F](../../boards/nxp/frdm_k64f/doc/index.md#frdm_k64f) board has an MK20 debug microcontroller,
so this board uses the [OpenSDA Onboard Debug Probe](#opensda-onboard-debug-probe).
...
- [MCU-Link CMSIS-DAP Onboard Debug Probe](#mcu-link-cmsis-onboard-debug-probe) (default firmware)
- [MCU-Link JLink Onboard Debug Probe](#mcu-link-jlink-onboard-debug-probe)

This probe is programmed using the MCU-Link host tools, which are installed
with the [LinkServer Debug Host Tools](host-tools.md#linkserver-debug-host-tools). NXP recommends using NXP’s
[MCUXpresso Installer](https://www.nxp.com/lgfiles/updates/mcuxpresso/MCUXpressoInstaller.exe) to install the Linkserver tools.
...
3. USB to UART, SPI and/or I2C interfaces (depending on MCU-Link
   type/implementation)
4. Energy measurements of the target MCU

This debug probe is compatible with the following debug host tools:

- [LinkServer Debug Host Tools](host-tools.md#linkserver-debug-host-tools)
...
1. Make sure the MCU-Link utility is present on your host machine. This can
   be done by installing [LinkServer Debug Host Tools](host-tools.md#linkserver-debug-host-tools).
2. Put the MCU-Link microcontroller into DFU boot mode by attaching the DFU
   ...
   on the LPC55S69.
3. Run the `program_CMSIS` script, found in the installed MCU-Link `scripts`
   folder.
...
- [J-Link Debug Host Tools](host-tools.md#jlink-debug-host-tools)
...
2. Put the MCU-Link microcontroller into DFU boot mode by attaching the DFU
   ...
   also be referred to as the ISP jumper, and will be connected to `PIO0_5`
   on the LPC55S69.
3. Run the `program_JLINK` script, found in the installed MCU-Link `scripts`
   folder.
4. Remove the DFU jumper and power cycle the board.
...
- [LPC-LINK2 CMSIS DAP Onboard Debug Probe](#lpclink2-cmsis-onboard-debug-probe)
- [LPC-Link2 J-Link Onboard Debug Probe](#lpclink2-jlink-onboard-debug-probe)
- [LPC-Link2 DAPLink Onboard Debug Probe](#lpclink2-daplink-onboard-debug-probe) (default firmware)
...
1. Make sure the LPCScrypt utility is present on your host machine. This can
   be done by installing [LinkServer Debug Host Tools](host-tools.md#linkserver-debug-host-tools).
2. Put the LPC-Link2 microcontroller into DFU boot mode by attaching the DFU
   jumper, then connecting to the USB debug port on the board. This
   jumper is connected to `P2_6` on the LPC4322 SOC.
3. Run the `program_CMSIS` script, found in the installed LPCScrypt `scripts`
   folder.
4. Remove the DFU jumper and power cycle the board.
...
On some boards, the J-Link probe firmware will no longer power the
...
Verify the firmware supports your board by visiting [Firmware for LPCXpresso](https://www.segger.com/products/debug-probes/j-link/models/other-j-links/lpcxpresso-on-board/)
...
2. Put the LPC-Link2 microcontroller into DFU boot mode by attaching the DFU
   jumper, then connecting to the USB debug port on the board. This
   jumper is connected to `P2_6` on the LPC4322 SOC.
3. Run the `program_JLINK` script, found in the installed LPCScrypt `scripts`
   folder.
4. Remove the DFU jumper and power cycle the board.
...
the [LPC-LINK2 CMSIS DAP Onboard Debug Probe](#lpclink2-cmsis-onboard-debug-probe) firmware following the
instructions provided above. For details on programming the DAPLink firmware,
see [NXP AN13206](https://www.nxp.com/docs/en/application-note/AN13206.pdf).
...
- [OpenSDA DAPLink Onboard Debug Probe](#opensda-daplink-onboard-debug-probe) (default firmware)
- [OpenSDA J-Link Onboard Debug Probe](#opensda-jlink-onboard-debug-probe)

### OpenSDA DAPLink Onboard Debug Probe

This debug probe firmware is compatible with the following debug host tools:

- [pyOCD Debug Host Tools](host-tools.md#pyocd-debug-host-tools)
- [OpenOCD Debug Host Tools](host-tools.md#openocd-debug-host-tools)
- [LinkServer Debug Host Tools](host-tools.md#linkserver-debug-host-tools)

This probe is realized by programming the OpenSDA microcontroller with DAPLink
OpenSDA firmware. NXP provides [OpenSDA DAPLink Board-Specific Firmwares](https://www.nxp.com/opensda).
...
2. After you power on the board, release the reset button. A USB mass storage
...
4. Power cycle the board, this time without holding the reset button. You
...
OpenSDA firmware. Segger provides [OpenSDA J-Link Generic Firmwares](https://www.segger.com/downloads/jlink/#JLinkOpenSDAGenericFirmwares) and
[OpenSDA J-Link Board-Specific Firmwares](https://www.segger.com/downloads/jlink/#JLinkOpenSDABoardSpecificFirmwares), where the latter is generally
...
[Segger J-Link](https://www.segger.com/products/debug-probes/j-link/) is a family of external debug probes, including J-Link EDU,
...
- [OpenOCD Debug Host Tools](host-tools.md#openocd-debug-host-tools)
- [J-Link Debug Host Tools](host-tools.md#jlink-debug-host-tools)
- [STM32CubeCLT Flash & Debug Host Tools](host-tools.md#stm32cubeclt-host-tools)
...
firmware please visit: [Segger over ST-Link](https://www.segger.com/products/debug-probes/j-link/models/other-j-links/st-link-on-board/)
...
For more information about West and available options, see [West (Zephyr’s meta-tool)](../west/index.md#west).

If you configured your Zephyr application to use [Segger RTT](https://www.segger.com/jlink-rtt.html) console instead,
open telnet:
...
application you are running, such as disable UART\_CONSOLE in menuconfig

### Updating or restoring ST-Link firmware

ST-Link firmware can be updated using [STM32CubeProgrammer Tool](https://www.st.com/en/development-tools/stm32cubeprog.html).
...
Where board\_uid can be obtained using twister’s generate-hardware-map
option. For more information about twister and available options, see
[Test Runner (Twister)](../test/twister.md#twister-script).

## NXP S32 Debug Probe

[NXP S32 Debug Probe](https://www.nxp.com/design/software/automotive-software-and-tools/s32-debug-probe:S32-DP) enables NXP S32 target system debugging via a standard
...
host tools as in indicated in [NXP S32 Debug Probe Host Tools](host-tools.md#nxp-s32-debug-host-tools) before you program
the firmware.

## Black Magic Probe

The Black Magic Probe is an open-source hardware for debugging that is designed
to be used with [Black Magic Debug](https://black-magic.org/index.html) firmware.
The firmware incorporates GDB Server so that you can connect directly from `gdb`
to the target device.

Some of the STM32F103-based boards can run the [Black Magic Debug](https://black-magic.org/index.html) firmware.
See [Black Magic Debug supported hardware](https://black-magic.org/hardware.html).
