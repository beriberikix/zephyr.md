---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/develop/west/release-notes.html
original_path: develop/west/release-notes.html
---
...
- Added support for [West aliases](alias.md#west-aliases) commands.
- Adopt the [pyproject TOML specification](https://packaging.python.org/en/latest/specifications/pyproject-toml/) for packaging.
...
- New `west grep` command for running a “grep tool” in your west workspace’s
  repositories. Currently, `git grep`, [ripgrep](https://github.com/BurntSushi/ripgrep#readme), and standard `grep` are
...
- The manifest file format now supports a `description` field in each
  `projects:` element. See [Projects](manifest.md#west-manifests-projects) for examples.
...
- `west compare` now always prints information about
  [The manifest-rev branch](workspaces.md#west-manifest-rev).
...
- Support for a new `manifest.project-filter` configuration option.
  See [Built-in Configuration Options](config.md#west-config-index) for details. The `west manifest --freeze`
...
- Manifest schema version “1.0” is now available for use in this release. This
  ...
  field. See [Version](manifest.md#west-manifest-schema-version) for details on this feature.
...
- The [West APIs](west-apis.md#west-apis) are now declared stable. Any breaking changes will be
...
- `west update`: new `--submodule-init-config` option.
  See commit [9ba92b05](https://github.com/zephyrproject-rtos/west/commit/9ba92b054500d75518ff4c4646590bfe134db523) for details.
...
- The west package now contains the metadata files necessary for some static
  analyzers (such as [mypy](https://www.mypy-lang.org/)) to auto-detect its type annotations.
  See commit [d9f00e24](https://github.com/zephyrproject-rtos/west/commit/d9f00e242b8cb297b56e941982adf231281c6bae) for details.
...
- the `west.log` module is now deprecated. This module uses global state,
  ...
  modules may rely on.
- The [west.commands](west-apis.md#west-apis-commands) module got some new APIs which lay groundwork
...
- The `west.manifest.MalformedConfig` exception type has been moved to the
  [west.configuration](west-apis.md#west-apis-configuration) module
...
- When calling west.manifest.Manifest.from\_file() when outside of a
  workspace, west again falls back on the ZEPHYR\_BASE environment
...
[API](west-apis.md#west-apis) changes:
...
- `west.manifest.ManifestImportFailed`: the constructor can now take any
  value; this can be used to reflect failed imports from a [map](manifest.md#west-manifest-import-map) or other compound value.
...
- West now works on the [MSYS2](https://www.msys2.org/) platform.
- West manifest files can now contain arbitrary user data associated with each
  project. See [Repository user data](manifest.md#west-project-userdata) for details.
...
- The manifest file parser was incorrectly allowing project names which contain
  ...
  [Projects](manifest.md#west-manifests-projects) for examples.
- The changes made in west v0.10.1 to the `west init --manifest-rev` option
  ...
  instead of `git init` and `git fetch`. See [issue #522](https://github.com/zephyrproject-rtos/west/issues/522) for details.
- The `WEST_CONFIG_LOCAL` environment variable now correctly
  overrides the default location, `<workspace topdir>/.west/config`.
- `west update --fetch=smart` (`smart` is the default) now correctly skips
  fetches for project revisions which are [lightweight tags](https://git-scm.com/book/en/v2/Git-Basics-Tagging) (it already
...
- `west.manifest.Manifest.get_projects()` avoids incorrect results in
  some edge cases described in [issue #523](https://github.com/zephyrproject-rtos/west/issues/523).
...
- `west update` now supports `--narrow`, `--name-cache`, and
  ...
  `update.name-cache`, and `update.path-cache` [Configuration](config.md#west-config) options.
...
- the [west.manifest](west-apis.md#west-apis-manifest) module has fixed docstrings for the Project
  class
...
- The [west init](built-in.md#west-init) command’s `--manifest-rev` (`--mr`) option no longer
...
- The `name` key in a project’s [submodules list](manifest.md#west-manifest-submodules) is now optional.

Bug fixes:

- West now checks that the manifest schema version is one of the explicitly
  allowed values documented in [Version](manifest.md#west-manifest-schema-version). The old
...
- A manifest file’s `group-filter` is now propagated through an `import`.
  ...
  are also imported. For details, see [Group Filters and Imports](manifest.md#west-group-filter-imports).
  ...
  [Version](manifest.md#west-manifest-schema-version) for more information on schema versions.

  See [west pull request #482](https://github.com/zephyrproject-rtos/west/pull/482) for the motivation
...
- West manifests now support [Git Submodules in Projects](manifest.md#west-manifest-submodules). This allows you
  to clone [Git submodules](https://git-scm.com/book/en/v2/Git-Tools-Submodules) into a west project
  repository in addition to the project repository itself.
- West manifests now support [Project Groups](manifest.md#west-manifest-groups). Project groups can be
...
- The `west config` command and `west.configuration` API did not correctly
  ...
  has been fixed; see [commit 36f3f91e](https://github.com/zephyrproject-rtos/west/commit/36f3f91e270782fb05f6da13800f433a9c48f130)
  for details.
...
- A bug affecting the behavior of the `west init -l .` command was fixed; see
  [issue #435](https://github.com/zephyrproject-rtos/west/issues/435).
...
- The [Manifest Imports](manifest.md#west-manifest-import) feature now supports the terms `allowlist`
...
- Manifest import mappings now support a `path-prefix:` key, which places
  ...
  See [Example 3.4: Import into a subdirectory](manifest.md#west-manifest-ex3-4) for an example.
- The west command line application can now also be run using `python3 -m
  ...
  interpreter without modifying the [`PATH`](../env_vars.md#envvar-PATH) environment variable.
- [west manifest –path](manifest.md#west-manifest-path) prints the absolute path to
  west.yml
- `west init` now supports an `--mf foo.yml` option, which initializes the
  workspace using `foo.yml` instead of `west.yml`.
- `west list` now prints the manifest repository’s path using the
  `manifest.path` [configuration option](config.md#west-config), which may differ
  ...
  option.
- Various Python API changes; see [West APIs](west-apis.md#west-apis) for details.
...
The main user-visible feature in west 0.7 is the [Manifest Imports](manifest.md#west-manifest-import)
...
- West manifests now support a [schema version](manifest.md#west-manifest-schema-version).
...
- There is a new [west topdir](built-in.md#west-built-in-misc) command, which
...
The developer-visible changes to the [West APIs](west-apis.md#west-apis) are:
...
- west.commands:
  ...
  - WestCommand.requires\_installation: deprecated; use requires\_workspace instead
  - WestCommand.requires\_workspace: new
  - WestCommand.has\_manifest: new
...
- west.manifest:
  ...
  - QUAL\_REFS\_WEST: new
  - SCHEMA\_VERSION: new
  - Defaults: removed
  - Manifest.as\_dict(): new
  - Manifest.as\_frozen\_yaml(): new
  - Manifest.as\_yaml(): new
  - Manifest.from\_file() and from\_data(): these factory methods are more
  ...
  - Project.name\_and\_path: new
  - Project.remote\_name: new
...
- The [west update](built-in.md#west-update) command has a new `--fetch`
  command line flag and `update.fetch` [configuration option](config.md#west-config). The default value, “smart”, skips fetching SHAs and tags
...
- The [west manifest](built-in.md#west-built-in-misc) command also handles errors
  better.
- The [west list](built-in.md#west-built-in-misc) command now works even when the
...
- west.log.banner(): new
- west.log.small\_banner(): new
- west.manifest.Manifest.get\_projects(): new
- west.manifest.Project.is\_cloned(): new
- west.commands.WestCommand instances can now access the parsed
  Manifest object via a new self.manifest property during the
  do\_run() call. If read, it returns the Manifest object or
  aborts the command if it could not be parsed.
- west.manifest.Project.git() now has a capture\_stderr kwarg
...
- No separate bootstrapper
  ...
  documentation](https://docs.zephyrproject.org/1.14.0/guides/west/repo-tool.html) for more details.
...
its v1.14 Long-Term Support (LTS) release. The [west v0.5.x documentation](https://docs.zephyrproject.org/1.14.0/guides/west/index.html) is
