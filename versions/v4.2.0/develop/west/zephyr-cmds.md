---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/develop/west/zephyr-cmds.html
original_path: develop/west/zephyr-cmds.html
---

# Additional Zephyr extension commands

This page documents miscellaneous [Zephyr Extensions](basics.md#west-zephyr-extensions).
...
See [share/zephyr-package/cmake](https://github.com/zephyrproject-rtos/zephyr/blob/main/share/zephyr-package/cmake) for details.
...
When building with [Sysbuild (System build)](../../build/sysbuild/index.md#sysbuild), make sure you target the actual application
...
- `modules-deps.spdx`: BOM for modules dependencies. Check
  [modules](../modules.md#modules-vulnerability-monitoring) for more details.
...
Copyright notices are extracted using the third-party **reuse** tool from the REUSE group.
...
- `-n PREFIX`: a prefix for the Document Namespaces that will be included in
  the generated SPDX documents. See [SPDX specification clause 6](https://spdx.github.io/spdx-spec/v2.2.2/document-creation-information/) for
...
- `-s SPDX_DIR`: specifies an alternate directory where the SPDX documents
  should be written instead of `BUILD_DIR/spdx/`.
...
- `--include-sdk`: with `--analyze-includes`, also create a fourth SPDX
  document, `sdk.spdx`, which lists header files included from the SDK.
...
The `blobs` command allows users to interact with [binary blobs](../../contribute/bin_blobs.md#bin-blobs) declared in one or more [modules](../modules.md#modules) via their
[module.yml](../modules.md#module-yml) file.
...
Note that, as described in [the modules section](../modules.md#modules-bin-blobs),
fetched blobs are stored in a `zephyr/blobs/` folder relative to the root
...
This command is a wrapper for [twister](../test/twister.md#twister-script).
...
The `bindesc` command allows users to read [binary descriptors](../../services/binary_descriptors/index.md#binary-descriptors)
...
by [GNU Global](https://www.gnu.org/software/global/) to use this command.
...
This will create a tags file named `GTAGS` in the workspace [topdir](basics.md#west-workspace) (it will also create other Global-related metadata files
...
use the [T2 star topology](workspaces.md#west-t2). The [patches.yml](#patches-yml) file stores
...
In this example, the [west manifest](manifest.md#west-manifests) file, `west.yml`, would pin to a
