---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/pinctrl/silabs,dbus-pinctrl.html
original_path: build/dts/api/bindings/pinctrl/silabs,dbus-pinctrl.html
---
...
[drivers/pinctrl/pinctrl\_silabs\_dbus.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/pinctrl/pinctrl_silabs_dbus.c).

## Description

```text
Silicon Labs Series 2 DBUS (Digital Bus) Pin Controller
...
These property descriptions apply to “silabs,dbus-pinctrl”
...
may apply to the “silabs,dbus-pinctrl” compatible.
...
| Name | Type | Details |
| --- | --- | --- |
| `pins` | `array` | ```text An array of pins sharing the same group properties. The pins should be defined using the <peripheral>_<signal>_<pin> macros available from the SoC DeviceTree files. ``` |
| `silabs,input-filter` | `boolean` | ```text Enable input glitch filter on this pin. May be used in input-enable and drive-open-drain modes. ``` |
| `silabs,alternate-port-control` | `boolean` | ```text Use Alternate Port Control settings for Slew Rate and Input Disable for this pin. May be used in drive-push-pull and drive-open-drain modes. ``` |
| `silabs,analog-bus` | `array` | ```text Assign one or more analog buses to the given GPIO port. The bus should be defined using the ABUS_<bus>_<peripheral> macros available from the SoC DeviceTree files. ``` |
