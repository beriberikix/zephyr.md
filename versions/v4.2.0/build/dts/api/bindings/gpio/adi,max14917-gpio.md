---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/gpio/adi,max14917-gpio.html
original_path: build/dts/api/bindings/gpio/adi,max14917-gpio.html
---

# adi,max14917-gpio (on spi bus)
...
[drivers/gpio/gpio\_max14917.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/gpio/gpio_max14917.c).

## Description

```text
ADI  MAX14917 is octal industrial output
```
...
These property descriptions apply to “adi,max14917-gpio”
...
| `vddok-gpios` | `phandle-array` | ```text High-Side Open-Drain Output. VDDOK is passive low when the internal logic supply is higher than the UVLO threshold, indicating that the registers have adequate supply voltage. ``` |
| `ready-gpios` | `phandle-array` | ```text Ready pin indicates when the device is ready to be used. ``` |
| `comerr-gpios` | `phandle-array` | ```text Communication Error pin indicates when there is a communication error on the SPI bus. ``` |
| `fault-gpios` | `phandle-array` | ```text Fault pin indicates cases of overload or undervoltage conditions ``` |
| `en-gpios` | `phandle-array` | ```text Enable Pin. Drive the EN pin high to enable the outputs. Drive EN low to disable/three-state all outputs. ``` |
| `sync-gpios` | `phandle-array` | ```text All eight output switches are updated simultaneously on the rising edge of SYNCH, as determined by the content of the SPI command ``` |
| `crcen-gpios` | `phandle-array` | ```text Drive CRCEN high to enable CRC generation and error detection on the serial data. ``` |
...
may apply to the “adi,max14917-gpio” compatible.
