---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/dsa/microchip,ksz8794.html
original_path: build/dts/api/bindings/dsa/microchip,ksz8794.html
---
...
[drivers/ethernet/dsa/dsa\_ksz8xxx.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/ethernet/dsa/dsa_ksz8xxx.c).
...
These property descriptions apply to “microchip,ksz8794”
...
| Name | Type | Details |
| --- | --- | --- |
| `workaround` | `int` | ```text Define the applied workaround for the switch used for short connections. Use bitmask to select the workaround or more 0x01: Short Cable Problems with the KSZ8795 Family 0x02: 1) CAT-5E/6 Short Cable with a Link Issue for the KSZ8795 Family 0x04: 2) CAT-5E/6 Short Cable with a Link Issue for the KSZ8795 Family ``` |
| `mii-lowspeed-drivestrength` | `int` | ```text Define the Low-Speed Interface Drive Strength for MII and RMMI Supported values 2,4,8,12,16,20,24,28mA see Register 163 (0xA3): Global Control 20 for more details ```  Legal values: `2`, `4`, `8`, `12`, `16`, `20`, `24`, `28` |
| `dsa-master-port` | `phandle` | ```text Phandle to master port. ``` |
| `dsa-slave-ports` | `int` | ```text Number of slave ports on the switch ``` |
| `reset-gpios` | `phandle-array` | ```text The pin is asserted for 10ms during boot to reset the KSZ8794. ``` |
...
may apply to the “microchip,ksz8794” compatible.
...
| Name | Type | Details |
| --- | --- | --- |
| `local-mac-address` | `uint8-array` | ```text Specifies the MAC address that was assigned to the port ``` |
