---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/clock/st,stm32mp13-pll-clock.html
original_path: build/dts/api/bindings/clock/st,stm32mp13-pll-clock.html
---
...
| `div-m` | `int` | ```text PLLx division factor (aka DIVM1 + 1) of the input clock divider Valid range: 1 - 64 ```  This property is **required**. |
| `mul-n` | `int` | ```text PLLx multiplication factor (aka DIVN + 1) for VCO Valid range: 25 - 200 ```  This property is **required**. |
| `div-p` | `int` | ```text PLLx_P division factor (aka DIVP + 1) Valid range: 1 - 128 ``` |
| `div-q` | `int` | ```text PLLx_Q division factor (aka DIVQ + 1) Valid range: 1 - 128 ``` |
| `div-r` | `int` | ```text PLLx_R division factor (aka DIVR + 1) Valid range: 1 - 128 ``` |
| `fracn` | `int` | ```text PLLx FRACV fractional latch Valid range: 0 - 8191 ``` |
...
may apply to the “st,stm32mp13-pll-clock” compatible.
