---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/sensor/pixart,pat9136.html
original_path: build/dts/api/bindings/sensor/pixart,pat9136.html
---

# pixart,pat9136 (on spi bus)
...
[drivers/sensor/pixart/pat9136/pat9136.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/sensor/pixart/pat9136/pat9136.c).

## Description

```text
PAT9136 optical flow sensor
```
...
| Name | Type | Details |
| --- | --- | --- |
| `resolution` | `int` | ```text The resolution of the sensor which indirectly results in counts per inch (CPI), following the formula CPI = (1 + resolution) * 100. Valid range is 0 to 199. Default is value set during Performance Optimization Settings. ```  Default value: `19` |
| `int-gpios` | `phandle-array` | ```text The INT signal default configuration is active-low. The property value should ensure the flags properly describe the signal that is presented to the driver. ``` |
| `backup-timer-ms` | `int` | ```text The back-up timer in milliseconds used to ensure sensor interaction even if no motion is present, when Data-Ready streaming trigger is configured. Default is 1000X the fastest frame rate (20,000 fps). ```  Default value: `50` |
| `cooldown-timer-ms` | `int` | ```text The cooldown timer in milliseconds used to prevent flooding the system with interrupts (potentially at max 20,000 fps) while Data-Ready streaming trigger is configured. Default is 200X the fastest frame rate (20,000 fps). Must be less than backup-timer-ms. ```  Default value: `10` |
...
may apply to the “pixart,pat9136” compatible.
