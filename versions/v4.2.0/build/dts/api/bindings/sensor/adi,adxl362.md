---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/sensor/adi,adxl362.html
original_path: build/dts/api/bindings/sensor/adi,adxl362.html
---

# adi,adxl362 (on spi bus)
...
[drivers/sensor/adi/adxl362/adxl362.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/sensor/adi/adxl362/adxl362.c).
...
| `wakeup-mode` | `boolean` | ```text This mode is intended for extremely low power consumption, checking for motion only about six times a second. ``` |
| `autosleep` | `boolean` | ```text Enter Wake-Up mode when inactivity is detected, reenter Measurement mode when activity is detected. Only applies for Linked and Loop mode, ignored otherwise. ``` |
| `fifo-mode` | `int` | ```text Accelerometer FIFO Mode. 0 # ADXL362_FIFO_MODE_DISABLED 1 # ADXL362_FIFO_MODE_OLDEST_SAVED 2 # ADXL362_FIFO_MODE_STREAM 3 # ADXL362_FIFO_MODE_TRIGGERED ```  Legal values: `0`, `1`, `2`, `3` |
| `fifo-watermark` | `int` | ```text Specify the FIFO watermark level in frame count. Valid range: 0 - 511 ``` |
...
may apply to the “adi,adxl362” compatible.
