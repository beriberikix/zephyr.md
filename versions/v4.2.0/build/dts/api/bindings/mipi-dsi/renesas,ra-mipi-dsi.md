---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/mipi-dsi/renesas,ra-mipi-dsi.html
original_path: build/dts/api/bindings/mipi-dsi/renesas,ra-mipi-dsi.html
---
...
[drivers/mipi\_dsi/dsi\_renesas\_ra.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/mipi_dsi/dsi_renesas_ra.c).
...
These property descriptions apply to “renesas,ra-mipi-dsi”
...
| Name | Type | Details |
| --- | --- | --- |
| `pll-div` | `int` | ```text PHY PLL divisor. ```  Legal values: `1`, `2`, `3`, `4` |
| `pll-mul-int` | `int` | ```text PHY PLL integer multiplier. ``` |
| `pll-mul-frac` | `string` | ```text PHY PLL fractional multiplier. ```  Legal values: `'0.00'`, `'0.33'`, `'0.66'`, `'0.50'` |
| `lp-divisor` | `int` | ```text PHY PLL LP speed divisor. ``` |
| `ulps-wakeup-period` | `int` | ```text ULPS wakeup period. ``` |
| `video-mode-delay` | `int` | ```text Set the delay value inside DSI Host until the transfer begins. ``` |
| `timing` | `array` | ```text MIPI DSI timing parameter: <CLKSTPT CLKBFHT CLKKPT GOLPBKT> ``` |
...
may apply to the “renesas,ra-mipi-dsi” compatible.
...
| Name | Type | Details |
| --- | --- | --- |
| `t-init` | `int` | ```text Minimum duration of the TINIT state (Units: operation clock cycles). ```  This property is **required**. |
| `t-clk-prep` | `int` | ```text Duration of the clock lane LP-00 state (immediately before entry to the HS-0 state). ```  This property is **required**. |
| `t-hs-prep` | `int` | ```text Duration of the data lane LP-00 state (immediately before entry to the HS-0 state). ```  This property is **required**. |
| `t-lp-exit` | `int` | ```text Low-power transition time to High-Speed mode. ```  This property is **required**. |
| `dphytim4` | `array` | ```text Clock lane pre and post data timing parameter: <TCLKZERO TCLKPRE TCLKPOST TCLKTRL> ```  This property is **required**. |
| `dphytim5` | `array` | ```text High-Speed data lane timing parameter: <THSZERO THSTRL THSEXIT> ```  This property is **required**. |
