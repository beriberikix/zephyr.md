---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/display/sinowealth,sh1122-mipi.html
original_path: build/dts/api/bindings/display/sinowealth,sh1122-mipi.html
---

# sinowealth,sh1122 (on mipi-dbi bus)

Vendor: [Sino Wealth Electronic Ltd](../../bindings.md#dt-vendor-sinowealth)

## Description

```text
SH1122 256x64 dot-matrix display controller on MIPI DBI bus
```
...
| Name | Type | Details |
| --- | --- | --- |
| `oscillator-freq` | `int` | ```text Front clock divider (3:0) / oscillator frequency (7:4). It can be set to 0x0. If you get weird scanlines, increase oscillator frequency and play with phase length values, for example set this to 0x90. Note this increases power consumption. ```  This property is **required**. |
...
| `multiplex-ratio` | `int` | ```text Multiplex ratio from 0MUX to 63MUX. Typically same value as height - 1. ```  This property is **required**. |
| `phase-length` | `int` | ```text Phase Length for segment charging (7:4) and discharging (3:0). ```  This property is **required**. |
| `vcomh-voltage` | `int` | ```text Set COM deselect voltage ```  This property is **required**. |
| `precharge-voltage` | `int` | ```text Set precharge voltage VSEGM. refer to datasheet for formula ```  This property is **required**. |
| `dc-dc` | `int` | ```text Internal DC-DC enable and set frequency. ```  This property is **required**. |
| `low-voltage` | `int` | ```text set discharge VSL level. 0x0 (0V) is the reset value ``` |
...
may apply to the “sinowealth,sh1122” compatible.
