---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/display/sitronix,st75256.html
original_path: build/dts/api/bindings/display/sitronix,st75256.html
---

# sitronix,st75256 (on mipi-dbi bus)
...
[drivers/display/display\_st75256.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/display/display_st75256.c).

## Description

```text
ST75256 Display Controller
```
...
| Name | Type | Details |
| --- | --- | --- |
| `booster-frequency` | `int` | ```text Sets frequency of booster, 0 or 1 (6 Khz) ```  This property is **required**. |
| `bias-ratio` | `int` | ```text Sets bias (0:1/14, 1:1/13, 2:1/12, 3:1/11, 4:1/10, 5:1/9) ```  This property is **required**. |
| `flip-configuration` | `int` | ```text 3 bits, MV[2], MX[1], MY[0] ```  This property is **required**. |
| `duty` | `int` | ```text 31, 62, 79, 127, 159, 161. Typically height-1 ```  This property is **required**. |
| `light-grey` | `int` | ```text 5-bit value describing light grey (0x01) pixel. Heavily affected by contrast. ```  This property is **required**. |
| `dark-grey` | `int` | ```text 5-bit value describing dark grey (0x10) pixel. Heavily affected by contrast. ```  This property is **required**. |
| `power-control` | `int` | ```text Power control register (x[0] = VR, x[1] = VF, x[3] = VB). Most displays have all turned on. ```  Default value: `11` |
| `fi-settings` | `int` | ```text Frame inversion settings. LF4, FI, LF3, LF2, LF1, LF0 (5:0). Most displays use 0 ``` |
| `lsb-invdir` | `boolean` | ```text Inverse vertically tiled Byte scan direction. Use when display has locally inverted Y patterns or when flipping display. ``` |
| `external-power` | `boolean` | ```text Use external power instead of internal power ``` |
...
may apply to the “sitronix,st75256” compatible.
