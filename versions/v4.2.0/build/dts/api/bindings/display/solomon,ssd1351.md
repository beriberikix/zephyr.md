---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/display/solomon,ssd1351.html
original_path: build/dts/api/bindings/display/solomon,ssd1351.html
---

# solomon,ssd1351 (on mipi-dbi bus)
...
| `multiplex-ratio` | `int` | ```text Multiplex ratio from 16MUX to 128MUX, ranges from 15 to 127. Typically same value as height - 1. ```  This property is **required**. |
| `phase-length` | `int` | ```text Phase 2 Period (7:4) and Phase 1 Period (3:0). ```  This property is **required**. |
| `oscillator-freq` | `int` | ```text Front clock divider (3:0) / oscillator frequency (7:4). If you get weird scanlines, increase oscillator frequency and play with phase length values, for example set this to 0xF1. Note this increases power consumption. ```  This property is **required**. |
| `precharge-time` | `int` | ```text Set 2nd precharge time for segments from 0 DCLK to 15 DCLK, 0 is invalid. A[3:0]. Typically 0x8. ```  This property is **required**. |
| `precharge-voltage` | `int` | ```text Set precharge voltage. A[4:0], from 0 (0.20 x VCC) to 0x1F (0.6 x VCC) ```  This property is **required**. |
| `vcomh-voltage` | `int` | ```text Set COM deselect voltage. A[2:0]. From 0.72xVCC to 0.86xVCC. ```  This property is **required**. |
| `current-att` | `int` | ```text Set Master current attenuation. A[3:0], from 1/16 to 16/16. ```  This property is **required**. |
| `remap-value` | `int` | ```text Remap register Has multiple configurations (see each bit setting in the datasheet) - A[0] HTiled or VTiled. Must be 0 for this driver. - A[1] Invert segment order - A[2] Invert ABC segment order - A[4] Invert COM order - A[5] Stripped odd/even display - A[7:6] Set color format. Must be 0x0 for this driver. ```  This property is **required**. |
...
may apply to the “solomon,ssd1351” compatible.
