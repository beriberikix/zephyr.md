---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/display/solomon,ssd1322.html
original_path: build/dts/api/bindings/display/solomon,ssd1322.html
---
...
[drivers/display/ssd1322.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/display/ssd1322.c).

## Description

```text
Solomon SSD1322 display controller
...
| Name | Type | Details |
| --- | --- | --- |
| `column-offset` | `int` | ```text First visible column number. ```  This property is **required**. |
| `row-offset` | `int` | ```text COM pin used as first row, mapped to the line set by start-line. The default corresponds to the reset value of the register. ``` |
| `start-line` | `int` | ```text Starting line address of display ram (0-127). The default corresponds to the reset value of the register. ``` |
| `mux-ratio` | `int` | ```text COM Pin Multiplex ratio from 16-128. The default corresponds to the reset value of the register. ```  Default value: `128` |
| `remap-row-first` | `boolean` | ```text Set scan direction to vertical first (swap rows/columns). ``` |
| `remap-columns` | `boolean` | ```text Reverse the column order (flip horizontally). ``` |
| `remap-rows` | `boolean` | ```text Reverse the row order (flip vertically). ``` |
| `remap-nibble` | `boolean` | ```text Reverse every group of 4 nibbles. ``` |
| `remap-com-odd-even-split` | `boolean` | ```text Odd even split of COM pins. ``` |
| `remap-com-dual` | `boolean` | ```text Dual COM mode. ``` |
| `segments-per-pixel` | `int` | ```text Map multiple adjacent segments to one pixel. Some displays (such as the Newhaven Display NHD-2.7-12864WDW3) connect two adjacent pixel drivers to each physical pixel. By setting this to 2, the driver will repeat each pixel. The default disables this functionality, as it only applies to very specific display models. ```  Default value: `1`  Legal values: `1`, `2` |
...
may apply to the “solomon,ssd1322” compatible.
