---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/i2s/st,stm32-sai.html
original_path: build/dts/api/bindings/i2s/st,stm32-sai.html
---
...
[drivers/i2s/i2s\_stm32\_sai.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/i2s/i2s_stm32_sai.c).
...
| `mclk-enable` | `boolean` | ```text Master Clock Output function. An mck pin must be listed within pinctrl-0 when enabling this property. ``` |
| `mclk-divider` | `string` | ```text Master Clock Divider Configuration.  When no-div property is present: - MCKDIV = SAI_CK_x / (FS * (FRL + 1)) Otherwise: - MCKDIV = SAI_CK_x / (FS * (OSR + 1) * 256)  When div-256 is present OSR is set to 0. When div-512 is present OSR is set to 1. ```  Default value: `no-div`  Legal values: `'no-div'`, `'div-256'`, `'div-512'` |
| `synchronous` | `boolean` | ```text Synchronous mode. When present, the SAI controller is configured to work in synchronous mode. ``` |
...
may apply to the “st,stm32-sai” compatible.
...
| `dmas` | `phandle-array` | ```text DMA channel specifiers relevant to the device. ```  This property is **required**. |
| `dma-names` | `string-array` | ```text DMA channel name: "tx" or "rx", depending of expected device behavior. ```  This property is **required**. |
