---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/i2s/nxp,mcux-i2s.html
...
| `clock-mux` | `int` | ```text Clock mux source for SAI root clock ``` |
| `mclk-output` | `boolean` | ```text Set if MCLK signal is an output.  Hardware default sets MCLK signal as input. ``` |
...
| `dma-names` | `string-array` | ```text Optional names given to the DMA channel specifiers in the "dmas" property. ```  This property is **required**. |
