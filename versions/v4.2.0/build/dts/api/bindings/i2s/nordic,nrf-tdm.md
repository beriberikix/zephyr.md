---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/i2s/nordic,nrf-tdm.html
original_path: build/dts/api/bindings/i2s/nordic,nrf-tdm.html
---
...
| Name | Type | Details |
| --- | --- | --- |
| `mck-frequency` | `int` | ```text Frequency of the MCK clock. Configured independently of SCK. ``` |
...
| `mck-clock-source` | `string` | ```text Clock source to be used by the MCK signal. The following options are available: - "PCLK": Peripheral clock - "ACLK": Audio PLL clock with configurable frequency (frequency for   this clock must be set via the "frequency" property   in the "nordic,nrfs-audiopll" node). ```  Default value: `PCLK`  Legal values: `'PCLK'`, `'ACLK'` |
| `sck-clock-source` | `string` | ```text Clock source to be used by the SCK signal. The following options are available: - "PCLK": Peripheral clock - "ACLK": Audio PLL clock with configurable frequency (frequency for   this clock must be set via the "frequency" property   in the "nordic,nrfs-audiopll" node). ```  Default value: `PCLK`  Legal values: `'PCLK'`, `'ACLK'` |
...
may apply to the “nordic,nrf-tdm” compatible.
