---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/flash_controller/st,stm32-ospi-nor.html
original_path: build/dts/api/bindings/flash_controller/st,stm32-ospi-nor.html
---
...
[drivers/flash/flash\_stm32\_ospi.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/flash/flash_stm32_ospi.c).
...
| `spi-bus-width` | `int` | ```text The width of (Octo)SPI bus to which flash memory is connected.  Possible values are :  - OSPI_SPI_MODE  <1> = SPI mode on 1 data line  - OSPI_DUAL_MODE <2> = Dual mode on 2 data lines  - OSPI_QUAD_MODE <4> = Quad mode on 4 data lines  - OSPI_OPI_MODE  <8> = Octo mode on 8 data lines ```  This property is **required**.  Legal values: `1`, `2`, `4`, `8` |
| `data-rate` | `int` | ```text The SPI data Rate is STR or DTR  Possible values are :  - OSPI_STR_TRANSFER <1> = Single Rate Transfer  - OSPI_DTR_TRANSFER <2> = Dual Rate Transfer (only with OSPI_OPI_MODE) ```  This property is **required**.  Legal values: `1`, `2` |
...
may apply to the “st,stm32-ospi-nor” compatible.
