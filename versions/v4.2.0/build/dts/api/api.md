---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/api.html
...
The [`DT_FOREACH_ANCESTOR`](../../../doxygen/html/group__devicetree-generic-foreach.md#gac4f9fee38bffbd5d315d386fe3c7bc91) macro allows iterating over the ancestor node
of a devicetree node.
Additionally, the [`DT_FOREACH_CHILD`](../../../doxygen/html/group__devicetree-generic-foreach.md#ga2f4eead8e8190110f5c0eb353e6a408f) macro allows iterating over the
...
| zephyr,camera | Video input device, typically a camera. |
...
| zephyr,ipc\_rsc\_table | Specifies a memory region that will be used for the OpenAMP resource table. Only needed if [`CONFIG_OPENAMP_COPY_RSC_TABLE`](../../../kconfig.md#CONFIG_OPENAMP_COPY_RSC_TABLE "CONFIG_OPENAMP_COPY_RSC_TABLE") is enabled. |
...
| mcuboot,ram-load-dev | When a Zephyr application is built to be loaded to RAM by MCUboot, with [`CONFIG_MCUBOOT_BOOTLOADER_MODE_SINGLE_APP_RAM_LOAD`](../../../kconfig.md#CONFIG_MCUBOOT_BOOTLOADER_MODE_SINGLE_APP_RAM_LOAD "CONFIG_MCUBOOT_BOOTLOADER_MODE_SINGLE_APP_RAM_LOAD"), this property is used to tell MCUboot the load address of the image, which will be the `reg` of the chosen node. |
| zephyr,boot-mode | Used for [Boot mode](../../../services/retention/index.md#boot-mode-api) selection, part of [Retention System](../../../services/retention/index.md#retention-api), which specifies what image on a device should be booted. |
