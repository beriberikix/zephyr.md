---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/sysbuild/index.html
original_path: build/sysbuild/index.html
---
...
Like Zephyr’s [build system](../index.md#build-overview), sysbuild is written in
CMake and uses [Kconfig](../kconfig/index.md#kconfig).
...
[![Sysbuild architectural overview](https://docs.zephyrproject.org/4.2.0/_images/sysbuild.svg)
](https://docs.zephyrproject.org/4.2.0/_images/sysbuild.svg)

The following are some key sysbuild features indicated in this figure:

- You can run sysbuild either with [west build](../../develop/west/build-flash-debug.md#west-building) or directly via `cmake`.
...
- Sysbuild itself is also configured using Kconfig. For example, you can
  ...
  Kconfig files.
- Sysbuild integrates with west’s [Building, Flashing and Debugging](../../develop/west/build-flash-debug.md#west-build-flash-debug) commands. It
  does this by managing the [Flash and debug runners](../../develop/west/build-flash-debug.md#west-runner), and specifically the
  `runners.yaml` files that each Zephyr build system will contain. These
  are packaged into a global view of how to flash and debug each build system
  in a `domains.yaml` file generated and managed by sysbuild.
...
Here is an example. For details, see [Sysbuild (multi-domain builds)](../../develop/west/build-flash-debug.md#west-multi-domain-builds) in
...
To set number of jobs for ninja for all sysbuild images, set the CMAKE\_BUILD\_PARALLEL\_LEVEL
...
The following example shows how to build [Hello World](../../samples/hello_world/README.md#hello_world "Print "Hello World" to the console.") with MCUboot enabled,
...
domain’s application name. See [Adding Zephyr applications to sysbuild](#sysbuild-zephyr-application) for more
information.
...
[CMake variable namespacing](#sysbuild-cmake-namespace).
...
You can use [west flash](../../develop/west/build-flash-debug.md#west-flashing) to flash applications with
sysbuild.
...
For more details, see [Multi-domain flashing](../../develop/west/build-flash-debug.md#west-multi-domain-flashing).
...
Just follow the existing [west debug](../../develop/west/build-flash-debug.md#west-debugging) guide to debug the main sample.
...
For more details, see [Multi-domain debugging](../../develop/west/build-flash-debug.md#west-multi-domain-debugging).
...
[Dedicated image build targets](#sysbuild-dedicated-image-build-targets) section.
...
When using [Trusted Firmware](../../services/tfm/build.md#tfm-build-system), the build targets prefixed
...
- A specific core on a physical board with a multi-core SoC, such as
  [nRF5340 DK](../../boards/nordic/nrf5340dk/doc/index.md#nrf5340dk).
- A specific SoC on a physical board with multiple SoCs, such as
  [nRF9160 DK](../../boards/nordic/nrf9160dk/doc/index.md#nrf9160dk-nrf9160) and [nRF9160 DK - nRF52840](../../boards/nordic/nrf9160dk/doc/index.md#nrf9160dk-nrf52840).
...
see [Kconfig namespacing](#sysbuild-kconfig-namespacing).
...
Sysbuild also allows users to change [Application Configuration Directory](../../develop/application/index.md#application-configuration-directory)
...
File suffix support through the **FILE\_SUFFIX** is supported in sysbuild
(see [File Suffixes](../../develop/application/index.md#application-file-suffixes) for details on this feature in applications). For sysbuild,
...
is required by an SoC, a \_runner\_, or something else.
...
standard CMake module [ExternalProject](https://cmake.org/cmake/help/latest/module/ExternalProject.html). Please refer to the CMake
...
instead these must be set by using a [a module](../../develop/modules.md#modules-build-settings) or by using a custom
...
[modules](../../develop/modules.md#module-yml) do, this will cause the files to be included in
each image that is part of a project. Alternatively, there are
[sysbuild-specific module extension](../../develop/modules.md#sysbuild-module-integration) files
