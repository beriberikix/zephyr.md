---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/services/logging/cs_stm.html
original_path: services/logging/cs_stm.html
---
...
STM infrastructure readiness is notified ([`log_frontend_stmesp_etr_ready()`](../../doxygen/html/log__frontend__stmesp_8h.md#afdfa76ad219bd073b54825ae50cf13d3)) then RAM buffer
...
- [`log_frontend_stmesp_tp()`](../../doxygen/html/log__frontend__stmesp_8h.md#ab2b829f38108c54490090dcd97719cec) - It accepts single argument - index. Index is between
  0 and 65280.
- [`log_frontend_stmesp_tp_d32()`](../../doxygen/html/log__frontend__stmesp_8h.md#a68b8bdb65457ffa2c0ab7c97b8414600) - It accepts two arguments - index and user data.
...
![../../_images/coresight_architecture.png](https://docs.zephyrproject.org/4.2.0/_images/coresight_architecture.png)
...
If standalone logging is used then proxy is using [ARM Coresight Trace Deformatter](../debugging/cs_trace_defmt.md#cs-trace-defmt) and
[MIPI STP Decoder](../debugging/mipi_stp_decoder.md#mipi-stp-decoder) to decode data and demultiplex messages. Messages are formatted to human
readable strings using logging [Message formatting](index.md#log-output).
...
to STM, and is based on the [Dictionary-based Logging](index.md#logging-guide-dictionary) feature of the logging API provided by
Zephyr.
...
[drivers/misc/coresight/nrf\_etr.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/misc/coresight/nrf_etr.c).

#### Configuration

For Nordic SoC a dedicated snippet ([Nordic Dictionary-based STM logging snippet (nordic-log-stm-dict)](../../snippets/nordic-log-stm-dict/README.md#nordic-log-stm-dict)) shall be used to enable logging.
...
2. Capture and decode the logs.
   ...
   The decoded logs will be saved in the specified output file (the `out.txt` file in the previous example).
...
in [subsys/logging/frontends/log\_frontend\_stmesp\_demux.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/subsys/logging/frontends/log_frontend_stmesp_demux.c).
...
located in [drivers/misc/coresight/nrf\_etr.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/misc/coresight/nrf_etr.c). It is using [ARM Coresight Trace Deformatter](../debugging/cs_trace_defmt.md#cs-trace-defmt) and
[MIPI STP Decoder](../debugging/mipi_stp_decoder.md#mipi-stp-decoder) and above-mentioned demultiplexer to decode messages.
...
For Nordic SoC a dedicated snippet ([Nordic Standalone STM logging snippet (nordic-log-stm)](../../snippets/nordic-log-stm/README.md#nordic-log-stm)) shall be used to enable logging.
Each core shall be build with that snippet. If any core wants to use it then application core is
required to have it enabled too since it acts as the proxy (ETR buffer processing). All cores must use the same configuration of the logging.
...
For more details, see [Introduction to devicetree](../../build/dts/intro.md#devicetree-intro).
...
- For memory constrained applications (for example, when running on the PPR core), disable the `printk()` function by setting both the [`CONFIG_PRINTK`](../../kconfig.md#CONFIG_PRINTK "CONFIG_PRINTK") and [`CONFIG_BOOT_BANNER`](../../kconfig.md#CONFIG_BOOT_BANNER "CONFIG_BOOT_BANNER") Kconfig options to `n` in your project configuration.
