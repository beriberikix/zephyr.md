---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/hardware/arch/arm-scmi.html
original_path: hardware/arch/arm-scmi.html
---
...
[DEN0056E](https://developer.arm.com/documentation/den0056/latest/). For more details
...
memory area is performed using a driver (`drivers/firmware/scmi/shmem.c`),
...
transport driver (`drivers/firmware/scmi/mailbox.c`).
...
> 1. **Power domain management**
> 2. **Clock management**
> 3. **Pin Control**

NXP-specific protocols:
:   1. **CPU domain management**
...
via a driver (`drivers/clock_control/clock_control_arm_scmi.c`), which
...
the `pinctrl_configure_pins` function. Each vendor should use
their own definition of `pinctrl_configure_pins`, which should
...
Finally, all that’s left to do is enable [`CONFIG_ARM_SCMI`](../../kconfig.md#CONFIG_ARM_SCMI "CONFIG_ARM_SCMI").
