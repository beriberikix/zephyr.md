---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/hardware/arch/arm_cortex_m.html
original_path: hardware/arch/arm_cortex_m.html
---
...
| **OS Features** |  |  | | | | | | | |  |
| Programmable fault IRQ priorities |  | Y | N | Y | Y | Y | N | Y | Y | Y |
...
| Native system timer (SysTick) |  | N [[1]](#f1) | Y | Y | Y | Y | Y | Y | Y | Y |
| **Memory protection** |  |  | | | | | | | |  |
|  | User mode | N | Y | Y | Y | Y | Y | Y | Y | Y |
|  | HW stack protection (MPU) | N | N | Y | Y | Y | Y | Y | Y | Y |
|  | HW-assisted stack limit checking | N | N | N | N | N | Y [[2]](#f2) | Y | Y | Y |
|  | Privileged Execute Never [[3]](#f3) | N | N | N | N | N | N | N | Y | Y |
| HW-assisted Control Flow integrity | PACBTI | N | N | N | N | N | N | N | N | Y |
| HW-assisted null-pointer dereference detection |  | N | N | Y | Y | Y | Y | Y | Y | Y |
...
[https://developer.arm.com/documentation/107655/100/RTOS-and-Secure-software-design-considerations/Secure-software-development-design-considerations/Security-and-privilege-combination/Using-PXN-bit?lang=en](https://developer.arm.com/documentation/107655/100/RTOS-and-Secure-software-design-considerations/Secure-software-development-design-considerations/Security-and-privilege-combination/Using-PXN-bit?lang=en)
...
[`CONFIG_STACK_ALIGN_DOUBLE_WORD`](../../kconfig.md#CONFIG_STACK_ALIGN_DOUBLE_WORD "CONFIG_STACK_ALIGN_DOUBLE_WORD"). If MPU-based HW-assisted stack overflow detection ([`CONFIG_MPU_STACK_GUARD`](../../kconfig.md#CONFIG_MPU_STACK_GUARD "CONFIG_MPU_STACK_GUARD"))
is enabled, thread stacks need to be aligned with a larger value, reflected by [`CONFIG_ARM_MPU_REGION_MIN_ALIGN_AND_SIZE`](../../kconfig.md#CONFIG_ARM_MPU_REGION_MIN_ALIGN_AND_SIZE "CONFIG_ARM_MPU_REGION_MIN_ALIGN_AND_SIZE").
...
in applications that need to support user mode ([`CONFIG_USERSPACE`](../../kconfig.md#CONFIG_USERSPACE "CONFIG_USERSPACE")). The thread stack sizes in that case need to be a power
of two. This is all reflected by [`CONFIG_MPU_REQUIRES_POWER_OF_TWO_ALIGNMENT`](../../kconfig.md#CONFIG_MPU_REQUIRES_POWER_OF_TWO_ALIGNMENT "CONFIG_MPU_REQUIRES_POWER_OF_TWO_ALIGNMENT"), that is enforced in Arm v6-M and Arm v7-M
...
stack size using [`CONFIG_ISR_STACK_SIZE`](../../kconfig.md#CONFIG_ISR_STACK_SIZE "CONFIG_ISR_STACK_SIZE").
...
- When switching-out the current thread, the processor stores
  ...
  >   > - the EXC\_RETURN value of the current handler context (PendSV)
...
- When switching-in a new thread the processor
  ...
  >   register (if applicable, see [`CONFIG_BUILTIN_STACK_GUARD`](../../kconfig.md#CONFIG_BUILTIN_STACK_GUARD "CONFIG_BUILTIN_STACK_GUARD"))
  > - optionally does a stack limit checking for the switched-in thread, if
  >   sentinel-based stack limit checking is enabled (see [`CONFIG_STACK_SENTINEL`](../../kconfig.md#CONFIG_STACK_SENTINEL "CONFIG_STACK_SENTINEL")).
...
core registers. The feature is enabled when [`CONFIG_BUILTIN_STACK_GUARD`](../../kconfig.md#CONFIG_BUILTIN_STACK_GUARD "CONFIG_BUILTIN_STACK_GUARD") is set.
...
([`CONFIG_CPU_CORTEX_M_HAS_PROGRAMMABLE_FAULT_PRIOS`](../../kconfig.md#CONFIG_CPU_CORTEX_M_HAS_PROGRAMMABLE_FAULT_PRIOS "CONFIG_CPU_CORTEX_M_HAS_PROGRAMMABLE_FAULT_PRIOS") signifies explicitly
...
[`CONFIG_ZERO_LATENCY_IRQS`](../../kconfig.md#CONFIG_ZERO_LATENCY_IRQS "CONFIG_ZERO_LATENCY_IRQS")), then
...
Cortex-M builds select [`CONFIG_CPU_CORTEX_M_HAS_BASEPRI`](../../kconfig.md#CONFIG_CPU_CORTEX_M_HAS_BASEPRI "CONFIG_CPU_CORTEX_M_HAS_BASEPRI") to signify that BASEPRI register is
implemented.). By modifying BASEPRI (or BASEPRI\_MAX) arch\_irq\_lock() masks all system and HW
...
[`CONFIG_DYNAMIC_DIRECT_INTERRUPTS`](../../kconfig.md#CONFIG_DYNAMIC_DIRECT_INTERRUPTS "CONFIG_DYNAMIC_DIRECT_INTERRUPTS").

Note that enabling direct dynamic interrupts requires enabling support for
dynamic interrupts in the kernel, as well (see [`CONFIG_DYNAMIC_INTERRUPTS`](../../kconfig.md#CONFIG_DYNAMIC_INTERRUPTS "CONFIG_DYNAMIC_INTERRUPTS")).
...
locking. To use the ZLI feature [`CONFIG_ZERO_LATENCY_IRQS`](../../kconfig.md#CONFIG_ZERO_LATENCY_IRQS "CONFIG_ZERO_LATENCY_IRQS") needs to be enabled.
...
The Cortex-M architecture port implements both k\_cpu\_idle()
and k\_cpu\_atomic\_idle(). The implementation is present in
`arch/arm/core/cortex_m/cpu_idle.c`.

In both implementations, the processor
will attempt to put the core to low power mode.
In k\_cpu\_idle() the processor ends up executing WFI (Wait For Interrupt)
instruction, while in k\_cpu\_atomic\_idle() the processor will
execute a WFE (Wait For Event) instruction.

When using the CPU idling API in Cortex-M it is important to note the
following:

- Both k\_cpu\_idle() and k\_cpu\_atomic\_idle() are *assumed* to be invoked
...
The Zephyr CPU Idling mechanism is detailed in [CPU Idling](../../kernel/services/scheduling/index.md#cpu-idle).
...
`CONFIG_ARCH_HAS_USERSPACE` is selected if [`CONFIG_ARM_MPU`](../../kconfig.md#CONFIG_ARM_MPU "CONFIG_ARM_MPU") is enabled
...
Cortex-M platforms with MPU may enable [`CONFIG_MPU_STACK_GUARD`](../../kconfig.md#CONFIG_MPU_STACK_GUARD "CONFIG_MPU_STACK_GUARD") to enable the MPU-based
...
- the mechanism is essential for detecting stack overflows in supervisor threads, or
  ...
  detected regardless of [`CONFIG_MPU_STACK_GUARD`](../../kconfig.md#CONFIG_MPU_STACK_GUARD "CONFIG_MPU_STACK_GUARD") being set.
...
- [`CONFIG_MPU_STACK_GUARD`](../../kconfig.md#CONFIG_MPU_STACK_GUARD "CONFIG_MPU_STACK_GUARD") will normally reserve one MPU region for programming
  the stack guard (in certain Arm v8-M configurations with [`CONFIG_MPU_GAP_FILLING`](../../kconfig.md#CONFIG_MPU_GAP_FILLING "CONFIG_MPU_GAP_FILLING")
...
- MPU guards are re-programmed at every context-switch, adding a small overhead to the
  thread swap routine. Compared, however, to the [`CONFIG_BUILTIN_STACK_GUARD`](../../kconfig.md#CONFIG_BUILTIN_STACK_GUARD "CONFIG_BUILTIN_STACK_GUARD") feature,
  no re-programming occurs during system calls.
- When [`CONFIG_HW_STACK_PROTECTION`](../../kconfig.md#CONFIG_HW_STACK_PROTECTION "CONFIG_HW_STACK_PROTECTION") is enabled on Arm v8-M platforms the native
  stack limit checking mechanism is used by default instead of the MPU-based stack overflow
  detection mechanism; users may override this setting by manually enabling [`CONFIG_MPU_STACK_GUARD`](../../kconfig.md#CONFIG_MPU_STACK_GUARD "CONFIG_MPU_STACK_GUARD")
  in these scenarios.
...
- [`CONFIG_ARMV8_1_M_PACBTI_STANDARD`](../../kconfig.md#CONFIG_ARMV8_1_M_PACBTI_STANDARD "CONFIG_ARMV8_1_M_PACBTI_STANDARD")
- [`CONFIG_ARMV8_1_M_PACBTI_PACRET`](../../kconfig.md#CONFIG_ARMV8_1_M_PACBTI_PACRET "CONFIG_ARMV8_1_M_PACBTI_PACRET")
- [`CONFIG_ARMV8_1_M_PACBTI_PACRET_LEAF`](../../kconfig.md#CONFIG_ARMV8_1_M_PACBTI_PACRET_LEAF "CONFIG_ARMV8_1_M_PACBTI_PACRET_LEAF")
- [`CONFIG_ARMV8_1_M_PACBTI_BTI`](../../kconfig.md#CONFIG_ARMV8_1_M_PACBTI_BTI "CONFIG_ARMV8_1_M_PACBTI_BTI")
- [`CONFIG_ARMV8_1_M_PACBTI_PACRET_BTI`](../../kconfig.md#CONFIG_ARMV8_1_M_PACBTI_PACRET_BTI "CONFIG_ARMV8_1_M_PACBTI_PACRET_BTI")
- [`CONFIG_ARMV8_1_M_PACBTI_PACRET_LEAF_BTI`](../../kconfig.md#CONFIG_ARMV8_1_M_PACBTI_PACRET_LEAF_BTI "CONFIG_ARMV8_1_M_PACBTI_PACRET_LEAF_BTI")
- [`CONFIG_ARMV8_1_M_PACBTI_NONE`](../../kconfig.md#CONFIG_ARMV8_1_M_PACBTI_NONE "CONFIG_ARMV8_1_M_PACBTI_NONE")
...
By default, when [`CONFIG_ARM_MPU`](../../kconfig.md#CONFIG_ARM_MPU "CONFIG_ARM_MPU") is enabled a set of *fixed* MPU regions
are programmed during system boot.

- One MPU region programs the entire flash area as read-execute.
  User can override this setting by enabling [`CONFIG_MPU_ALLOW_FLASH_WRITE`](../../kconfig.md#CONFIG_MPU_ALLOW_FLASH_WRITE "CONFIG_MPU_ALLOW_FLASH_WRITE"),
  which programs the flash with RWX permissions. If [`CONFIG_USERSPACE`](../../kconfig.md#CONFIG_USERSPACE "CONFIG_USERSPACE") is
...
- One MPU region programs the entire SRAM area with privileged-only
  ...
  SRAM. (An exception to this setting is when [`CONFIG_MPU_GAP_FILLING`](../../kconfig.md#CONFIG_MPU_GAP_FILLING "CONFIG_MPU_GAP_FILLING") is disabled (Arm v8-M only);
...
The above MPU regions are defined in `arch/arm/core/mpu/arm_mpu_regions.c`.
Alternative MPU configurations are allowed by enabling [`CONFIG_CPU_HAS_CUSTOM_FIXED_SOC_MPU_REGIONS`](../../kconfig.md#CONFIG_CPU_HAS_CUSTOM_FIXED_SOC_MPU_REGIONS "CONFIG_CPU_HAS_CUSTOM_FIXED_SOC_MPU_REGIONS").
...
size and attributes gathered directly from the devicetree. See [Caching Basics](../cache/guide.md#cache-guide)
and [Memory Attributes](../../services/mem_mgmt/index.md#mem-mgmt-api) for more details.

#### Static MPU regions

Additional *static* MPU regions may be programmed once during system boot. These regions
are required to enable certain features. See [Caching Basics](../cache/guide.md#cache-guide) for more details.

- a RX region to allow execution from SRAM, when `CONFIG_ARCH_HAS_RAMFUNC_SUPPORT` is
  enabled and users have defined functions to execute from SRAM.
- a RX region for relocating text sections to SRAM, when [`CONFIG_CODE_DATA_RELOCATION_SRAM`](../../kconfig.md#CONFIG_CODE_DATA_RELOCATION_SRAM "CONFIG_CODE_DATA_RELOCATION_SRAM") is enabled
- a `nocache` region to allow for a non-cacheable SRAM area, when [`CONFIG_NOCACHE_MEMORY`](../../kconfig.md#CONFIG_NOCACHE_MEMORY "CONFIG_NOCACHE_MEMORY") is enabled
- a possibly unprivileged RW region for GCOV code coverage accounting area, when [`CONFIG_COVERAGE_GCOV`](../../kconfig.md#CONFIG_COVERAGE_GCOV "CONFIG_COVERAGE_GCOV") is enabled
- a no-access region to implement null pointer dereference detection, when [`CONFIG_NULL_POINTER_EXCEPTION_DETECTION_MPU`](../../kconfig.md#CONFIG_NULL_POINTER_EXCEPTION_DETECTION_MPU "CONFIG_NULL_POINTER_EXCEPTION_DETECTION_MPU") is enabled
...
overlap. [`CONFIG_MPU_GAP_FILLING`](../../kconfig.md#CONFIG_MPU_GAP_FILLING "CONFIG_MPU_GAP_FILLING") controls whether the fixed MPU region
...
the total number of required MPU regions. When [`CONFIG_MPU_GAP_FILLING`](../../kconfig.md#CONFIG_MPU_GAP_FILLING "CONFIG_MPU_GAP_FILLING") is not
...
[`CONFIG_MPU_GAP_FILLING`](../../kconfig.md#CONFIG_MPU_GAP_FILLING "CONFIG_MPU_GAP_FILLING") is not set Zephyr is not protected against attacks
...
[Floating Point Services](../../kernel/services/other/float.md#float-v2) for more details).

When FPU support is enabled in the build
([`CONFIG_FPU`](../../kconfig.md#CONFIG_FPU "CONFIG_FPU") is enabled), the
sharing FP registers mode ([`CONFIG_FPU_SHARING`](../../kconfig.md#CONFIG_FPU_SHARING "CONFIG_FPU_SHARING"))
...
[`CONFIG_USE_DT_CODE_PARTITION`](../../kconfig.md#CONFIG_USE_DT_CODE_PARTITION "CONFIG_USE_DT_CODE_PARTITION") will ensure that a Zephyr chain-loadable image
...
[`CONFIG_INIT_ARCH_HW_AT_BOOT`](../../kconfig.md#CONFIG_INIT_ARCH_HW_AT_BOOT "CONFIG_INIT_ARCH_HW_AT_BOOT") Zephyr to force the initialization of the
...
In Cortex-M platforms that implement the VTOR register (see [`CONFIG_CPU_CORTEX_M_HAS_VTOR`](../../kconfig.md#CONFIG_CPU_CORTEX_M_HAS_VTOR "CONFIG_CPU_CORTEX_M_HAS_VTOR")),
...
When a bootloader image enables [`CONFIG_SW_VECTOR_RELAY`](../../kconfig.md#CONFIG_SW_VECTOR_RELAY "CONFIG_SW_VECTOR_RELAY")
...
the [`CONFIG_SW_VECTOR_RELAY_CLIENT`](../../kconfig.md#CONFIG_SW_VECTOR_RELAY_CLIENT "CONFIG_SW_VECTOR_RELAY_CLIENT") option to instruct the boot
...
[`CONFIG_CODE_DATA_RELOCATION_SRAM`](../../kconfig.md#CONFIG_CODE_DATA_RELOCATION_SRAM "CONFIG_CODE_DATA_RELOCATION_SRAM") is selected,
...
feature can be found in [Code And Data Relocation](../../kernel/code-relocation.md#code-data-relocation).

## Linking Cortex-M applications

Most Cortex-M platforms make use of the default Cortex-M
GCC linker script in `include/zephyr/arch/arm/cortex_m/scripts/linker.ld`,
...
- **CMSIS 5**: [zephyrproject-rtos/cmsis](https://github.com/zephyrproject-rtos/cmsis)
- **CMSIS 6**: [zephyrproject-rtos/CMSIS\_6](https://github.com/zephyrproject-rtos/CMSIS_6)
...
[`CONFIG_CPU_CORTEX_M`](../../kconfig.md#CONFIG_CPU_CORTEX_M "CONFIG_CPU_CORTEX_M") selects `CONFIG_HAS_CMSIS_CORE` to signify that
...
is present in `tests/arch/arm/`. The tests suites are continuously
...
| --- | --- | --- | --- | --- | --- |
| Architecture variant | Arm v6-M | Arm v7-M | | Arm v8-M | Arm v8.1-M |
|  | **qemu\_cortex\_m0** | **qemu\_cortex\_m3** | **mps2/an385** | **mps2/an521/cpu0** | **mps3/corstone300/an547** |
| **Emulated features** |  | | | | |
| NVIC | Y | Y | Y | Y | Y |
| BASEPRI | N | Y | Y | Y | Y |
| SysTick | N | Y | Y | Y | Y |
