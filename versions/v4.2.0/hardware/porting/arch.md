---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/hardware/porting/arch.html
...
- Use a different approach than calling into z\_swap() on Cortex-M to prevent
  race conditions.
...
- The parameters to the ISR are inserted and tracked via a separate table
  ...
  x86, `_isr_wrapper()` in ARM, or the full implementation description for
  ARC in [arch/arc/core/isr\_wrapper.S](https://github.com/zephyrproject-rtos/zephyr/blob/main/arch/arc/core/isr_wrapper.S).
...
preferred interface to use is `arch_switch` which is selected when
[`CONFIG_USE_SWITCH`](../../kconfig.md#CONFIG_USE_SWITCH "CONFIG_USE_SWITCH") is enabled. The alternative interface is
`arch_swap`–selected when [`CONFIG_USE_SWITCH`](../../kconfig.md#CONFIG_USE_SWITCH "CONFIG_USE_SWITCH")
is disabled. When porting to a new architecture, only one of these needs to
implemented; however, for SMP platforms it must be `arch_switch`.
...
internal kernel routine `z_swap` (or one of its variants). This in turn
will call either `arch_switch` or `arch_swap` as appropriate.
When these are called, no checks are done to determine if the context switch is
to happen–the context switch must happen.

Note

On x86 and Nios2, `arch_swap` is generic enough and the architecture
...
Since `z_swap` is cooperative, the caller-saved registers from the ABI are
...
- `z_arm_exc_exit` and `z_arm_int_exit` on ARM.
...
When [`CONFIG_USE_SWITCH`](../../kconfig.md#CONFIG_USE_SWITCH "CONFIG_USE_SWITCH") is enabled …

- The interrupt exit code shall call `z_get_next_switch_handle()`, and
  return to the thread context identified by the returned switch handle

When [`CONFIG_USE_SWITCH`](../../kconfig.md#CONFIG_USE_SWITCH "CONFIG_USE_SWITCH") is not enabled …
...
- [`ARCH_KERNEL_STACK_RESERVED`](../../doxygen/html/group__arch-threads.md#ga75b8e6cce01f5a34e9f3d649e561c3af): default no reserved space
...
[`ARCH_KERNEL_STACK_RESERVED`](../../doxygen/html/group__arch-threads.md#ga75b8e6cce01f5a34e9f3d649e561c3af) should be defined to the minimum size
