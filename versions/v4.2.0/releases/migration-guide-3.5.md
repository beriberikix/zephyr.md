---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/releases/migration-guide-3.5.html
original_path: releases/migration-guide-3.5.html
---
...
the [release notes](release-notes-3.5.md#zephyr-3-5).

## Required changes

### Kernel

- The kernel [`k_mem_slab_free()`](../doxygen/html/group__mem__slab__apis.md#ga2635ea8f9a30b8751ec966fe62adc0e1) function has changed its signature, now
  ...
  statement, that you can enable with [`CONFIG_ASSERT`](../kconfig.md#CONFIG_ASSERT "CONFIG_ASSERT"), will
  detect if you pass the function memory not belonging to the memory blocks in
  the slab.
- [`CONTAINER_OF`](../doxygen/html/group__sys-util.md#gac5bc561d1bfd1bf68877fe577779bd2f) now performs type checking, this was very commonly
  misused to obtain user structure from [`k_work`](../doxygen/html/structk__work.md) pointers without
  passing from [`k_work_delayable`](../doxygen/html/structk__work__delayable.md). This would now result in a build
  error and have to be done correctly using
  [`k_work_delayable_from_work()`](../doxygen/html/group__workqueue__apis.md#gabcb822a03ce7ea9ee1ed046afe31ffca).
...
- The default C library used on most targets has changed from the built-in
  ...
  - Picolibc enables thread local storage
    ([`CONFIG_THREAD_LOCAL_STORAGE`](../kconfig.md#CONFIG_THREAD_LOCAL_STORAGE "CONFIG_THREAD_LOCAL_STORAGE")) where supported. This
  ...
  - Picolibc uses the same malloc implementation as the minimal C library, but
    ...
    [`CONFIG_MMU`](../kconfig.md#CONFIG_MMU "CONFIG_MMU") or `ARCH_POSIX`, 2kB with
    [`CONFIG_USERSPACE`](../kconfig.md#CONFIG_USERSPACE "CONFIG_USERSPACE") and
    [`CONFIG_MPU_REQUIRES_POWER_OF_TWO_ALIGNMENT`](../kconfig.md#CONFIG_MPU_REQUIRES_POWER_OF_TWO_ALIGNMENT "CONFIG_MPU_REQUIRES_POWER_OF_TWO_ALIGNMENT"). For all
  ...
  - Picolibc’s default floating point input/output code is larger than the
    ...
    [`CONFIG_CBPRINTF_FP_SUPPORT`](../kconfig.md#CONFIG_CBPRINTF_FP_SUPPORT "CONFIG_CBPRINTF_FP_SUPPORT"), you will see increased
    memory usage unless you also disable
    [`CONFIG_PICOLIBC_IO_FLOAT_EXACT`](../kconfig.md#CONFIG_PICOLIBC_IO_FLOAT_EXACT "CONFIG_PICOLIBC_IO_FLOAT_EXACT"), which switches Picolibc
...
- Device dependencies (incorrectly referred as “device handles” in some areas)
  are now an optional feature enabled by [`CONFIG_DEVICE_DEPS`](../kconfig.md#CONFIG_DEVICE_DEPS "CONFIG_DEVICE_DEPS").
...
- The RAM disk driver has been changed to support multiple instances and instantiation
  using devicetree. As a result, Kconfig option `CONFIG_DISK_RAM_VOLUME_SIZE`
  and Kconfig option `CONFIG_DISK_RAM_VOLUME_NAME` are removed,
...
- The `zephyr,gpio-keys` binding has been merged into
  ...
  `INPUT_LISTENER_CB_DEFINE` to [`INPUT_CALLBACK_DEFINE`](../doxygen/html/group__input__interface.md#gac986cdf392f9aa0a771c8e4e92c479a3).
...
- STM32 Ethernet driver was misusing [`hwinfo_get_device_id()`](../doxygen/html/group__hwinfo__interface.md#ga197b58d995c77aae423527d0f8d9ff31) to generate
...
- On NXP boards with LPC DMA, the DMA controller node used to have its `dma-channels` property
  ...
  [`CONFIG_DMA_MCUX_LPC_NUMBER_OF_CHANNELS_ALLOCATED`](../kconfig.md#CONFIG_DMA_MCUX_LPC_NUMBER_OF_CHANNELS_ALLOCATED "CONFIG_DMA_MCUX_LPC_NUMBER_OF_CHANNELS_ALLOCATED").
- The LPC55XXX series SOC (except LPC55S06) default main clock has been
  ...
  kconfig option [`CONFIG_INIT_PLL1`](../kconfig.md#CONFIG_INIT_PLL1 "CONFIG_INIT_PLL1")
  is disabled then the main clock is muxed to FRO\_HR as before.
- The Kconfig option `CONFIG_GPIO_NCT38XX_INTERRUPT` has been renamed to
  [`CONFIG_GPIO_NCT38XX_ALERT`](../kconfig.md#CONFIG_GPIO_NCT38XX_ALERT "CONFIG_GPIO_NCT38XX_ALERT").
- The CAN controller timing API functions [`can_set_timing()`](../doxygen/html/group__can__interface.md#ga1b134af5d6286ea0fee130b6da5217da) and [`can_set_timing_data()`](../doxygen/html/group__can__interface.md#ga606cf9fda4c66a0f4abf74e1d357eac2)
  ...
  the `sjw` field in [`can_timing`](../doxygen/html/structcan__timing.md). To aid in this, the [`can_calc_timing()`](../doxygen/html/group__can__interface.md#gac27fe64142603f0d32d422594356b2d7) and
  [`can_calc_timing_data()`](../doxygen/html/group__can__interface.md#ga358cd73ed59c2099f4b2c6ceb397ca11) functions now automatically calculate a suitable SJW. The
  calculated SJW can be overwritten by the caller if needed. The CAN controller API functions
  [`can_set_bitrate()`](../doxygen/html/group__can__interface.md#ga0685ebfacfb79d33131167ac3aaa6f9b) and [`can_set_bitrate_data()`](../doxygen/html/group__can__interface.md#ga0afd2c446fc5e685370641a1678f87b7) now also automatically calculate a
  suitable SJW, but their SJW cannot be overwritten by the caller.
- The CAN ISO-TP message configuration in [`isotp_msg_id`](../doxygen/html/structisotp__msg__id.md) is changed to use the following
  flags instead of bit fields:

  - [`ISOTP_MSG_EXT_ADDR`](../doxygen/html/group__can__isotp.md#ga903d326b3bfb05151b9d5d0a0ace59b9) to enable ISO-TP extended addressing
  - [`ISOTP_MSG_FIXED_ADDR`](../doxygen/html/group__can__isotp.md#ga998ae9b1645485f567e692bfbaa50cbe) to enable ISO-TP fixed addressing
  - [`ISOTP_MSG_IDE`](../doxygen/html/group__can__isotp.md#ga16935466c1a543c03d11b71b8944d0b8) to use extended (29-bit) CAN IDs

  The two new flags [`ISOTP_MSG_FDF`](../doxygen/html/group__can__isotp.md#ga70b485a2e576b1b0fec2cc8037215bd9) and [`ISOTP_MSG_BRS`](../doxygen/html/group__can__isotp.md#ga9275708b5afba61c8b5c98c718ec4635) were added for CAN FD
  mode.
- NXP i.MX RT based boards should now enable
  [`CONFIG_DEVICE_CONFIGURATION_DATA`](../kconfig.md#CONFIG_DEVICE_CONFIGURATION_DATA "CONFIG_DEVICE_CONFIGURATION_DATA") at the board level when
  using a DCD with the RT bootrom, and enable
  [`CONFIG_NXP_IMX_EXTERNAL_SDRAM`](../kconfig.md#CONFIG_NXP_IMX_EXTERNAL_SDRAM "CONFIG_NXP_IMX_EXTERNAL_SDRAM") when using external SDRAM
  via the SEMC
...
- Platforms that implement power management hooks must explicitly select
  [`CONFIG_HAS_PM`](../kconfig.md#CONFIG_HAS_PM "CONFIG_HAS_PM") in Kconfig. This is now a dependency of
  [`CONFIG_PM`](../kconfig.md#CONFIG_PM "CONFIG_PM"). Before this change all platforms could enable
  [`CONFIG_PM`](../kconfig.md#CONFIG_PM "CONFIG_PM") because empty weak stubs were provided, however,
...
- Multiple platforms no longer support powering the system off using
  [`pm_state_force()`](../doxygen/html/group__subsys__pm__sys.md#ga075be307983f4efdcc93252a31a4258a). The new [`sys_poweroff()`](../doxygen/html/group__sys__poweroff.md#gaf4552a8ea40b4adb94f85b0a1ff06e39) API must be used.
  Migrated platforms include Nordic nRF, STM32, ESP32 and TI CC13XX/26XX. The
  new API is independent from [`CONFIG_PM`](../kconfig.md#CONFIG_PM "CONFIG_PM"). It requires
  [`CONFIG_POWEROFF`](../kconfig.md#CONFIG_POWEROFF "CONFIG_POWEROFF") to be enabled, which depends on
  [`CONFIG_HAS_POWEROFF`](../kconfig.md#CONFIG_HAS_POWEROFF "CONFIG_HAS_POWEROFF"), an option selected by platforms
  implementing the required new hooks.

### Bootloader

- The `CONFIG_BOOTLOADER_SRAM_SIZE` default value is now `0` (was
  `16`). Bootloaders that use a part of the SRAM should set this value to an
  appropriate size. [GitHub #60371](https://github.com/zephyrproject-rtos/zephyr/issues/60371)

### Bluetooth

- The `accept()` callback’s signature in [`bt_l2cap_server`](../doxygen/html/structbt__l2cap__server.md) has
  changed to `int (*accept)(struct bt_conn *conn, struct bt_l2cap_server
  *server, struct bt_l2cap_chan **chan)`,
  adding a new `server` parameter pointing to the [`bt_l2cap_server`](../doxygen/html/structbt__l2cap__server.md)
  structure instance the callback relates to. [GitHub #60536](https://github.com/zephyrproject-rtos/zephyr/issues/60536)

### Networking

- A new networking Kconfig option [`CONFIG_NET_INTERFACE_NAME`](../kconfig.md#CONFIG_NET_INTERFACE_NAME "CONFIG_NET_INTERFACE_NAME")
...
- ZBus runtime observers implementation now relies on the HEAP memory instead of a memory slab.
  ...
  - Replace the integer `CONFIG_ZBUS_RUNTIME_OBSERVERS_POOL_SIZE` with the boolean
    [`CONFIG_ZBUS_RUNTIME_OBSERVERS`](../kconfig.md#CONFIG_ZBUS_RUNTIME_OBSERVERS "CONFIG_ZBUS_RUNTIME_OBSERVERS");
  - Set the HEAP size with the [`CONFIG_HEAP_MEM_POOL_SIZE`](../kconfig.md#CONFIG_HEAP_MEM_POOL_SIZE "CONFIG_HEAP_MEM_POOL_SIZE").
- The zbus VDED delivery sequence has changed. Check the [documentation](../services/zbus/index.md#zbus-delivery-sequence) to verify if it will affect your code.
- MCUmgr SMP version 2 error codes entry has changed due to a collision with an
  existing response in shell\_mgmt. Previously, these errors had the entry `ret`
  but now have the entry `err`. `smp_add_cmd_ret()` is now deprecated and
  [`smp_add_cmd_err()`](../doxygen/html/mgmt_2mcumgr_2smp_2smp_8h.md#a191eb8c8a5a531b158374a1071925ca7) should be used instead, `MGMT_CB_ERROR_RET` is
  now deprecated and [`MGMT_CB_ERROR_ERR`](../doxygen/html/group__mcumgr__callback__api.md#gga1ba636c0efb7a91630ed9743e6fd4d6cad88f720a1d62658c346e8bebfb81d66d) should be used instead.
...
- MCUmgr SMP version 2 error translation (to legacy MCUmgr error code) is now
  ...
  pointer of [`mgmt_group`](../doxygen/html/structmgmt__group.md) when registering a group. See
  `smp_translate_error_fn` for function details. Any SMP version 2
  handlers made for Zephyr 3.4 need to be updated to include these translation
  functions when the groups are registered.

### ARM

- ARM SoC initialization routines no longer need to call NMI\_INIT(). The
  macro call has been removed as it was not doing anything useful.

### RISC V

- The `CONFIG_RISCV_MTVEC_VECTORED_MODE` Kconfig option was renamed to
  [`CONFIG_RISCV_VECTORED_MODE`](../kconfig.md#CONFIG_RISCV_VECTORED_MODE "CONFIG_RISCV_VECTORED_MODE").
...
- Nordic nRF based boards using `CONFIG_NFCT_PINS_AS_GPIOS`
  ...
  ```
- Nordic nRF based boards using [`CONFIG_GPIO_AS_PINRESET`](../kconfig.md#CONFIG_GPIO_AS_PINRESET "CONFIG_GPIO_AS_PINRESET")
  ...
  ```
- The `CONFIG_MODEM_GSM_PPP` modem driver is obsolete.
  Instead the new [`CONFIG_MODEM_CELLULAR`](../kconfig.md#CONFIG_MODEM_CELLULAR "CONFIG_MODEM_CELLULAR") driver should be used.
  As part of this `CONFIG_GSM_MUX` and `CONFIG_UART_MUX` are being
  marked as deprecated as well. The new modem subsystem [`CONFIG_MODEM_CMUX`](../kconfig.md#CONFIG_MODEM_CMUX "CONFIG_MODEM_CMUX")
  and [`CONFIG_MODEM_PPP`](../kconfig.md#CONFIG_MODEM_PPP "CONFIG_MODEM_PPP") should be used instead.
- Device drivers should now be restricted to `PRE_KERNEL_1`, `PRE_KERNEL_2`
  ...
  e.g. [`SYS_INIT`](../doxygen/html/group__sys__init.md#gaf507cc0613add8113c41896bd631254f).
