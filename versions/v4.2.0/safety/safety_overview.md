---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/safety/safety_overview.html
original_path: safety/safety_overview.html
---
...
1. Changes will be submitted from the interested party(ies) via pull requests to the Zephyr
   documentation repository.
2. The Zephyr Safety Committee will review these changes and provide feedback or acceptance of
   the changes.
3. Once accepted, these changes will become part of the document.

## General safety scope

The general scope of the Safety Committee is to achieve a certification for the [IEC 61508](https://en.wikipedia.org/wiki/IEC_61508) standard and the Safety Integrity Level (SIL) 3 /
...
1. **Hazard and Risk Analysis**: The IEC 61508 standard requires a thorough analysis of potential
...
2. **Safety Integrity Level (SIL)**: The standard introduces the concept of Safety Integrity Level
...
3. **System Design**: The IEC 61508 standard requires a systematic approach to system design that
   ...
   SIL.
4. **Verification and Validation**: The standard requires rigorous testing and evaluation of the
...
5. **Documentation and Traceability**: The IEC 61508 standard requires a comprehensive
...
![IEC 61508 relation to other standards](https://docs.zephyrproject.org/4.2.0/_images/IEC-61508-basis.svg)
...
1. Basic software quality standards
   ...
   1. [Coding Guidelines](../contribute/coding_guidelines/index.md#coding-guidelines) (including: static code analysis, coding style, etc.)
   2. [Safety Requirements](safety_requirements.md#safety-requirements) and requirements tracing
   3. Test coverage
2. Software architecture design principles

   1. Layered architecture model
   2. Encapsulated components
   3. Encapsulated single functionality (if not fitable and manageable in safety)
...
![Safety process and workflow overview](https://docs.zephyrproject.org/4.2.0/_images/zephyr-safety-process.svg)
...
1. On the main branch, the safety scope of the project should be identified, which typically
   ...
   ([Quality](#quality)) and safety processes within this scope. The Safety Architect works alongside the
...
2. At this point, the safety architect plays an increasingly important role. For PRs/issues that
   ...
   action.
3. This section describes the certification side. At this point, the code base has to be in an
...
4. This describes the necessary safety process required for certification itself. Here, the final
...
5. This transition from the auditable branch to the main branch should only occur in exceptional
