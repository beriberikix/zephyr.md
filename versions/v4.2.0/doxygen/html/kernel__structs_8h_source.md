---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/doxygen/html/kernel__structs_8h_source.html
...
136#if defined(CONFIG\_SCHED\_SIMPLE)
...
220#if defined(CONFIG\_THREAD\_MONITOR)

221 struct [k\_thread](structk__thread.md) \*threads; /\* singly linked list of ALL threads \*/

222#endif

223#ifdef CONFIG\_SCHED\_THREAD\_USAGE\_ALL

224 struct [k\_cycle\_stats](structk__cycle__stats.md) usage[CONFIG\_MP\_MAX\_NUM\_CPUS];

225#endif

226

227#ifdef CONFIG\_OBJ\_CORE\_SYSTEM

228 struct [k\_obj\_core](structk__obj__core.md) obj\_core;

229#endif

230

231#if defined(CONFIG\_SMP) && defined(CONFIG\_SCHED\_IPI\_SUPPORTED)

232 /\* Identify CPUs to send IPIs to at the next scheduling point \*/

233 [atomic\_t](atomic__types_8h.md#a124f07c3a788e53c3a40e4e1c06d8af8) pending\_ipi;
...
237typedef struct z\_kernel \_kernel\_t;

238

239extern struct z\_kernel \_kernel;

240

241extern [atomic\_t](atomic__types_8h.md#a124f07c3a788e53c3a40e4e1c06d8af8) \_cpus\_active;

242

243#ifdef CONFIG\_SMP
...
248bool z\_smp\_cpu\_mobile(void);

249#define \_current\_cpu ({ \_\_ASSERT\_NO\_MSG(!z\_smp\_cpu\_mobile()); \

250 arch\_curr\_cpu(); })

251

252\_\_attribute\_const\_\_ struct [k\_thread](structk__thread.md) \*z\_smp\_current\_get(void);

253#define \_current z\_smp\_current\_get()

254

255#else

256#define \_current\_cpu (&\_kernel.cpus[0])
...
[ 260](kernel__structs_8h.md#a80c653d62783a4e7ea960794bd287b49)#define CPU\_ID ((CONFIG\_MP\_MAX\_NUM\_CPUS == 1) ? 0 : \_current\_cpu->id)

261

262/\* This is always invoked from a context where preemption is disabled \*/

263#define z\_current\_thread\_set(thread) ({ \_current\_cpu->current = (thread); })

264

265#ifdef CONFIG\_ARCH\_HAS\_CUSTOM\_CURRENT\_IMPL

266#undef \_current

267#define \_current arch\_current\_thread()

268#undef z\_current\_thread\_set

269#define z\_current\_thread\_set(thread) \

270 arch\_current\_thread\_set(({ \_current\_cpu->current = (thread); }))
...
274#ifdef CONFIG\_WAITQ\_SCALABLE

275

276typedef struct {

277 struct \_priq\_rb waitq;

278} \_wait\_q\_t;

279

280/\* defined in kernel/priority\_queues.c \*/

281bool z\_priq\_rb\_lessthan(struct [rbnode](structrbnode.md) \*a, struct [rbnode](structrbnode.md) \*b);

282

283#define Z\_WAIT\_Q\_INIT(wait\_q) { { { .lessthan\_fn = z\_priq\_rb\_lessthan } } }
...
288 [sys\_dlist\_t](group__doubly-linked-list__apis.md#gaa03f9557215b486fee1039dd4c07e683) waitq;

289} \_wait\_q\_t;

290

291#define Z\_WAIT\_Q\_INIT(wait\_q) { SYS\_DLIST\_STATIC\_INIT(&(wait\_q)->waitq) }

292

293#endif /\* CONFIG\_WAITQ\_SCALABLE \*/
...
297typedef void (\*\_timeout\_func\_t)(struct \_timeout \*t);

298

299struct \_timeout {

300 [sys\_dnode\_t](group__doubly-linked-list__apis.md#ga57fdb936802a617d16c00ab08cd2ad98) node;

301 \_timeout\_func\_t fn;

302#ifdef CONFIG\_TIMEOUT\_64BIT

303 /\* Can't use k\_ticks\_t for header dependency reasons \*/

304 [int64\_t](stdint_8h.md#ac714c0d2c1a4adb10e73cab29623314b) dticks;

305#else

306 [int32\_t](stdint_8h.md#a0c18914b3041c2f583aba76f418399c2) dticks;
...
[ 310](kernel__structs_8h.md#a44c6f88a879877ad8da28706e274064f)typedef void (\*[k\_thread\_timeslice\_fn\_t](kernel__structs_8h.md#a44c6f88a879877ad8da28706e274064f))(struct [k\_thread](structk__thread.md) \*thread, void \*data);

311

312#ifdef \_\_cplusplus
...
318#endif /\* ZEPHYR\_KERNEL\_INCLUDE\_KERNEL\_STRUCTS\_H\_ \*/
