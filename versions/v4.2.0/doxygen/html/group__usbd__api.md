---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/doxygen/html/group__usbd__api.html
...
| Macros | |
| --- | --- |
| #define | [USBD\_SUPPORTS\_HIGH\_SPEED](#ga3f675bab13e089ab9576b29a6bb52364)   [IS\_EQ](group__sys-util.md#gaa35a6894bca38a8ffb558c2c22a1eee1)(CONFIG\_USBD\_MAX\_SPEED, 1) |
| #define | [USBD\_MAX\_BULK\_MPS](#gae5b1ba785fb89b129096c255b4084da1)   [COND\_CODE\_1](group__sys-util.md#ga358bc3e7669c860a98839a51cd526b20)([USBD\_SUPPORTS\_HIGH\_SPEED](#ga3f675bab13e089ab9576b29a6bb52364), (512), (64)) |
...
| #define | [USBD\_VREQUEST\_DEFINE](#ga0bdfa97d67ee62aa32af5fad2670a8a0)(name, vcode, vto\_host, vto\_dev) |
|  | Define a vendor request with recipient device. |
| #define | [USBD\_DESC\_BOS\_VREQ\_DEFINE](#ga0481d759856a0dc4b937335e8768eedd)(name, len, subset, vcode, vto\_host, vto\_dev) |
...
| int | [usbd\_register\_all\_classes](#ga06541b17f5afc917cc5154bd4d816ec3) (struct [usbd\_context](structusbd__context.md) \*uds\_ctx, const enum [usbd\_speed](#ga2f2da3d634530f08cd59d408c811ad71) speed, [uint8\_t](stdint_8h.md#a3cb4a16b0e8d6af0af86d4fd6ba5fd9d) cfg, const char \*const blocklist[]) |
...
| void | [usbd\_self\_powered](#ga9056e1f01b64ddd26db6a5ee45439e22) (struct [usbd\_context](structusbd__context.md) \*uds\_ctx, const [bool](stdbool_8h.md#abb452686968e48b67397da5f97445f5b) status) |
...
| int | [usbd\_device\_register\_vreq](#gaaebba63d8a8b78a1f06c1a5a476cd306) (struct [usbd\_context](structusbd__context.md) \*const uds\_ctx, struct [usbd\_vreq\_node](structusbd__vreq__node.md) \*const vreq\_nd) |
...
`#include <[zephyr/usb/usbd.h](usbd_8h.md)>`
...
IF\_ENABLED([USBD\_SUPPORTS\_HIGH\_SPEED](#ga3f675bab13e089ab9576b29a6bb52364), ( \
...
[USBD\_SUPPORTS\_HIGH\_SPEED](#ga3f675bab13e089ab9576b29a6bb52364)

#define USBD\_SUPPORTS\_HIGH\_SPEED
...
BUILD\_ASSERT([IS\_ENABLED](group__sys-util.md#ga111fe4e9d63758262fc6810a782cb32a)(CONFIG\_USBD\_BOS\_SUPPORT), \
...
Note
:   It requires Kconfig options USBD\_BOS\_SUPPORT to be enabled.
...
## [◆ ](#ga0481d759856a0dc4b937335e8768eedd)USBD\_DESC\_BOS\_VREQ\_DEFINE

| #define USBD\_DESC\_BOS\_VREQ\_DEFINE | ( |  | *name*, |
| --- | --- | --- | --- |
|  |  |  | *len*, |
|  |  |  | *subset*, |
|  |  |  | *vcode*, |
...
USBD\_VREQUEST\_DEFINE(vreq\_nd\_##name, vcode, vto\_host, vto\_dev); \

static struct [usbd\_desc\_node](structusbd__desc__node.md) name = { \

.bos = { \

.utype = USBD\_DUT\_BOS\_VREQ, \

.vreq\_nd = &vreq\_nd\_##name, \
...
This macro defines a BOS descriptor, usually a platform capability, with a vendor request node.

[USBD\_DESC\_BOS\_VREQ\_DEFINE(bos\_vreq\_webusb, sizeof(bos\_cap\_webusb), &bos\_cap\_webusb,
SAMPLE\_WEBUSB\_VENDOR\_CODE, webusb\_to\_host\_cb, NULL)](#ga0481d759856a0dc4b937335e8768eedd);

Note
:   It requires Kconfig options USBD\_VREQ\_SUPPORT and USBD\_BOS\_SUPPORT to be enabled.
...
:   | name | Descriptor node identifier |
    ...
    | len | Device Capability descriptor length |
    | subset | Pointer to a Device Capability descriptor |
    | vcode | Vendor request code |
    | vto\_host | Vendor callback for to-host direction request |
    | vto\_dev | Vendor callback for to-device direction request |
...
static const [uint16\_t](stdint_8h.md#a5debae8b2a1ec20a6694c0c443ee399e) langid\_##name = [sys\_cpu\_to\_le16](sys_2byteorder_8h.md#ae7f653c0bca81809b53d8a91854ca4c9)(0x0409); \
...
BUILD\_ASSERT([IS\_ENABLED](group__sys-util.md#ga111fe4e9d63758262fc6810a782cb32a)(CONFIG\_HWINFO), "HWINFO not enabled"); \
...
This macro defines a descriptor node that, when added to the device context, is automatically used as the serial number string descriptor. A valid serial number is obtained from [Hardware Info Interface](group__hwinfo__interface.md "Hardware Info Interface") whenever this string descriptor is requested.

Note
:   The HWINFO driver must be available and the Kconfig option HWINFO enabled.
...
static const [uint8\_t](stdint_8h.md#a3cb4a16b0e8d6af0af86d4fd6ba5fd9d) ascii\_##d\_name[sizeof(d\_string)] = d\_string; \
...
## [◆ ](#gae5b1ba785fb89b129096c255b4084da1)USBD\_MAX\_BULK\_MPS

| #define USBD\_MAX\_BULK\_MPS   [COND\_CODE\_1](group__sys-util.md#ga358bc3e7669c860a98839a51cd526b20)([USBD\_SUPPORTS\_HIGH\_SPEED](#ga3f675bab13e089ab9576b29a6bb52364), (512), (64)) |
| --- |
...
## [◆ ](#ga3f675bab13e089ab9576b29a6bb52364)USBD\_SUPPORTS\_HIGH\_SPEED

| #define USBD\_SUPPORTS\_HIGH\_SPEED   [IS\_EQ](group__sys-util.md#gaa35a6894bca38a8ffb558c2c22a1eee1)(CONFIG\_USBD\_MAX\_SPEED, 1) |
| --- |
...
## [◆ ](#ga0bdfa97d67ee62aa32af5fad2670a8a0)USBD\_VREQUEST\_DEFINE

| #define USBD\_VREQUEST\_DEFINE | ( |  | *name*, |
| --- | --- | --- | --- |
|  |  |  | *vcode*, |
|  |  |  | *vto\_host*, |
|  |  |  | *vto\_dev* ) |
...
BUILD\_ASSERT([IS\_ENABLED](group__sys-util.md#ga111fe4e9d63758262fc6810a782cb32a)(CONFIG\_USBD\_VREQ\_SUPPORT), \

"USB device vendor request support is disabled"); \

static struct [usbd\_vreq\_node](structusbd__vreq__node.md) name = { \
...
Note
:   It requires Kconfig options USBD\_VREQ\_SUPPORT to be enabled.

Parameters
:   | name | Vendor request identifier |
    | --- | --- |
    | vcode | Vendor request code |
    | vto\_host | Vendor callback for to-host direction request |
    | vto\_dev | Vendor callback for to-device direction request |
...
If the Kconfig option USBD\_MSG\_DEFERRED\_MODE is enabled, then the callback is executed in the context of the system workqueue. Notification messages are stored in a queue and delivered to the callback in sequence.

If the Kconfig option USBD\_MSG\_DEFERRED\_MODE is disabled, the callback is executed in the context of the USB device stack thread. The user should make sure that the callback execution does not block or disrupt device stack handling.
...
## [◆ ](#gaaebba63d8a8b78a1f06c1a5a476cd306)usbd\_device\_register\_vreq()

| int usbd\_device\_register\_vreq | ( | struct [usbd\_context](structusbd__context.md) \*const | *uds\_ctx*, |
| --- | --- | --- | --- |
|  |  | struct [usbd\_vreq\_node](structusbd__vreq__node.md) \*const | *vreq\_nd* ) |

`#include <[zephyr/usb/usbd.h](usbd_8h.md)>`

Register an USB vendor request with recipient device.

The vendor request with the recipient device applies to all configurations within the device.

Parameters
:   | [in] | uds\_ctx | Pointer to USB device support context |
    | --- | --- | --- |
    | [in] | vreq\_nd | Pointer to vendor request node |
...
## [◆ ](#ga06541b17f5afc917cc5154bd4d816ec3)usbd\_register\_all\_classes()
...
:   | [in] | uds\_ctx | Pointer to USB device support context |
    ...
    | [in] | blocklist | Null pointer terminated array of pointers to string literals to be used as a block list |
...
## [◆ ](#ga9056e1f01b64ddd26db6a5ee45439e22)usbd\_self\_powered()

| void usbd\_self\_powered | ( | struct [usbd\_context](structusbd__context.md) \* | *uds\_ctx*, |
| --- | --- | --- | --- |
|  |  | const [bool](stdbool_8h.md#abb452686968e48b67397da5f97445f5b) | *status* ) |

`#include <[zephyr/usb/usbd.h](usbd_8h.md)>`

Set the self-powered status of the USB device.

The status is used in the Self Powered field of the Get Status request response to indicate whether the device is currently self-powered.

Parameters
:   | [in] | uds\_ctx | Pointer to a device context |
    | --- | --- | --- |
    | [in] | status | Sets self-powered status if true, clears it otherwise |
