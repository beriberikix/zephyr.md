---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/doxygen/html/structbt__le__per__adv__sync__cb.html
...
This struct defines the callback functions that are invoked for various periodic advertising sync events. These include when the sync is successfully established, terminated, when data is received, state changes, BIG info reports, and IQ samples from the periodic advertising.

Note
:   Used in [bt\_le\_per\_adv\_sync\_cb\_register](group__bt__gap.md#ga4ee87bbf79e6ac844d14c3dafb2dadf4 "bt_le_per_adv_sync_cb_register") function.
...
:   | sync | The periodic advertising sync object. |
    | --- | --- |
    | info | Information about the termination event. |
