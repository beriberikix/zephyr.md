---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/doxygen/html/log__core_8h_source.html
...
265 TOOLCHAIN\_DISABLE\_GCC\_WARNING(TOOLCHAIN\_WARNING\_POINTER\_ARITH); \

266 string\_ok = Z\_CBPRINTF\_POINTERS\_VALIDATE(\_\_VA\_ARGS\_\_); \

267 TOOLCHAIN\_ENABLE\_GCC\_WARNING(TOOLCHAIN\_WARNING\_POINTER\_ARITH);

268#else

[ 269](log__core_8h.md#a6d29de8e144ab8e57f42f096b0b47ca0)#define LOG\_POINTERS\_VALIDATE(string\_ok, ...) string\_ok = true

[ 270](log__core_8h.md#ad9fb171d1afec420ff46f6e0e4b0481d)#define LOG\_STRING\_WARNING(\_mode, \_src, ...)
...
274/\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\* Macros for standard logging \*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*/

275/\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*/

291#define Z\_LOG2(\_level, \_inst, \_source, ...) \

292 do { \

293 if (!Z\_LOG\_LEVEL\_ALL\_CHECK(\_level, \_inst, \_source)) { \

294 break; \

295 } \

296 if (IS\_ENABLED(CONFIG\_LOG\_MODE\_MINIMAL)) { \

297 Z\_LOG\_TO\_PRINTK(\_level, \_\_VA\_ARGS\_\_); \
...
302 LOG\_POINTERS\_VALIDATE(string\_ok, \_\_VA\_ARGS\_\_); \

303 if (!string\_ok) { \

304 LOG\_STRING\_WARNING(\_mode, \_source, \_\_VA\_ARGS\_\_); \

305 break; \

306 } \

307 Z\_LOG\_MSG\_CREATE(UTIL\_NOT(IS\_ENABLED(CONFIG\_USERSPACE)), \_mode, \

308 Z\_LOG\_LOCAL\_DOMAIN\_ID, \_source, \_level, NULL, 0, \_\_VA\_ARGS\_\_); \

309 (void)\_mode; \

310 if (false) { \

311 /\* Arguments checker present but never evaluated.\*/ \

312 /\* Placed here to ensure that \_\_VA\_ARGS\_\_ are\*/ \

313 /\* evaluated once when log is enabled.\*/ \

314 z\_log\_printf\_arg\_checker(\_\_VA\_ARGS\_\_); \
...
318#define Z\_LOG(\_level, ...) Z\_LOG2(\_level, 0, Z\_LOG\_CURRENT\_DATA(), \_\_VA\_ARGS\_\_)

319#define Z\_LOG\_INSTANCE(\_level, \_inst, ...) Z\_LOG2(\_level, 1, Z\_LOG\_INST(\_inst), \_\_VA\_ARGS\_\_)

320

321/\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*/

322/\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\* Macros for hexdump logging \*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*/

323/\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*/

344#define Z\_LOG\_HEXDUMP2(\_level, \_inst, \_source, \_data, \_len, ...) \

345 do { \

346 if (!Z\_LOG\_LEVEL\_ALL\_CHECK(\_level, \_inst, \_source)) { \

347 break; \

348 } \

349 const char \*\_str = GET\_ARG\_N(1, \_\_VA\_ARGS\_\_); \

350 if (IS\_ENABLED(CONFIG\_LOG\_MODE\_MINIMAL)) { \

351 Z\_LOG\_TO\_PRINTK(\_level, "%s", \_str); \

352 z\_log\_minimal\_hexdump\_print((\_level), (const char \*)(\_data), (\_len)); \
...
356 Z\_LOG\_MSG\_CREATE(UTIL\_NOT(IS\_ENABLED(CONFIG\_USERSPACE)), \_mode, \

357 Z\_LOG\_LOCAL\_DOMAIN\_ID, \_source, \_level, \_data, \_len, \

358 COND\_CODE\_0(NUM\_VA\_ARGS\_LESS\_1(\_, ##\_\_VA\_ARGS\_\_), \

359 (), \

360 (COND\_CODE\_0(NUM\_VA\_ARGS\_LESS\_1(\_\_VA\_ARGS\_\_), \

361 ("%s", \_\_VA\_ARGS\_\_), (\_\_VA\_ARGS\_\_))))); \
...
405 uint32\_t others = \*(\_filters) & ~(LOG\_FILTER\_SLOT\_MASK << \

406 LOG\_FILTER\_SLOT\_SHIFT(\_id)); \

407 \*(\_filters) = others | (((\_filter) & LOG\_FILTER\_SLOT\_MASK) << \
...
531 z\_log\_msg\_runtime\_vcreate(Z\_LOG\_LOCAL\_DOMAIN\_ID, [NULL](iar__missing__defs_8h.md#a070d2ce7b6bb7e5c05602aa8c308d0c4), level,

532 [NULL](iar__missing__defs_8h.md#a070d2ce7b6bb7e5c05602aa8c308d0c4), 0, 0, fmt, ap);
