---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/doxygen/html/group__coap__service.html
...
| #define | [COAPS\_SERVICE\_DEFINE](#ga1ec49f2bc2c378431c4721080a13d11d)(\_name, \_host, \_port, \_flags, \_sec\_tag\_list, \_sec\_tag\_list\_size) |
|  | Define a CoAP secure service with static resources. |
...
`#include <[zephyr/net/coap_service.h](coap__service_8h.md)>`

**Value:**

[STRUCT\_SECTION\_ITERABLE\_ALTERNATE](group__iterable__section__apis.md#gae08ef16db3e04967fdca69a19ca4c70b)(\_CONCAT(coap\_resource\_, \_service), [coap\_resource](structcoap__resource.md), \

\_name) = \_\_VA\_ARGS\_\_
...
**Definition** coap\_service.h:128
...
[STRUCT\_SECTION\_FOREACH\_ALTERNATE](group__iterable__section__apis.md#ga06fb73cfb2dd5036a6e0f7098105ccd4)(\_CONCAT(coap\_resource\_, \_service), [coap\_resource](structcoap__resource.md), \_it)
...
extern struct [coap\_resource](structcoap__resource.md) \_CONCAT(\_CONCAT(\_coap\_resource\_, \_name), \_list\_start)[]; \

extern struct [coap\_resource](structcoap__resource.md) \_CONCAT(\_CONCAT(\_coap\_resource\_, \_name), \_list\_end)[]; \

\_\_z\_coap\_service\_define(\_name, \_host, \_port, \_flags, \

&\_CONCAT(\_CONCAT(\_coap\_resource\_, \_name), \_list\_start)[0], \

&\_CONCAT(\_CONCAT(\_coap\_resource\_, \_name), \_list\_end)[0], \

[NULL](iar__missing__defs_8h.md#a070d2ce7b6bb7e5c05602aa8c308d0c4), 0)
...
Note
:   The `_host` parameter can be [NULL](iar__missing__defs_8h.md#a070d2ce7b6bb7e5c05602aa8c308d0c4). If not, it is used to specify an IP address either in IPv4 or IPv6 format a fully-qualified hostname or a virtual host, otherwise the any address is used.
:   The `_port` parameter must be non-[NULL](iar__missing__defs_8h.md#a070d2ce7b6bb7e5c05602aa8c308d0c4). It points to a location that specifies the port number to use for the service. If the specified port number is zero, then an ephemeral port number will be used and the actual port number assigned will be written back to memory. For ephemeral port numbers, the memory pointed to by `_port` must be writeable.
...
## [◆ ](#ga1ec49f2bc2c378431c4721080a13d11d)COAPS\_SERVICE\_DEFINE

| #define COAPS\_SERVICE\_DEFINE | ( |  | *\_name*, |
| --- | --- | --- | --- |
|  |  |  | *\_host*, |
|  |  |  | *\_port*, |
|  |  |  | *\_flags*, |
...
BUILD\_ASSERT([IS\_ENABLED](group__sys-util.md#ga111fe4e9d63758262fc6810a782cb32a)(CONFIG\_NET\_SOCKETS\_ENABLE\_DTLS), \

"DTLS is required for CoAP secure (CONFIG\_NET\_SOCKETS\_ENABLE\_DTLS)"); \
...
:   The `_host` parameter can be [NULL](iar__missing__defs_8h.md#a070d2ce7b6bb7e5c05602aa8c308d0c4). If not, it is used to specify an IP address either in IPv4 or IPv6 format a fully-qualified hostname or a virtual host, otherwise the any address is used.
:   The `_port` parameter must be non-[NULL](iar__missing__defs_8h.md#a070d2ce7b6bb7e5c05602aa8c308d0c4). It points to a location that specifies the port number to use for the service. If the specified port number is zero, then an ephemeral port number will be used and the actual port number assigned will be written back to memory. For ephemeral port numbers, the memory pointed to by `_port` must be writeable.
:   `CONFIG_NET_SOCKETS_ENABLE_DTLS` has to be enabled for CoAP secure support.
...
Parameters
:   | \_sec\_tag\_list | DTLS security tag list used to setup a COAPS socket. |
    | --- | --- |
    | \_sec\_tag\_list\_size | DTLS security tag list size used to setup a COAPS socket. |
