---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/doxygen/html/group__secure__sockets__options.html
...
| Data Structures | |
| --- | --- |
| struct | [tls\_cert\_verify\_cb](structtls__cert__verify__cb.md) |
|  | Data structure for [TLS\_CERT\_VERIFY\_CALLBACK](#ga6fc0f394602ae713426568c6371c0120) socket option. [More...](structtls__cert__verify__cb.md#details) |
...
| #define | [TLS\_CERT\_VERIFY\_RESULT](#ga5100c3fe08cbf63e782318dec2bba6ee)   19 |
|  | Read-only socket option to obtain the result of the certificate verification from the most recent handshake if TLS\_PEER\_VERIFY\_OPTIONAL was set on the socket. |
| #define | [TLS\_CERT\_VERIFY\_CALLBACK](#ga6fc0f394602ae713426568c6371c0120)   20 |
|  | Write-only socket option to configure a certificate verification callback for the socket. |
...
`#include <[zephyr/net/socket.h](net_2socket_8h.md)>`
...
## [◆ ](#ga6fc0f394602ae713426568c6371c0120)TLS\_CERT\_VERIFY\_CALLBACK

| #define TLS\_CERT\_VERIFY\_CALLBACK   20 |
| --- |
...
The option accepts a pointer to a [tls\_cert\_verify\_cb](structtls__cert__verify__cb.md "tls_cert_verify_cb") structure, which contains pointers to the actual callback function and application-defined context.

If set, the certificate verification is delegated to the registered callback. In such case it's the application's responsibility to verify the provided certificates and decide whether to proceed or abort the handshake.

The option is only available if CONFIG\_NET\_SOCKETS\_TLS\_CERT\_VERIFY\_CALLBACK Kconfig option is enabled.

## [◆ ](#ga5100c3fe08cbf63e782318dec2bba6ee)TLS\_CERT\_VERIFY\_RESULT

| #define TLS\_CERT\_VERIFY\_RESULT   19 |
| --- |
...
The option accepts a pointer to a 32-bit unsigned integer, holding the verification result on return. A result of 0 indicates that verification was successful, otherwise the verification result is indicated by a set of flags. For mbed TLS backend, the flags are defined in "X509 Verify codes" section of x509.h header.
...
If set, DTLS [connect()](posix_2sys_2socket_8h.md#a90f0aa598d0f4ab4ea99ecf289a6a7fb) will execute the handshake with the configured peer. This is the default behavior. Otherwise, DTLS [connect()](posix_2sys_2socket_8h.md#a90f0aa598d0f4ab4ea99ecf289a6a7fb) will only configure peer address (as with regular UDP socket) and will not attempt to execute DTLS handshake. The handshake will take place in consecutive [send()](posix_2sys_2socket_8h.md#a16485de18b1ec93572e5d74b4a04e42f)/recv() call.
...
This option is irrelevant for TLS connections, as for them role is selected based on [connect()](posix_2sys_2socket_8h.md#a90f0aa598d0f4ab4ea99ecf289a6a7fb)/listen() usage. By default, DTLS will assume client role. This option accepts an integer with a TLS role, compatible with mbedTLS values:
