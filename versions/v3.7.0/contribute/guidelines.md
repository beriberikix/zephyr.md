---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/contribute/guidelines.html
original_path: contribute/guidelines.html
---
...
Zephyr uses the [Apache 2.0 license](https://github.com/zephyrproject-rtos/zephyr/blob/main/LICENSE) (as found in the LICENSE file in
the project’s [GitHub repo](https://github.com/zephyrproject-rtos/zephyr)) to strike a balance between open
...
this, check out articles such as [Why choose Apache 2.0 licensing](https://www.zephyrproject.org/faqs/#1571346989065-9216c551-f523) and
...
use other licensing, as described in [Licensing of Zephyr Project components](../LICENSING.md#zephyr-licensing).
...
[Developer Certification of Origin (DCO)](#dco) for contributed code, we can ensure that
the Zephyr community can develop products with the Zephyr Project
without concerns over patent or copyright issues.

See [Contributing External Components](external.md#external-contributions) for more information about
this contributing and review process for imported components.

## Copyrights Notices

Please follow this [Community Best Practice](https://www.linuxfoundation.org/blog/copyright-notices-in-open-source-software-projects/) for Copyright Notices from the
Linux Foundation.
...
below and at [http://developercertificate.org/](http://developercertificate.org/).
...
See [Git Setup](#git-setup) for instructions on configuring user and email settings
in Git.
...
[Modifying Contributions made by other developers](../project/modifying_contributions.md#modifying-contributions) describes additional recommended policies
...
configure, install, and use it as explained in the [Zephyr Project website](https://zephyrproject.org)
and how to set up your development environment as introduced in the Zephyr
[Getting Started Guide](../develop/getting_started/index.md#getting-started).

You should be familiar with common developer tools such as Git and CMake, and
platforms such as GitHub.

If you haven’t already done so, you’ll need to create a (free) GitHub account
on [https://github.com](https://github.com) and have Git tools available on your development system.
...
[Get Zephyr and install Python dependencies](../develop/getting_started/index.md#get-the-code).
...
`Kconfig`
:   The top-level Kconfig file, which refers to the file `Kconfig.zephyr`
    also found in the top-level directory.

    See [the Kconfig section of the manual](../build/kconfig/index.md#kconfig) for detailed Kconfig
    documentation.

`west.yml`
:   The [West (Zephyr’s meta-tool)](../develop/west/index.md#west) manifest, listing the external repositories managed by
...
:   Zephyr technical documentation source files and tools used to
    generate the [https://docs.zephyrproject.org](https://docs.zephyrproject.org) web content.

`drivers`
:   Device driver code.

`dts`
:   [devicetree](../build/dts/index.md#dt-guide) source files used to describe non-discoverable
...
Before starting on a patch, first check in our issues [Zephyr Project Issues](https://github.com/zephyrproject-rtos/zephyr/issues)
system to see what’s been reported on the issue you’d like to address. Have a
conversation on the [Zephyr devel mailing list](https://lists.zephyrproject.org/g/devel) (or the [Zephyr Discord
Server](https://chat.zephyrproject.org)) to see what others think of your issue (and proposed solution). You
...
mailing list](https://lists.zephyrproject.org/g/devel) to introduce and discuss your idea with the development
community.
...
You can find all [open pull requests](https://github.com/zephyrproject-rtos/zephyr/pulls) on GitHub and open [Zephyr Project
Issues](https://github.com/zephyrproject-rtos/zephyr/issues) in Github issues.
...
[changelog](https://github.com/zephyrproject-rtos/zephyr/commits/main).

#### Signed-off-by: …

Tip

You should have set your [Git Setup](#git-setup)
...
with the [Developer Certification of Origin (DCO)](#dco). For this reason, you must use your legal name.
...
See the [Contributor Expectations](contributor_expectations.md#contributor-expectations) for a more complete discussion of
contributor and reviewer expectations.

#### Adding links

Do not include [GitHub references](https://docs.github.com/en/get-started/writing-on-github/working-with-advanced-formatting/autolinked-references-and-urls) in the commit message directly, as it can
...
`Link:` tags. This includes, for example: GitHub issues, datasheets,
...
In general, follow the [Linux kernel coding style](https://kernel.org/doc/html/latest/process/coding-style.html), with the following
exceptions:
...
A more complete alternative to this is using [check\_compliance.py](#check-compliance-py) script.
...
[Coding Style](#id2) standards together with the `.clang-format` configuration file
...
When there are differences between the [Coding Style](#id2) guidelines and the
formatting generated by code formatting tools, the [Coding Style](#id2) guidelines
...
[CI Tests](#ci-tests) section. The CI results must be green indicating “All
...
and let the project CI automatically run the [Test Runner (Twister)](../develop/test/twister.md#twister-script) for you.
...
The results are available on the [Coverity Scan](https://scan.coverity.com/projects/zephyr) website. In order to access the
...
1. [Create a Fork of Zephyr](https://github.com/zephyrproject-rtos/zephyr#fork-destination-box)
...
2. On your development computer, change into the `zephyr` folder that was
   created when you [obtained the code](../develop/getting_started/index.md#get-the-code):

   ```text
   cd zephyrproject/zephyr
   ```

   Rename the default remote pointing to the [upstream repository](https://github.com/zephyrproject-rtos/zephyr) from `origin` to
   `upstream`:
   ...
   ```
3. Create a topic branch (off of `main`) for your work (if you’re addressing
   ...
   ```
4. Make changes, test locally, change, test, test again, … (Check out the
   prior chapter on [twister](#twister) as well).
5. When things look good, start the pull request process by adding your changed
   files:
...
7. Commit your changes to your local repo:
   ...
   agreement with the [Developer Certification of Origin (DCO)](#dco). See the [Commit Message Guidelines](#commit-guidelines) section for
   specific guidelines for writing your commit messages.
8. Push your topic branch with your changes to your fork in your personal
   GitHub account:

   ```text
   git push origin fix_comment_typo
   ```
9. In your web browser, go to your forked repo and click on the
   `Compare & pull request` button for the branch you just worked on and
   you want to open a pull request with.
10. Review the pull request changes, and verify that you are opening a pull
    request for the `main` branch. The title and message from your commit
    message should appear as well.
11. A bot will assign one or more suggested reviewers (based on the
    MAINTAINERS file in the repo). If you are a project member, you can
    select additional reviewers now too.
12. Click on the submit button and your pull request is sent and awaits
    review. Email will be sent as review comments are made, or you can check
    on your pull request at [https://github.com/zephyrproject-rtos/zephyr/pulls](https://github.com/zephyrproject-rtos/zephyr/pulls).

    Note

    As more commits are merged upstream, the GitHub PR page will show
    ...
    dropped.
13. While you’re waiting for your pull request to be accepted and merged, you
...
14. If reviewers do request changes to your patch, you can interactively rebase
...
15. After pushing the requested change, check on the PR page if there is a
    ...
    While amending commits and force pushing is a common review model
...
16. If the CI run fails, you will need to make changes to your code in order
...
1. When pushing follow-up changes, use the `--keep-base` option of
   [git-rebase](https://git-scm.com/docs/git-rebase#Documentation/git-rebase.txt---keep-base)
2. On the PR page, check if the change can still be merged with no merge
   conflicts
3. Make sure title of PR explains what is being fixed or added
4. Make sure your PR has a body with more details about the content of your
   submission
5. Make sure you reference the issue you are fixing in the body of the PR
6. Watch early CI results immediately after submissions and fix issues as they
   are discovered
7. Revisit PR after 1-2 hours to see the status of all CI checks, make sure all
   is green
8. If you get request for changes and submit a change to address them, make
...
In cases where the file is [imported from an external project](external.md#external-contributions), the commit message shall contain details regarding
...
Follow the guidelines in the [Modules (External projects)](../develop/modules.md#modules) section for contributing
[new modules](../develop/modules.md#submitting-new-modules) and
submitting changes to [existing modules](../develop/modules.md#changes-to-existing-module).
...
- The person proposing a treewide change must create an [RFC issue](https://github.com/zephyrproject-rtos/zephyr/issues/new?assignees=&labels=RFC&template=003_rfc-proposal.md&title=)
  describing the change, its rationale and impact, etc. before any pull
  requests related to the change can be merged
- The project’s [Architecture Working Group (WG)](https://github.com/zephyrproject-rtos/zephyr/wiki/Architecture-Working-Group)
...
- The person proposing a treewide change must email
  [devel@lists.zephyrproject.org](mailto:devel%40lists.zephyrproject.org) about the RFC if it is accepted by the
...
- the deprecation of version 1 of the [Logging API](../services/logging/index.md#logging-api) in favor
  of version 2 (see commit [262cc55609](https://github.com/zephyrproject-rtos/zephyr/commit/262cc55609b73ea61b5f999c6c6daaba20bc5240))
- the removal of support for a legacy [Devicetree bindings](../build/dts/bindings.md#dt-bindings) syntax
  ([6bf761fc0a](https://github.com/zephyrproject-rtos/zephyr/commit/6bf761fc0a2811b037abec0c963d60b00c452acb))
