---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/build/dts/api/api.html
...
*group* Node identifiers and helpers
:   Defines
    ...
    DT\_CHILD\_NUM\_STATUS\_OKAY(node\_id)
    ...
    DT\_NODELABEL\_STRING\_ARRAY(node\_id)
...
:   Defines
    ...
    DT\_REG\_HAS\_NAME(node\_id, name)
    :   Is `name` a valid register block name?

        If this returns 1, then [DT\_REG\_ADDR\_BY\_NAME(node\_id, name)](#group__devicetree-reg-prop_1gaeb5863e878bbd3a362e17616403da692) or [DT\_REG\_SIZE\_BY\_NAME(node\_id, name)](#group__devicetree-reg-prop_1ga042988feb27e58989baa7abb4930409e) are valid. If it returns 0, it is an error to use those macros with name `name`.
    ...
    DT\_REG\_ADDR\_BY\_NAME\_OR(node\_id, name, default\_value)
    :   Like [DT\_REG\_ADDR\_BY\_NAME()](#group__devicetree-reg-prop_1gaeb5863e878bbd3a362e17616403da692), but with a fallback to `default_value`.
    ...
    DT\_REG\_SIZE\_BY\_NAME\_OR(node\_id, name, default\_value)
    :   Like [DT\_REG\_SIZE\_BY\_NAME()](#group__devicetree-reg-prop_1ga042988feb27e58989baa7abb4930409e), but with a fallback to `default_value`.
...
:   Defines
    ...
    DT\_FOREACH\_NODELABEL(node\_id, fn)
    ...
    DT\_FOREACH\_NODELABEL\_VARGS(node\_id, fn, ...)
    :   Invokes `fn` for each node label of a given node with multiple arguments.

        This is like [DT\_FOREACH\_NODELABEL()](#group__devicetree-generic-foreach_1gad5585436896efc4c5154a93b5980d3b0) except you can also pass additional arguments to `fn`.
...
*group* Instance-based devicetree APIs
:   Defines
    ...
    :   Get the number of child nodes of a given node.
        ...
        `[DT_CHILD_NUM(DT_DRV_INST(inst))](#group__devicetree-generic-id_1ga37cf660c2a7a844f70191d21b6543dc1)`.
    ...
    DT\_INST\_CHILD\_NUM\_STATUS\_OKAY(inst)
    :   Get the number of child nodes of a given node.
        ...
        `[DT_CHILD_NUM_STATUS_OKAY(DT_DRV_INST(inst))](#group__devicetree-generic-id_1ga98544b8fd880fdd632f18e2410d39739)`.
    ...
    DT\_INST\_NODELABEL\_STRING\_ARRAY(inst)
    :   Get a string array of [DT\_DRV\_INST(inst)](#group__devicetree-inst_1ga219f413efba2f4c0151468b9a25a8dc1)’s node labels.

        Equivalent to [DT\_NODELABEL\_STRING\_ARRAY(DT\_DRV\_INST(inst))](#group__devicetree-generic-id_1ga0114cbfa3a2075558769d4632b7bb1e9).
    ...
    DT\_INST\_NUM\_NODELABELS(inst)
    :   Get the number of node labels by instance number.

        Equivalent to [DT\_NUM\_NODELABELS(DT\_DRV\_INST(inst))](#group__devicetree-interrupts-prop_1ga7b63eb05db40d7b95ccb62a9fd1f4404).
    ...
    DT\_INST\_REG\_HAS\_NAME(inst, name)
    ...
    DT\_INST\_REG\_ADDR\_BY\_NAME\_OR(inst, name, default\_value)
    :   Like [DT\_INST\_REG\_ADDR\_BY\_NAME()](#group__devicetree-inst_1ga722d6f7b97136aa9229242e4ba7dd25c), but with a fallback to `default_value`.
    ...
    DT\_INST\_REG\_SIZE\_BY\_NAME\_OR(inst, name, default\_value)
    :   Like [DT\_INST\_REG\_SIZE\_BY\_NAME()](#group__devicetree-inst_1gaf82457c5dcfef7eeba074afb95d48714), but with a fallback to `default_value`.
    ...
    DT\_INST\_FOREACH\_NODELABEL(inst, fn)
    :   Call `fn` on all node labels for a given `DT_DRV_COMPAT` instance.

        Equivalent to [DT\_FOREACH\_NODELABEL(DT\_DRV\_INST(inst), fn)](#group__devicetree-generic-foreach_1gad5585436896efc4c5154a93b5980d3b0).

        Parameters:
        :   - **inst** – instance number
            - **fn** – macro which will be passed each node label for the node with that instance number

    DT\_INST\_FOREACH\_NODELABEL\_VARGS(inst, fn, ...)
    :   Call `fn` on all node labels for a given `DT_DRV_COMPAT` instance with multiple arguments.

        Equivalent to [DT\_FOREACH\_NODELABEL\_VARGS(DT\_DRV\_INST(inst), fn, …)](#group__devicetree-generic-foreach_1ga2a88abdb46158bcf36a8c976a1e2186d).
    ...
    DT\_INST\_NODE\_HAS\_COMPAT(inst, compat)
...
*group* Devicetree CAN API
:   Defines

    DT\_CAN\_TRANSCEIVER\_MIN\_BITRATE(node\_id, min)
    ...
    DT\_INST\_CAN\_TRANSCEIVER\_MIN\_BITRATE(inst, min)
    :   Get the minimum transceiver bitrate for a DT\_DRV\_COMPAT CAN controller.

        See also

        [DT\_CAN\_TRANSCEIVER\_MIN\_BITRATE()](#group__devicetree-can_1ga60cb3dc5c2feb517dddcb5a57cce8a9b)
...
*group* Devicetree Fixed Partition API
:   Defines
...
*group* Devicetree IO Channels API
:   Defines
...
*group* Devicetree Reset Controller API
:   Defines
