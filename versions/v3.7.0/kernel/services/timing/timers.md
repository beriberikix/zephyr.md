---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/kernel/services/timing/timers.html
...
- A **duration** specifying the time interval before the timer
  expires for the first time. This is a [`k_timeout_t`](clocks.md#c.k_timeout_t "k_timeout_t") value that
  may be initialized via different units.
- A **period** specifying the time interval between all timer
  expirations after the first one, also a [`k_timeout_t`](clocks.md#c.k_timeout_t "k_timeout_t"). It must be
