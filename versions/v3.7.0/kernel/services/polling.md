---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/kernel/services/polling.html
...
**mode** *must* be set to [`K_POLL_MODE_NOTIFY_ONLY`](#c.k_poll_modes.K_POLL_MODE_NOTIFY_ONLY). The state
*must* be set to [`K_POLL_STATE_NOT_READY`](#c.K_POLL_STATE_NOT_READY) (the initializers take care
...
[`K_POLL_TYPE_IGNORE`](#c.K_POLL_TYPE_IGNORE).
...
reset to [`K_POLL_STATE_NOT_READY`](#c.K_POLL_STATE_NOT_READY) *and* its `result` must be
reset using [`k_poll_signal_reset()`](#c.k_poll_signal_reset) on each iteration if it has
been signaled.
