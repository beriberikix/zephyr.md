---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/kernel/services/scheduling/index.html
original_path: kernel/services/scheduling/index.html
---
...
- transition of a thread from running state to a suspended or waiting
  state, for example by [`k_sem_take()`](../synchronization/semaphores.md#c.k_sem_take "k_sem_take") or [`k_sleep()`](../threads/index.md#c.k_sleep "k_sleep").
- transition of a thread to the [ready state](../threads/index.md#thread-states), for
  example by [`k_sem_give()`](../synchronization/semaphores.md#c.k_sem_give "k_sem_give") or [`k_thread_start()`](../threads/index.md#c.k_thread_start "k_thread_start")
- return to thread context after processing an interrupt
- when a running thread invokes [`k_yield()`](../threads/index.md#c.k_yield "k_yield")
...
([`CONFIG_SCHED_DEADLINE`](../../../kconfig.md#CONFIG_SCHED_DEADLINE "CONFIG_SCHED_DEADLINE")) and a choice of threads have equal
...
[`k_thread_deadline_set()`](../threads/index.md#c.k_thread_deadline_set "k_thread_deadline_set") is used to set a thread’s deadline.
...
- Simple linked-list ready queue ([`CONFIG_SCHED_DUMB`](../../../kconfig.md#CONFIG_SCHED_DUMB "CONFIG_SCHED_DUMB"))
...
- Red/black tree ready queue ([`CONFIG_SCHED_SCALABLE`](../../../kconfig.md#CONFIG_SCHED_SCALABLE "CONFIG_SCHED_SCALABLE"))
...
- Traditional multi-queue ready queue ([`CONFIG_SCHED_MULTIQ`](../../../kconfig.md#CONFIG_SCHED_MULTIQ "CONFIG_SCHED_MULTIQ"))
...
The wait\_q abstraction used in IPC primitives to pend threads for later wakeup
shares the same backend data structure choices as the scheduler, and can use
the same options.

- Scalable wait\_q implementation ([`CONFIG_WAITQ_SCALABLE`](../../../kconfig.md#CONFIG_WAITQ_SCALABLE "CONFIG_WAITQ_SCALABLE"))

  When selected, the wait\_q will be implemented with a balanced tree. Choose
  this if you expect to have many threads waiting on individual primitives.
  There is a ~2kb code size increase over [`CONFIG_WAITQ_DUMB`](../../../kconfig.md#CONFIG_WAITQ_DUMB "CONFIG_WAITQ_DUMB") (which may
  be shared with [`CONFIG_SCHED_SCALABLE`](../../../kconfig.md#CONFIG_SCHED_SCALABLE "CONFIG_SCHED_SCALABLE")) if the red/black tree is not
  ...
  path).
- Simple linked-list wait\_q ([`CONFIG_WAITQ_DUMB`](../../../kconfig.md#CONFIG_WAITQ_DUMB "CONFIG_WAITQ_DUMB"))

  When selected, the wait\_q will be implemented with a doubly-linked list.
...
> ![../../../_images/cooperative.svg](https://docs.zephyrproject.org/3.7.0/_images/cooperative.svg)
...
- Calling [`k_yield()`](../threads/index.md#c.k_yield "k_yield") puts the thread at the back of the scheduler’s
...
- Calling [`k_sleep()`](../threads/index.md#c.k_sleep "k_sleep") makes the thread unready for a specified
...
> ![../../../_images/preemptive.svg](https://docs.zephyrproject.org/3.7.0/_images/preemptive.svg)
...
![../../../_images/timeslicing.svg](https://docs.zephyrproject.org/3.7.0/_images/timeslicing.svg)
...
thread is preemptible and, if so, implicitly invokes [`k_yield()`](../threads/index.md#c.k_yield "k_yield")
...
as a cooperative thread by calling [`k_sched_lock()`](../threads/index.md#c.k_sched_lock "k_sched_lock"). This prevents
...
[`k_sched_unlock()`](../threads/index.md#c.k_sched_unlock "k_sched_unlock") to restore its normal, preemptible status.

If a thread calls [`k_sched_lock()`](../threads/index.md#c.k_sched_lock "k_sched_lock") and subsequently performs an
...
A thread can call [`k_sleep()`](../threads/index.md#c.k_sleep "k_sleep") to delay its processing
...
[`k_wakeup()`](../threads/index.md#c.k_wakeup "k_wakeup"). This technique can sometimes be used
...
A thread can call [`k_busy_wait()`](../threads/index.md#c.k_busy_wait "k_busy_wait") to perform a `busy wait`
...
### [Making the CPU idle](#id7)

Making the CPU idle is simple: call the k\_cpu\_idle() API. The CPU will stop
...
k\_cpu\_idle() unconditionally unmasks interrupts.
...
### [Making the CPU idle in an atomic fashion](#id8)

It is possible that there is a need to do some work atomically before making
the CPU idle. In such a case, k\_cpu\_atomic\_idle() should be used instead.
...
system completely. To prevent this, k\_cpu\_atomic\_idle() should have been used,
...
Use k\_cpu\_atomic\_idle() when a thread has to do some real work in addition to
idling the CPU to wait for an event. See example above.

Use k\_cpu\_idle() only when a thread is only responsible for idling the CPU,
...
*group* CPU Idling APIs
:   Functions

    static inline void k\_cpu\_idle(void)
    :   Make the CPU idle.
    ...
    static inline void k\_cpu\_atomic\_idle(unsigned int key)
    :   Make the CPU idle in an atomic fashion.

        Similar to [k\_cpu\_idle()](#group__cpu__idle__apis_1ga7b25e1bed511a813b32fbd0f91b09356), but must be called with interrupts locked.
        ...
        After waking up from the low-power mode, the interrupt lockout state will be restored as if by [irq\_unlock(key)](../interrupts.md#group__isr__apis_1ga646045943b3b2a130738bcc48867bf57).

        Parameters:
        :   - **key** – Interrupt locking key obtained from [irq\_lock()](../interrupts.md#group__isr__apis_1ga19fdde73c3b02fcca6cf1d1e67631228).
