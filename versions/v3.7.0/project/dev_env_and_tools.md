---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/project/dev_env_and_tools.html
original_path: project/dev_env_and_tools.html
---
...
Pull requests should be appropriately [labeled](#gh-labels),
and linked to any relevant [bug or feature tracking issues](issues.md#bug-reporting)
.
...
on a component basis, as defined in the [MAINTAINERS.yml](https://github.com/zephyrproject-rtos/zephyr/blob/main/MAINTAINERS.yml) file
...
![Pull request classes](https://docs.zephyrproject.org/3.7.0/_images/pull_request_classes.png)
...
Group. For example for [breaking API changes](../develop/api/api_lifecycle.md#breaking-api-changes), the
...
| Label | Description |
| --- | --- |
| priority: {high|medium|low} | To classify the impact and importance of a bug or [feature](proposals.md#feature-tracking).  Note: Issue priorities are generally set or changed during the bug-triage or TSC meetings. |
| Regression | Something, which was working, but does not anymore (bug subtype). |
| Enhancement | Changes/Updates/Additions to existing [features](proposals.md#feature-tracking). |
| Feature request | A request for a new [feature](proposals.md#feature-tracking). |
| Feature | A [planned feature](proposals.md#feature-tracking) with a milestone. |
| Hardware Support | Covers porting an existing feature (including Zephyr itself) to new hardware. |
...
feature requests are handled and become features can be found in [Feature
Tracking](proposals.md#feature-tracking).
...
| Label | Description |
| --- | --- |
| Hotfix | Fix for an issue blocking development. |
| Trivial | Simple changes that can have shorter review time and be reviewed by anyone, i.e. typos, straightforward one-liner bug fixes, etc. |
| Maintainer | Maintainer review required. |
| Security Review | To be reviewed by a security expert. |
...
review. See [review process](#review-time) for details.

### Labels applicable to both pull requests and issues

| Label | Description |
| --- | --- |
| area: {area-name} | Indicates Zephyr subsystems (e.g, area: Kernel, area: I2C, area: Memory Management), project functions (e.g., area: Debugging, area: Documentation, area: Process), or other categories (e.g., area: Coding Style, area: MISRA-C) affected by the bug or the pull request.  An area maintainer should be able to filter by an area label and find all issues and PRs which relate to that area. |
| platform: {platform-name} | An issue or PR which affects only a particular platform. |
| dev-review | The issue is to be discussed in the following [dev-review](https://github.com/zephyrproject-rtos/zephyr/wiki/Zephyr-Committee-and-Working-Group-Meetings#zephyr-dev-meeting) if time permits. |
| TSC | TSC stands for Technical Steering Committee. The issue is to be discussed in the following [TSC meeting](https://github.com/zephyrproject-rtos/zephyr/wiki/Zephyr-Committee-and-Working-Group-Meetings#technical-steering-committee-tsc) if time permits. |
| Breaking API Change | The issue or PR describes a breaking change to a stable API. See additional information in [Introducing breaking API changes](../develop/api/api_lifecycle.md#breaking-api-changes). |
...
| Stale | An issue or a PR which seems abandoned, and requires attention by the author. |
...
| EXT | Related to an external component. |
