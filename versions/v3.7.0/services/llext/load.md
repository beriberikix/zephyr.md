---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/services/llext/load.html
...
An extension may be loaded using any implementation of a [`llext_loader`](api.md#c.llext_loader "llext_loader")
...
needed by the [`llext_load()`](api.md#c.llext_load "llext_load") function. An implementation over a buffer
...
The extensions are loaded with a call to the [`llext_load()`](api.md#c.llext_load "llext_load") function,
...
[`llext_add_domain()`](api.md#c.llext_add_domain "llext_add_domain") function must be called.

## Accessing code and data

To interact with the newly loaded extension, the host application must use the
[`llext_find_sym()`](api.md#c.llext_find_sym "llext_find_sym") function to get the address of the exported symbol.
...
[`llext_call_fn()`](api.md#c.llext_call_fn "llext_call_fn").

## Cleaning up after use

The [`llext_unload()`](api.md#c.llext_unload "llext_unload") function must be called to free the memory used by
...
- Results from [`llext_find_sym()`](api.md#c.llext_find_sym "llext_find_sym") point to an invalid address;
...
- Make sure `CONFIG_LLEXT_LOG_LEVEL` is set to `DEBUG`, then
  obtain a log of the [`llext_load()`](api.md#c.llext_load "llext_load") invocation.
