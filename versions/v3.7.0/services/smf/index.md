---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/services/smf/index.html
...
defined structure that has the state machine context, [`smf_ctx`](#c.smf_ctx), as
...
The [`smf_ctx`](#c.smf_ctx) member must be first because the state machine
framework’s functions casts the user defined object to the [`smf_ctx`](#c.smf_ctx)
type with the [`SMF_CTX`](#c.SMF_CTX) macro.
...
- [`SMF_CREATE_STATE`](#c.SMF_CREATE_STATE) Create a state
...
To set the initial state, the [`smf_set_initial()`](#c.smf_set_initial) function should be
called.

To transition from one state to another, the [`smf_set_state()`](#c.smf_set_state)
function is used.

Note

If [`CONFIG_SMF_INITIAL_TRANSITION`](../../kconfig.md#CONFIG_SMF_INITIAL_TRANSITION "CONFIG_SMF_INITIAL_TRANSITION") is not set,
[`smf_set_initial()`](#c.smf_set_initial) and [`smf_set_state()`](#c.smf_set_state) function should
...
While the state machine is running, [`smf_set_state()`](#c.smf_set_state) should
only be called from the Entry or Run function. Calling
[`smf_set_state()`](#c.smf_set_state) from Exit functions will generate a warning in the
log and no transition will occur.

## State Machine Execution

To run the state machine, the [`smf_run_state()`](#c.smf_run_state) function should be
...
Calling [`smf_set_handled()`](#c.smf_set_handled) prevents calling the run action of parent
states. It is not required to call [`smf_set_handled()`](#c.smf_set_handled) if the state
calls [`smf_set_state()`](#c.smf_set_state).

## State Machine Termination

To terminate the state machine, the [`smf_set_terminate()`](#c.smf_set_terminate) function
...
[`smf_run_state()`](#c.smf_run_state) function.
...
3. Prohibiting transitions using [`smf_set_state()`](#c.smf_set_state) in exit actions.

SMF also does not provide any pseudostates except the Initial Pseudostate.
Terminate pseudostates can be modelled by calling [`smf_set_terminate()`](#c.smf_set_terminate)
from the entry action of a ‘terminate’ state. Orthogonal regions are modelled
by calling [`smf_run_state()`](#c.smf_run_state) for each region.
...
:   - Ancestor entry actions are executed before the sibling entry actions. For
    ...
    - The parent\_run function only executes if the child\_run function does not
      call either [`smf_set_state()`](#c.smf_set_state) or [`smf_set_handled()`](#c.smf_set_handled).
...
[Hierarchical State Machine Demo based on example from PSiCC2](../../samples/subsys/smf/hsm_psicc2/README.md#smf_hsm_psicc2 "Implement an event-driven hierarchical state machine using State Machine Framework (SMF).")
:   Implement an event-driven hierarchical state machine using State Machine Framework (SMF).

*group* State Machine Framework API
:   State Machine Framework API.
    ...
    **Version**
    ...
    SMF\_CREATE\_STATE(\_entry, \_run, \_exit, \_parent, \_initial)
    ...
    void smf\_set\_initial(struct [smf\_ctx](#c.smf_ctx) \*ctx, const struct [smf\_state](#c.smf_state) \*init\_state)
    ...
    void smf\_set\_state(struct [smf\_ctx](#c.smf_ctx) \*ctx, const struct [smf\_state](#c.smf_state) \*new\_state)
    ...
    void smf\_set\_terminate(struct [smf\_ctx](#c.smf_ctx) \*ctx, int32\_t val)
    ...
    void smf\_set\_handled(struct [smf\_ctx](#c.smf_ctx) \*ctx)
    ...
    int32\_t smf\_run\_state(struct [smf\_ctx](#c.smf_ctx) \*ctx)
    ...
    :   *#include <smf.h>*
        ...
        const [state\_execution](#c.state_execution) entry
        :   Optional method that will be run when this state is entered.

        const [state\_execution](#c.state_execution) run
        :   Optional method that will be run repeatedly during state machine loop.

        const [state\_execution](#c.state_execution) exit
    ...
    :   *#include <smf.h>*
        ...
        const struct [smf\_state](#c.smf_state) \*current
        :   Current state the state machine is executing.

        const struct [smf\_state](#c.smf_state) \*previous
