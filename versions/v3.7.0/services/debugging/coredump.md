---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/services/debugging/coredump.html
original_path: services/debugging/coredump.html
---
...
Additional memory can be included in a dump (even with the “DEBUG\_COREDUMP\_MEMORY\_DUMP\_MIN”
config selected) through one or more [coredump devices](../../hardware/peripherals/coredump.md#coredump-device-api)
...
2. Convert the core dump log into a binary format that can be parsed by
   ...
   [scripts/coredump/coredump\_serial\_log\_parser.py](https://github.com/zephyrproject-rtos/zephyr/blob/main/scripts/coredump/coredump_serial_log_parser.py) can be used
   to convert the serial console log into a binary file.
3. Start the custom GDB server using the script
   [scripts/coredump/coredump\_gdbserver.py](https://github.com/zephyrproject-rtos/zephyr/blob/main/scripts/coredump/coredump_gdbserver.py) with the core dump
...
This was done on [X86 Emulation (QEMU)](../../boards/qemu/x86/doc/index.md#qemu-x86) where a null pointer was dereferenced.
...
| Pointer size | ‘uint8\_t’ | Size of `uintptr_t` in power of 2. (e.g. 5 for 32-bit, 6 for 64-bit). This is needed to accommodate 32-bit and 64-bit target in parsing the memory block addresses. |
| Flags | `uint8_t` |  |
| Fatal error reason | `unsigned int` | Reason for the fatal error, as the same in `enum k_fatal_error_reason` defined in [include/zephyr/fatal.h](https://github.com/zephyrproject-rtos/zephyr/blob/main/include/zephyr/fatal.h) |
...
1. Add a new target code to the `enum coredump_tgt_code` in
   [include/zephyr/debug/coredump.h](https://github.com/zephyrproject-rtos/zephyr/blob/main/include/zephyr/debug/coredump.h).
2. Implement [`arch_coredump_tgt_code_get()`](#c.arch_coredump_tgt_code_get) simply to return
   the newly introduced target code.
3. Implement [`arch_coredump_info_dump()`](#c.arch_coredump_info_dump) to construct
   a target architecture block and call [`coredump_buffer_output()`](#c.coredump_buffer_output)
   to output the block to core dump backend.
4. Add a parser to the core dump GDB stub scripts under
   `scripts/coredump/gdbstubs/`

   1. Extends the `gdbstubs.gdbstub.GdbStub` class.
   2. During `__init__`, store the GDB signal corresponding to
      the exception reason in `self.gdb_signal`.
   3. Parse the architecture-specific block from
      `self.logfile.get_arch_data()`. This needs to match the format
      as implemented in step 3 (inside [`arch_coredump_info_dump()`](#c.arch_coredump_info_dump)).
   4. Implement the abstract method `handle_register_group_read_packet`
      where it returns the register group as GDB expected. Refer to
      GDB’s code and documentation on what it is expecting for
      the new target.
   5. Optionally implement `handle_register_single_read_packet`
      for registers not covered in the `g` packet.
5. Extend `get_gdbstub()` in
   [scripts/coredump/gdbstubs/\_\_init\_\_.py](https://github.com/zephyrproject-rtos/zephyr/blob/main/scripts/coredump/gdbstubs/__init__.py) to return
...
:   Coredump APIs.
    ...
    enum coredump\_query\_id
    :   Query ID.

        *Values:*

        enumerator COREDUMP\_QUERY\_GET\_ERROR
        :   Returns error code from backend.

        enumerator COREDUMP\_QUERY\_HAS\_STORED\_DUMP
        ...
        enumerator COREDUMP\_QUERY\_GET\_STORED\_DUMP\_SIZE
        :   Returns:
        ...
        enumerator COREDUMP\_QUERY\_MAX
        :   Max value for query ID.

    enum coredump\_cmd\_id
    :   Command ID.

        *Values:*

        enumerator COREDUMP\_CMD\_CLEAR\_ERROR
        :   Clear error code from backend.

            Returns 0 if successful, failed otherwise.

        enumerator COREDUMP\_CMD\_VERIFY\_STORED\_DUMP
        ...
        enumerator COREDUMP\_CMD\_ERASE\_STORED\_DUMP
        ...
        enumerator COREDUMP\_CMD\_COPY\_STORED\_DUMP
        ...
        enumerator COREDUMP\_CMD\_INVALIDATE\_STORED\_DUMP
        ...
        enumerator COREDUMP\_CMD\_MAX
        :   Max value for command ID.

    Functions

    static inline void coredump(unsigned int reason, const struct arch\_esf \*esf, struct [k\_thread](../../kernel/services/threads/index.md#c.k_thread "k_thread") \*thread)
    :   Perform coredump.
    ...
    static inline void coredump\_memory\_dump(uintptr\_t start\_addr, uintptr\_t end\_addr)
    :   Dump memory region.
    ...
    static inline void coredump\_buffer\_output(uint8\_t \*buf, size\_t buflen)
    ...
    static inline int coredump\_query(enum [coredump\_query\_id](#c.coredump_query_id) query\_id, void \*arg)
    :   Perform query on coredump subsystem.

        Query the coredump subsystem for information, for example, if there is an error.

        Parameters:
        :   - **query\_id** – **[in]** Query ID
            - **arg** – **[inout]** Pointer to argument for exchanging information

        Returns:
        :   Depends on the query

    static inline int coredump\_cmd(enum [coredump\_cmd\_id](#c.coredump_cmd_id) query\_id, void \*arg)
    :   Perform command on coredump subsystem.
        ...
        Parameters:
        :   - **cmd\_id** – **[in]** Command ID
            - **arg** – **[inout]** Pointer to argument for exchanging information

        Returns:
        :   Depends on the command

    struct coredump\_cmd\_copy\_arg
    :   *#include <coredump.h>*

        Coredump copy command ([COREDUMP\_CMD\_COPY\_STORED\_DUMP](#group__coredump__apis_1gga48765a88d430aaec373a2ce6e4796578a0cc9387829efa1f3c6f802217adc3819)) argument definition.
...
*group* Architecture-specific core dump APIs
:   Functions
