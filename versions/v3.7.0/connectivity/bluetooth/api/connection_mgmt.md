---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/connectivity/bluetooth/api/connection_mgmt.html
...
:   Connection management.
    ...
    enum bt\_conn\_le\_path\_loss\_zone
    :   Path Loss zone that has been entered.
        ...
        *Values:*
        ...
        enumerator BT\_CONN\_LE\_PATH\_LOSS\_ZONE\_ENTERED\_LOW
        :   Low path loss zone entered.

        enumerator BT\_CONN\_LE\_PATH\_LOSS\_ZONE\_ENTERED\_MIDDLE
        :   Middle path loss zone entered.

        enumerator BT\_CONN\_LE\_PATH\_LOSS\_ZONE\_ENTERED\_HIGH
        :   High path loss zone entered.

        enumerator BT\_CONN\_LE\_PATH\_LOSS\_ZONE\_UNAVAILABLE
    ...
    int bt\_conn\_le\_set\_path\_loss\_mon\_param(struct bt\_conn \*conn, const struct [bt\_conn\_le\_path\_loss\_reporting\_param](#c.bt_conn_le_path_loss_reporting_param) \*param)
    :   Set Path Loss Monitoring Parameters.
        ...
        To use this API  [`CONFIG_BT_PATH_LOSS_MONITORING`](../../../kconfig.md#CONFIG_BT_PATH_LOSS_MONITORING "CONFIG_BT_PATH_LOSS_MONITORING") must be set.
    ...
    :   Initiate an LE connection to a remote device.
        ...
        The application must disable explicit scanning before initiating a new LE connection if  [`CONFIG_BT_SCAN_AND_INITIATE_IN_PARALLEL`](../../../kconfig.md#CONFIG_BT_SCAN_AND_INITIATE_IN_PARALLEL "CONFIG_BT_SCAN_AND_INITIATE_IN_PARALLEL") is not enabled.
    ...
    :   Automatically connect to remote devices in the filter accept list.
        ...
        This uses the Auto Connection Establishment procedure. The procedure will continue until a single connection is established or the procedure is stopped through [bt\_conn\_create\_auto\_stop](#group__bt__conn_1ga62dc2663b4fa39a33adb76dc9a136aa4). To establish connections to all devices in the filter accept list the procedure should be started again in the connected callback after a new connection has been established.
    ...
    int bt\_conn\_cb\_register(struct [bt\_conn\_cb](#c.bt_conn_cb) \*cb)
    ...
    static inline const char \*bt\_security\_err\_to\_str(enum [bt\_security\_err](#c.bt_security_err) err)
    :   Converts a security error to string.

        Returns:
        :   The string representation of the security error code. If  [`CONFIG_BT_SECURITY_ERR_TO_STR`](../../../kconfig.md#CONFIG_BT_SECURITY_ERR_TO_STR "CONFIG_BT_SECURITY_ERR_TO_STR") is not enabled, this just returns the empty string
    ...
    struct bt\_conn\_le\_path\_loss\_threshold\_report
    :   *#include <conn.h>*
        ...
        enum [bt\_conn\_le\_path\_loss\_zone](#c.bt_conn_le_path_loss_zone) zone
    ...
    struct bt\_conn\_le\_path\_loss\_reporting\_param
    :   *#include <conn.h>*
    ...
    :   *#include <conn.h>*
        ...
        This structure is used for tracking the state of a connection. It is registered with the help of the [bt\_conn\_cb\_register()](#group__bt__conn_1gaa9b79cd44734c1e560d6f30509be4d0b) API. It’s permissible to register multiple instances of this [bt\_conn\_cb](#structbt__conn__cb) type, in case different modules of an application are interested in tracking the connection state. If a callback is not of interest for an instance, it may be set to NULL and will as a consequence not be used for that instance.
