---
version: v3.7.0
source_url: https://docs.zephyrproject.org/3.7.0/connectivity/networking/overview.html
original_path: connectivity/networking/overview.html
---
...
- **IPv6** The support for IPv6 is enabled by default. Various IPv6 sub-options
  ...
  - The IPv6 neighbor discovery support
    ([RFC 4861](https://tools.ietf.org/html/rfc4861)) is enabled by default.
  - Multicast Listener Discovery v2 support
    ([RFC 3810](https://tools.ietf.org/html/rfc3810)) is enabled by default.
  - IPv6 header compression (6lo) is available for IPv6 connectivity for
    IEEE 802.15.4 networks ([RFC 4944](https://tools.ietf.org/html/rfc4944)).
...
- **UDP** User Datagram Protocol
  ([RFC 768](https://tools.ietf.org/html/rfc768)) is supported.
...
- **TCP** Transmission Control Protocol
  ([RFC 793](https://tools.ietf.org/html/rfc793)) is supported. Both server
...
- **BSD Sockets API** Support for a subset of a
  [BSD sockets compatible API](api/sockets.md#bsd-sockets-interface) is
...
- **MQTT** Message Queue Telemetry Transport (ISO/IEC PRF 20922) is supported.
  A sample [MQTT publisher](../../samples/net/mqtt_publisher/README.md#mqtt-publisher "Send MQTT PUBLISH messages to an MQTT server.") client application for MQTT v3.1.1 is
  implemented.
- **CoAP** Constrained Application Protocol
  ([RFC 7252](https://tools.ietf.org/html/rfc7252)) is supported.
  Both [CoAP client](../../samples/net/sockets/coap_client/README.md#coap-client "Use the CoAP library to implement a client that fetches a resource.") and [CoAP service](../../samples/net/sockets/coap_server/README.md#coap-server "Use the CoAP server subsystem to register CoAP resources.") sample
  applications are implemented.
- **LWM2M** OMA Lightweight Machine-to-Machine Protocol
  ([LwM2M specification 1.0.2](http://openmobilealliance.org/release/LightweightM2M/V1_0_2-20180209-A/OMA-TS-LightweightM2M-V1_0_2-20180209-A.pdf)) is supported via the “Bootstrap”, “Client
  ...
  well as several IPSO Smart Objects. ([LwM2M specification 1.1.1](http://openmobilealliance.org/release/LightweightM2M/V1_1_1-20190617-A/)) is
  supported in similar manner when enabled with a Kconfig option.
  [LwM2M client](../../samples/net/lwm2m_client/README.md#lwm2m-client "Implement a LwM2M client that connects to a LwM2M server.") sample implements the library as an example.
- **HTTP** Hypertext Transfer Protocol client and server are supported.
  [HTTP Client](api/http_client.md#http-client-interface) library supports HTTP/1.1 ([RFC 2616](https://tools.ietf.org/html/rfc2616)).
  [HTTP Server](api/http_server.md#http-server-interface) library supports HTTP/1.1 ([RFC 2616](https://tools.ietf.org/html/rfc2616)) and
  HTTP/2 ([RFC 9113](https://tools.ietf.org/html/rfc9113)).
  [HTTP Client](../../samples/net/sockets/http_client/README.md#sockets-http-client "Implement an HTTP(S) client that issues a variety of HTTP requests.") and
  [HTTP Server](../../samples/net/sockets/http_server/README.md#sockets-http-server "Implement an HTTP(s) Server demonstrating various resource types.") samples are provided.
- **DNS** Domain Name Service
  ([RFC 1035](https://tools.ietf.org/html/rfc1035)) client functionality
  is supported.
  ...
  be queried.
  Both multicast DNS (mDNS) ([RFC 6762](https://tools.ietf.org/html/rfc6762))
  and link-local multicast name resolution
  (LLMNR) ([RFC 4795](https://tools.ietf.org/html/rfc4795)) are supported.
...
- **Virtual LAN support.** Virtual LANs (VLANs) allow partitioning of physical
  ...
  See [VLAN support](api/vlan.md#vlan-interface) for more details.
- **Network traffic classification.** The sent and received network packets can
  be prioritized depending on application needs.
  See [traffic classification](api/traffic-class.md#traffic-class-support) for more details.
- **Time Sensitive Networking.** The gPTP (generalized Precision Time Protocol)
  is supported. See [gPTP support](api/gptp.md#gptp-interface) for more details.
- **Network shell.** The network shell provides helpers for figuring out
  ...
  See [network shell](api/net_shell.md#net-shell) for more details.
