---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/services/binary_descriptors/index.html
...
> 1. RAM - assuming the descriptors have been copied to RAM (e.g. by a bootloader), they
>    can be read from the buffer they reside in.
> 2. Memory mapped flash - If the flash where the image to be read resides in flash and is
...
> 3. Flash - Using an internal buffer, the descriptors are read one by one using the flash API,
>    and given to the user while they’re in the buffer.

To enable reading descriptors, enable [`CONFIG_BINDESC_READ`](../../kconfig.md#CONFIG_BINDESC_READ "CONFIG_BINDESC_READ"). The three backends are
enabled by these Kconfig symbols, respectively: [`CONFIG_BINDESC_READ_RAM`](../../kconfig.md#CONFIG_BINDESC_READ_RAM "CONFIG_BINDESC_READ_RAM"),
[`CONFIG_BINDESC_READ_MEMORY_MAPPED_FLASH`](../../kconfig.md#CONFIG_BINDESC_READ_MEMORY_MAPPED_FLASH "CONFIG_BINDESC_READ_MEMORY_MAPPED_FLASH"), and [`CONFIG_BINDESC_READ_FLASH`](../../kconfig.md#CONFIG_BINDESC_READ_FLASH "CONFIG_BINDESC_READ_FLASH").
...
- [Binary descriptors "Hello World"](../../samples/subsys/bindesc/hello_bindesc/README.md#hello-bindesc "Set and access binary descriptors for a basic Zephyr application.")Set and access binary descriptors for a basic Zephyr application.

[Bindesc Read](../../doxygen/html/group__bindesc__read.md)

Related code samples

- [Binary descriptors read](../../samples/subsys/bindesc/read_bindesc/README.md#read-bindesc "Define some binary descriptors and read them.")Define some binary descriptors and read them.
