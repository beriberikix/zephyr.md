---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/services/storage/zms/zms.html
original_path: services/storage/zms/zms.html
---
...
writing a garbage\_collect\_done ATE and the close ATE (one of the header entries).
...
| Sector 0 (closed) | Sector 1 (open) | Sector 2 (empty) |
| --- | --- | --- |
| Data\_a0 | Data\_b0 | Data\_c0 |
| Data\_a1 | Data\_b1 | Data\_c1 |
| Data\_a2 | Data\_b2 | Data\_c2 |
| GC\_done | . | . |
| . | . | . |
| . | . | . |
| . | ATE\_b2 | ATE\_c2 |
| ATE\_a2 | ATE\_b1 | ATE\_c1 |
| ATE\_a1 | ATE\_b0 | ATE\_c0 |
| ATE\_a0 | GC\_done | GC\_done |
| Close (cyc=1) | Close (cyc=1) | Close (cyc=1) |
| Empty (cyc=1) | Empty (cyc=2) | Empty (cyc=2) |
...
properties are correct (sector\_size, sector\_count …) then calling the zms\_init function to
make the storage ready.

To mount the filesystem some elements in the zms\_fs structure must be initialized.
...
Each sector has a lead cycle counter which is a uin8\_t that is used to validate all the other
ATEs.
...
The CRC of the data is checked only when the whole the element is read.
The CRC of the data is not checked for a partial read, as it is computed for the whole element.

Note

Enabling the CRC feature on previously existing ZMS content without CRC enabled
...
The maximum single data length that could be written at once in a sector is 64K
...
in each sectors (\(\frac{944}{80}\)).
Total data that could be stored in this partition for this case is \(11 \times 3 \times 64 = 2112 \text{ bytes}\)
...
See [Available space for user data (key-value pairs)](#free-space).
...
\(\frac{(944 \times 4)}{16} = 236 \text{ minutes}\).
...
typical set of data.
For id/data pair with data <= 8 bytes, effective\_size is 16 bytes
For id/data pair with data > 8 bytes, effective\_size is 16 bytes + sizeof(data)
Let’s suppose that total\_effective\_size is the total size of the set of data that is written in
...
`SECTOR_EFFECTIVE_SIZE`: is the size sector - header\_size(80 bytes)
...
functionality: [NVS](../nvs/nvs.md#nvs-api) and [FCB](../fcb/fcb.md#fcb-api).
...
- If you are using a non-erasable technology device like RRAM or MRAM, [ZMS](#zms-api) is definitely the
...
- For devices with large write\_block\_size and/or needs a sector size that is different than the
  classical flash page size (equal to erase\_block\_size), [ZMS](#zms-api) is also the best fit as there is
  the possibility to customize these parameters and add the support of these devices in ZMS.
- For classical flash technology devices, [NVS](../nvs/nvs.md#nvs-api) is recommended as it has low footprint (smaller
...
- If your application needs more than 64K IDs for storage, [ZMS](#zms-api) is recommended here as it
  has a 32-bit ID field.
- If your application is working in a FIFO mode (First-in First-out) then [FCB](../fcb/fcb.md#fcb-api) is
...
life expectancy of the device described in this section: [Wear leveling](#wear-leveling).
...
- The total size of the storage partition should be well dimensioned to achieve the best
  ...
  in the documentation. See [Available space for user data (key-value pairs)](#free-space).
...
- For some subsystems like [Settings](../../settings/index.md#settings-api), all path-value pairs are split into two ZMS entries (ATEs).
...
- Using small data to store in the ZMS entries can increase the performance, as this data is
  ...
  For example, for the [Settings](../../settings/index.md#settings-api) subsystem, choosing a path name that is
...
- Each additional cache entry will add 8 bytes to your RAM usage. Cache size should be carefully
  chosen.
- If you use ZMS through [Settings](../../settings/index.md#settings-api), you have to take into account that each Settings entry is
  divided into two ZMS entries. The recommended cache size should be, at least, twice the number
  of Settings entries.

## Sample

A sample of how ZMS can be used is supplied in [Zephyr Memory Storage (ZMS)](../../../samples/subsys/fs/zms/README.md#zms "Store and retrieve data from storage using the ZMS API.").
...
- [Zephyr Memory Storage (ZMS)](../../../samples/subsys/fs/zms/README.md#zms "Store and retrieve data from storage using the ZMS API.")Store and retrieve data from storage using the ZMS API.
