---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/services/debugging/coredump.html
...
- `DEBUG_COREDUMP_MEMORY_DUMP_LINKER_RAM`: Dumps the memory region between
  \_image\_ram\_start[] and \_image\_ram\_end[]. This includes at least data, noinit,
...
This was done on [QEMU Emulation for X86](../../boards/qemu/x86/doc/index.md#qemu_x86) where a null pointer was dereferenced.
...
| Field | Data Type | Description |
| --- | --- | --- |
| ID | `char` | `T` to indicate this is a threads metadata block. |
| Header version | `uint16_t` | Identify the version of the header. This needs to be incremented whenever the header struct is modified. This allows parser to reject older header versions so it will not incorrectly parse the header. |
| Number of bytes | `uint16_t` | Number of bytes following the header which contains the byte stream for target data. |
| Byte stream | `uint8_t[]` | Contains data necessary for debugging threads. |
...
2. Implement [`arch_coredump_tgt_code_get()`](../../doxygen/html/group__arch-coredump.md#ga1d9b0c011a03434272ff9bec120f273d) simply to return
   the newly introduced target code.
3. Implement [`arch_coredump_info_dump()`](../../doxygen/html/group__arch-coredump.md#gae997cb3dc249bec6d5166145f37d9292) to construct
   a target architecture block and call [`coredump_buffer_output()`](../../doxygen/html/group__coredump__apis.md#gabe58e47cc733572a6650d3fd2cfc7d9d)
...
4. Add a parser to the core dump GDB stub scripts under
   ...
   3. Parse the architecture-specific block from
      ...
      as implemented in step 3 (inside [`arch_coredump_info_dump()`](../../doxygen/html/group__arch-coredump.md#gae997cb3dc249bec6d5166145f37d9292)).
