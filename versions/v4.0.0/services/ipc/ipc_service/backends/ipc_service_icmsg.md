---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/services/ipc/ipc_service/backends/ipc_service_icmsg.html
original_path: services/ipc/ipc_service/backends/ipc_service_icmsg.html
---
...
memory footprint. The ICMsg backend is build on top of [Single Producer Single Consumer Packet Buffer](../../../../kernel/data_structures/spsc_pbuf.md#spsc-pbuf).
...
1. The domain (or CPU) writes a magic number to its `tx-region` of the shared
   memory.
2. It then sends a signal to the other domain or CPU, informing that the data
   has been written. Sending the signal to the other domain or CPU is repeated
   with timeout specified by
   [`CONFIG_IPC_SERVICE_ICMSG_BOND_NOTIFY_REPEAT_TO_MS`](../../../../kconfig.md#CONFIG_IPC_SERVICE_ICMSG_BOND_NOTIFY_REPEAT_TO_MS "CONFIG_IPC_SERVICE_ICMSG_BOND_NOTIFY_REPEAT_TO_MS") option.
3. When the signal from the other domain or CPU is received, the magic number
   ...
   [`ipc_service_cb.bound`](../../../../doxygen/html/structipc__service__cb.md#a76be12bf8a7964977b5d7512dd010110) callback.

## Samples

> - [IPC service: icmsg backend](../../../../samples/subsys/ipc/ipc_service/icmsg/README.md#ipc-icmsg "Send messages between two cores using the IPC service and icmsg backend.")
...
| Field name | Size (bytes) | Byte order | Description |
| --- | --- | --- | --- |
| `rd_idx` | 4 | little‑endian | Index of the first incoming byte in the `data` field. |
| `padding` | depends on cache alignment | n/a | Padding added to align `wr_idx` to the cache alignment. |
| `wr_idx` | 4 | little‑endian | Index of the byte after the last incoming byte in the `data` field. |
| `data` | everything to the end of the region | n/a | Circular buffer containing actual bytes to transfer. |
...
| Field name | Size (bytes) | Byte order | Description |
| --- | --- | --- | --- |
| `len` | 2 | big‑endian | Length of the `data` field. |
| `reserved` | 2 | n/a | Reserved for the future use. It must be 0 for the current protocol version. |
| `data` | `len` | n/a | Packet data. |
| `padding` | 0‑3 | n/a | Padding is added to align the total packet size to 4 bytes. |

The packet send procedure is the following:

1. Check if the packet fits into the buffer.
2. Write the packet to `data` FIFO buffer starting at `wr_idx`.
   Wrap it if needed.
3. Write a new value of the `wr_idx`.
4. Notify the receiver over the MBOX channel.

### Initialization

The initialization sequence is the following:

1. Set the `wr_idx` and `rd_idx` to zero.
2. Push a single packet to FIFO containing magic data: `45 6d 31 6c 31 4b 30 72 6e 33 6c 69 34`.
   The MBOX is not used yet.
3. Initialize the MBOX.
4. Repeat the notification over the MBOX channel using some interval, for example, 1 ms.
5. Wait for an incoming packet containing the magic data.
   It will arrive over the other pair (shared memory region and MBOX).
6. Stop repeating the MBOX notification.
