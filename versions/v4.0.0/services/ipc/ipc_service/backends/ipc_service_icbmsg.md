---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/services/ipc/ipc_service/backends/ipc_service_icbmsg.html
original_path: services/ipc/ipc_service/backends/ipc_service_icbmsg.html
---

# ICMsg with dynamically allocated buffers backend

This backend is built on top of the [ICMsg backend](ipc_service_icmsg.md#ipc-service-backend-icmsg).
...
- The sender allocates one or more blocks.
  If there are not enough sequential blocks, it waits using the timeout provided in the parameter that also includes K\_FOREVER and K\_NO\_WAIT.
...
- [IPC service: Multi-endpoint](../../../../samples/subsys/ipc/ipc_service/multi_endpoint/README.md#ipc_multi_endpoint "Use the IPC Service with multiple endpoints.")
...
1. Align region boundaries to cache:
...
2. Calculate the minimum size required for ICMsg area `icmsg_min_size`, which is a sum of:
...
3. Calculate available size for block area. Note that the actual size may be smaller because of block alignment:

   `blocks_area_available_size = region_size_aligned - icmsg_min_size`
4. Calculate single block size:

   `block_size = ROUND_DOWN(blocks_area_available_size / local_blocks, alignment)`
5. Calculate actual block area size:

   `blocks_area_size = block_size * local_blocks`
6. Calculate block area start address:
...
![../../../../_images/icbmsg_memory.svg](https://docs.zephyrproject.org/4.0.0/_images/icbmsg_memory.svg)
...
1. The sender wants to send a message that contains `K` bytes.
2. The sender reserves blocks from his `tx-region` blocks area that can hold at least `K + 4` bytes.
...
3. The sender fills the header with a 32-bit integer value, `K` (little-endian).
4. The sender fills the remaining part of the blocks with his data.
   Unused space is ignored.
5. The sender sends an `MSG_DATA` or `MSG_BOUND` control message over ICMsg that contains starting block number (where the header is located).
   Details about the control message are in the next section.
6. The control message travels to the receiver.
7. The receiver reads message size and data from his `rx-region` starting from the block number received in the control message.
8. The receiver processes the message.
9. The receiver sends `MSG_RELEASE_DATA` or `MSG_RELEASE_BOUND` control message over ICMsg containing the starting block number
   (the same as inside received control message).
10. The control message travels back to the sender.
...
![../../../../_images/icbmsg_message.svg](https://docs.zephyrproject.org/4.0.0/_images/icbmsg_message.svg)
...
| --- | --- | --- |
| MSG\_DATA | endpoint address | block number |
| 0x00 | 0x00 ÷ 0xFD | 0x00 ÷ N-1 |
...
| byte 0 | byte 1 | byte 2 |
| --- | --- | --- |
| MSG\_RELEASE\_DATA | unused | block number |
| 0x01 |  | 0x00 ÷ N-1 |
...
| byte 0 | byte 1 | byte 2 |
| --- | --- | --- |
| MSG\_BOUND | endpoint address | block number |
| 0x02 | 0x00 ÷ 0xFD | 0x00 ÷ N-1 |
...
| byte 0 | byte 1 | byte 2 |
| --- | --- | --- |
| MSG\_RELEASE\_BOUND | endpoint address | block number |
| 0x03 | 0x00 ÷ 0xFD | 0x00 ÷ N-1 |
...
| Header | Endpoint name | Null-terminator |
| --- | --- | --- |
| bytes 0-3 | bytes 4-10 | byte 11 |
| 0x00000008 | `example` | 0x00 |
...
1. The initiator assigns an endpoint address to this endpoint.
2. The initiator sends a binding message containing the endpoint name and address.
3. The initiator waits for any message from the follower using this endpoint address.
   Usually, it will be `MSG_RELEASE_BOUND`, but `MSG_DATA` is also allowed.
4. The initiator is bound to an endpoint, and it can send data messages using this endpoint.
...
1. The follower waits for a binding message containing its endpoint name.
   The message may be a newly received message or a message stored before the binding procedure started.
2. The follower stores the endpoint address assigned to this endpoint by the initiator.
3. The follower sends the `MSG_RELEASE_BOUND` control message.
4. The follower is bound to an endpoint, and it can send data messages using this endpoint.
...
![../../../../_images/icbmsg_flows.svg](https://docs.zephyrproject.org/4.0.0/_images/icbmsg_flows.svg)
