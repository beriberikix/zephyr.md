---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/security/secure-coding.html
original_path: security/secure-coding.html
---
...
uncontrolled access can be devastating [[1]](#attackf).
...
Much of this document comes from [[CIIBPB]](security-citations.md#ciibpb).

## Introduction and Scope

This document covers guidelines for the [Zephyr Project](https://www.zephyrproject.org/), from a
...
a section on [Secure development knowledge](#secure-development-knowledge), which
...
adhering to a defined set of design standards. In [[SALT75]](security-citations.md#salt75), the following,
...
- **Economy of mechanism** specifies that the underlying design of a
  ...
  [[PAUL09]](security-citations.md#paul09) and abstracted APIs.
...
- **Fail-safe defaults** defines that access is restricted by default
  ...
  Default” paradigm [[MS12]](security-citations.md#ms12).
- **Separation of privilege** is the principle that two conditions or
  ...
  of the Zephyr project, this could encompass split keys [[PAUL09]](security-citations.md#paul09).
...
- **Least common mechanism** specifies that mechanisms common to more
  ...
  required. The example given in [[SALT75]](security-citations.md#salt75) is a function that should be
...
- **Complementary Security/Defense in Depth**: do not rely on a single
  ...
  platform, or are not trusted, a defense in depth [[MS12]](security-citations.md#ms12) paradigm
  shall be used.
- **Less commonly used services off by default**: to reduce the
  ...
  threshold of 80% is given in [[MS12]](security-citations.md#ms12)). For the Zephyr project, this can
...
including the 8 principles from [[SALT75]](security-citations.md#salt75):
...
[CWE/SANS top 25](http://cwe.mitre.org/top25/) or [OWASP Top 10](https://owasp.org/www-project-top-ten/) for commonly used lists.

A free class from the nonprofit OpenSecurityTraining2 for C/C++ developers
is available at [OST2\_1001](https://ost2.fyi/Vulns1001). It teaches how to prevent, detect, and
mitigate linear stack/heap buffer overflows, non-linear out of bound writes,
integer overflows, and other integer issues. The follow-on class, [OST2\_1002](https://ost2.fyi/Vulns1002),
...
The Zephyr project shall have an issue tracking system (such as [GitHub](https://www.github.com))
...
An [attack](http://www.theverge.com/2016/10/21/13362354/dyn-dns-ddos-attack-cause-outage-status-explained) resulted in a significant portion of DNS
