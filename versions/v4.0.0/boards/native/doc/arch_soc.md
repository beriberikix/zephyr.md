---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/boards/native/doc/arch_soc.html
original_path: boards/native/doc/arch_soc.html
---
...
The native simulator in combination with the POSIX architecture and the inf\_clock SOC layer,
...
[POSIX OS abstraction](../../../services/portability/posix/index.md#posix-support).
...
[native\_sim](../native_sim/doc/index.md#native-sim), and the [bsim boards](bsim_boards_design.md#bsim-boards).
...
with focus on their radio (e.g. BT LE) and utilize the [BabbleSim](https://BabbleSim.github.io) physical layer
...
You may want to consider WSL2, or, if using [native\_sim](../native_sim/doc/index.md#native-sim),
you can also just use the `native_sim/native/64`
target: Check [32 and 64bit versions](../native_sim/doc/index.md#native-sim32-64).
Otherwise [with some tinkering](https://github.com/microsoft/WSL/issues/2468#issuecomment-374904520) it
should be possible to make it work.

## [Important limitations](#id5)
...
[Rationale for this port](#rationale-for-this-port) and [Architecture](#posix-arch-architecture)
sections
...
compiled call to [`k_cpu_idle()`](../../../doxygen/html/group__cpu__idle__apis.md#ga7b25e1bed511a813b32fbd0f91b09356) if you are waiting for an
interrupt, or a call to [`k_busy_wait()`](../../../doxygen/html/group__thread__apis.md#ga550b642e071480323e589866abb99c22) with some small delay in
microseconds.
...
## [Significant unsupported features](#id6)

Currently, these are the most significant features which are not supported in this architecture:

- [User mode/userspace](../../../kernel/usermode/index.md#usermode-api): When building for these targets,
  [`CONFIG_USERSPACE`](../../../kconfig.md#CONFIG_USERSPACE "CONFIG_USERSPACE") will always be disabled,
  and all calls into the kernel will be done as normal calls.
- Stack checks: [`CONFIG_HW_STACK_PROTECTION`](../../../kconfig.md#CONFIG_HW_STACK_PROTECTION "CONFIG_HW_STACK_PROTECTION"),
  [`CONFIG_STACK_CANARIES`](../../../kconfig.md#CONFIG_STACK_CANARIES "CONFIG_STACK_CANARIES"), and
  ...
  [the architecture section’s architecture layer paragraph](#posix-arch-design-archl)
  for more information.

## [Rationale for this port](#id7)
...
## [Comparison with other options](#id8)
...
![Comparison of different debugging targets](https://docs.zephyrproject.org/4.0.0/_images/Port_vs_QEMU_vs.svg)
...
## [Architecture and design](#id9)

Note

This section does not describe anymore the old [native\_posix](../native_posix/doc/index.md#native-posix) or
[`CONFIG_NATIVE_APPLICATION`](../../../kconfig.md#CONFIG_NATIVE_APPLICATION "CONFIG_NATIVE_APPLICATION") based architecture.
It only describes the new native simulator based architecture used by targets built with the
[`CONFIG_NATIVE_LIBRARY`](../../../kconfig.md#CONFIG_NATIVE_LIBRARY "CONFIG_NATIVE_LIBRARY") option.
...
![Zephyr layering in a native simulator build](https://docs.zephyrproject.org/4.0.0/_images/layering_natsim.svg)

Zephyr layering when built against an embedded target (left), and targeting a native\_simulator
based board (right)

![native_sim boards and the native simulator](https://docs.zephyrproject.org/4.0.0/_images/components_natsim.svg)

Relationship between Zephyr, the native\_sim target and the native simulator

When building targeting Zephyr’s [native\_sim](../native_sim/doc/index.md#native-sim) board, we build our embedded SW,
that is, our application, the Zephyr kernel, and any subsystems and drivers we have selected,
with the [POSIX architecture](#posix-arch-design-archl) and the
[inf\_clock](#posix-arch-design-socl) SOC layers.
The result of this build is a pre-linked elf library, which contains what we can call the
embedded SW.
Then the [native simulator](https://github.com/BabbleSim/native_simulator/) runner will be built.
...
executable lifetime.
When targeting native\_sim, the native simulator is also built with some basic HW models like a
system timer and an interrupt controller.
You can find more information on these in the
[native simulator design documentation](https://github.com/BabbleSim/native_simulator/blob/main/docs/Design.md).

The native\_sim target is a single microcontroller (MCU) target with simple HW models. Other targets
like the [simulated nRF5340 (nrf5340bsim)](../nrf_bsim/doc/nrf5340bsim.md#nrf5340bsim) are multi MCU targets. Where one
...
[arch/posix/CMakeLists.txt](https://github.com/zephyrproject-rtos/zephyr/blob/main/arch/posix/CMakeLists.txt) for more information.

You can find more information in the native simulator
[build documentation](https://github.com/BabbleSim/native_simulator/blob/main/docs/Design.md#build-and-symbol-visibility)
...
It also provides the [native\_tasks](#posix-arch-design-native-tasks), and specifies
...
[design documentation](https://github.com/BabbleSim/native_simulator/blob/main/docs/Design.md#overall-execution)

For more complex simulated boards, like [bsim ones](bsim_boards_design.md#bsim-boards), the board layer also provides
...
This does not need to be the same for all boards, but both [native\_sim](../native_sim/doc/index.md#native-sim) and the
[nrf\*bsim boards](bsim_boards_design.md#bsim-boards) work similarly through the combination of a board specific
[`arch_busy_wait()`](../../../doxygen/html/group__arch-timing.md#gaffc9f3013d53e72c25243ce4f972549f) and an special fake HW timer provided by the native simulator.

Please check the
[native simulator busy wait design documentation](https://github.com/BabbleSim/native_simulator/blob/main/docs/Design.md#busy-waits)
for more info.

### NATIVE\_TASKS

The soc\_inf layer provides a special type of hook called the NATIVE\_TASKS.
...
[native simulator tasks](https://github.com/BabbleSim/native_simulator/blob/main/docs/Design.md#native-simulator-tasks)
