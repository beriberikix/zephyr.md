---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/bindings-upstream.html
original_path: build/dts/bindings-upstream.html
---
...
## [Always check for existing bindings](#id1)

Zephyr aims for devicetree [Source compatibility with other operating systems](design.md#dt-source-compatibility). Therefore, if there
...
- There is an existing binding in the mainline Linux kernel. See
  `Documentation/devicetree/bindings` in [Linus’s tree](https://github.com/torvalds/linux/) for existing
  bindings and the [Linux devicetree documentation](https://www.kernel.org/doc/html/latest/devicetree/index.html) for more information.
...
the same [design guidelines laid out by Linux](https://docs.kernel.org/devicetree/bindings/writing-bindings.html).
...
### [Recommendations are requirements](#id4)

All recommendations in [default](bindings-syntax.md#dt-bindings-default) are requirements when
...
display properly in the [Bindings index](api/bindings.md#devicetree-binding-index).

### [Naming conventions](#id6)
...
## [Rules for vendor prefixes](#id7)

The following general rules apply to vendor prefixes in [compatible](intro-syntax-structure.md#dt-important-props) properties.
...
- If your device is manufactured by a specific vendor, then its compatible
  ...
  [dts/bindings/vendor-prefixes.txt](https://github.com/zephyrproject-rtos/zephyr/blob/main/dts/bindings/vendor-prefixes.txt), you must use that vendor
  prefix.
...
- Do not submit additions to Zephyr’s `dts/bindings/vendor-prefixes.txt`
  ...
  `dts/bindings/vendor-prefixes.txt` file to any directory in your
  [DTS\_ROOT](../../develop/application/index.md#dts-root). The devicetree tooling will respect these
...
- If your binding is describing an abstract class of hardware with Zephyr
  ...
  the vendor prefix. See [Zephyr-specific binding (zephyr)](api/bindings.md#dt-vendor-zephyr) for examples.

## [Rules for default values](#id8)
...
in the [Bindings index](api/bindings.md#devicetree-binding-index) output.
...
- Zephyr-specific extensions to bindings we share with upstream Linux. One
  ...
  controllers defined in [dts/bindings/adc/adc-controller.yaml](https://github.com/zephyrproject-rtos/zephyr/blob/main/dts/bindings/adc/adc-controller.yaml).
