---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/api/bindings/sensor/maxim,max31865.html
original_path: build/dts/api/bindings/sensor/maxim,max31865.html
---
...
[drivers/sensor/maxim/max31865](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/sensor/maxim/max31865).
...
| Name | Type | Details |
| --- | --- | --- |
| `resistance-at-zero` | `int` | ```text Sensor resistance in ohms at 0 Celsius (100 ohms for PT100, 1000 ohms for PT1000) ```  This property is **required**. |
| `resistance-reference` | `int` | ```text Circuit reference resistance in ohms (recommended on MAX31865 datasheet 400 ohms for PT100, 4000 ohms for PT1000, the Adafruit boards use 430 ohms and 4300 ohms) ```  This property is **required**. |
| `low-threshold` | `int` | ```text Low fault threshold (ADC CODE, 15-bit value, unit-free, default value is the minimum value) ``` |
| `high-threshold` | `int` | ```text High fault threshold (ADC CODE, 15-bit value, unit-free, default value is the maximum value) ```  Default value: `32767` |
| `maxim,3-wire` | `boolean` | ```text 3-wire enabled (@a true) or 2-wire/4-wire (@a false) ``` |
| `filter-50hz` | `boolean` | ```text 50 Hz filter enabled (@a true) or 60 Hz filter enabled (@a false) ``` |
...
may apply to the “maxim,max31865” compatible.
