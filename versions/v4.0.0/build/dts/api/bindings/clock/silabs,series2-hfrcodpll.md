---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/api/bindings/clock/silabs,series2-hfrcodpll.html
original_path: build/dts/api/bindings/clock/silabs,series2-hfrcodpll.html
---
...
| Name | Type | Details |
| --- | --- | --- |
| `dpll-n` | `int` | ```text Numerator used in f_out = f_ref * (n+1) / (m+1) ``` |
| `dpll-m` | `int` | ```text Denominator used in f_out = f_ref * (n+1) / (m+1) ``` |
| `dpll-edge` | `string` | ```text Which edge of the reference is detected ```  Legal values: `'fall'`, `'rise'` |
| `dpll-lock` | `string` | ```text PLL lock mode. freq: Frequency-lock loop mode phase: Phase-lock loop mode ```  Legal values: `'freq'`, `'phase'` |
| `dpll-autorecover` | `boolean` | ```text Automatically re-lock if the PLL loses the lock ``` |
| `dpll-dither` | `boolean` | ```text Enable dither function ``` |
...
may apply to the “silabs,series2-hfrcodpll” compatible.
