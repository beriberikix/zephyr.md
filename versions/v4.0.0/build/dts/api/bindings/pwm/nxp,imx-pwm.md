---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/api/bindings/pwm/nxp,imx-pwm.html
original_path: build/dts/api/bindings/pwm/nxp,imx-pwm.html
---
...
[drivers/pwm/pwm\_mcux.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/pwm/pwm_mcux.c).
...
| Name | Type | Details |
| --- | --- | --- |
| `index` | `int` | ```text flexpwm submodule index ```  This property is **required**. |
| `run-in-wait` | `boolean` | ```text Enable for PWM to keep running in WAIT mode. ``` |
| `run-in-debug` | `boolean` | ```text Enable for PWM to keep running in debug mode. ``` |
| `nxp,prescaler` | `int` | ```text Clock prescaler at the input of the PWM. ```  This property is **required**.  Legal values: `1`, `2`, `4`, `8`, `16`, `32`, `64`, `128` |
| `nxp,reload` | `string` | ```text Select how to load the buffered-registers with new values: "immediate" - buffered-registers get loaded with new values as soon as LDOK bit is set; "half-cycle" - registers loaded on a PWM half cycle; "full-cycle" - registers loaded on a PWM full cycle; "half-and-full-cycle" - registers loaded on a PWM half & full cycle. ```  Legal values: `'immediate'`, `'half-cycle'`, `'full-cycle'`, `'half-and-full-cycle'` |
...
may apply to the “nxp,imx-pwm” compatible.
