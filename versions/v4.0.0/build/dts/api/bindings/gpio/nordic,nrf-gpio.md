---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/api/bindings/gpio/nordic,nrf-gpio.html
original_path: build/dts/api/bindings/gpio/nordic,nrf-gpio.html
---
...
[drivers/gpio/gpio\_nrfx.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/gpio/gpio_nrfx.c).
...
These property descriptions apply to “nordic,nrf-gpio”
...
| Name | Type | Details |
| --- | --- | --- |
| `gpiote-instance` | `phandle` | ```text GPIOTE instance that can be used with this GPIO port. ``` |
| `#gpio-cells` | `int` | ```text Number of items to expect in a GPIO specifier ```  This property is **required**.  Constant value: `2` |
| `sense-edge-mask` | `int` | ```text Mask of pins that use the GPIO sense mechanism for edge detection. Pins not included in the mask use GPIOTE channels in the event mode. ``` |
| `port` | `int` | ```text The GPIO port number. GPIO port P0 has:    port = <0>;  And P1 has:    port = <1>; ```  This property is **required**. |
...
may apply to the “nordic,nrf-gpio” compatible.
