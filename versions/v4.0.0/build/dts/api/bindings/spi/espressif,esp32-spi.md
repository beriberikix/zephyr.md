---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/api/bindings/spi/espressif,esp32-spi.html
original_path: build/dts/api/bindings/spi/espressif,esp32-spi.html
---
...
[drivers/spi/spi\_esp32\_spim.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/spi/spi_esp32_spim.c).
...
| `half-duplex` | `boolean` | ```text Enable half-duplex communication mode.  Transmit data before receiving it, instead of simultaneously ``` |
| `dummy-comp` | `boolean` | ```text Enable dummy SPI compensation cycles ``` |
| `sio` | `boolean` | ```text Enable 3-wire mode  Use MOSI for both sending and receiving data ``` |
| `dma-enabled` | `boolean` | ```text Enable SPI DMA support ``` |
| `dma-clk` | `int` | ```text DMA clock source ``` |
| `dma-host` | `int` | ```text DMA Host - 0 -> SPI2, 1 -> SPI3 ``` |
| `clk-as-cs` | `boolean` | ```text Support to toggle the CS while the clock toggles  Output clock on CS line if CS is active ``` |
| `positive-cs` | `boolean` | ```text Make CS positive during a transaction instead of negative ``` |
| `use-iomux` | `boolean` | ```text Some pins are allowed to bypass the GPIO Matrix and use the IO_MUX routing mechanism instead, this avoids extra routing latency and makes possible the use of operating frequencies higher than 20 MHz.  Refer to SoC's Technical Reference Manual to check which pins are allowed to use this routing path. ``` |
| `cs-setup-time` | `int` | ```text Chip select setup time setting, see TRF for SOC for details of timing applied. ``` |
| `cs-hold-time` | `int` | ```text Chip select hold time setting, see TRF for SOC for details of timing applied. ``` |
| `line-idle-low` | `boolean` | ```text Default MISO and MOSI pins GPIO level when idle. Defaults to high by default. ``` |
