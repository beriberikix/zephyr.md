---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/api/bindings/spi/nxp,imx-flexspi.html
...
[drivers/memc/memc\_mcux\_flexspi.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/memc/memc_mcux_flexspi.c).
...
| `rx-clock-source-b` | `int` | ```text Source clock for flash read on port B. Only supported by some instances of this IP. See the RXCLKSRC_B field in register MCR2. The default corresponds to the reset value of the register field. ```  Legal values: `0`, `1`, `2`, `3` |
...
| `ahb-boundary` | `string` | ```text Sets the AHB read/write boundary. Only supported by some versions of the FLEXSPI IP. When set, all memory accesses that cross an address boundary of the specified size will be divided into smaller sub accesses. ```  Default value: `no-boundary`  Legal values: `'no-boundary'`, `'1024'`, `'512'`, `'256'` |
