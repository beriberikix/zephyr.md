---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/api/bindings/adc/nxp,lpc-lpadc.html
original_path: build/dts/api/bindings/adc/nxp,lpc-lpadc.html
---
...
[drivers/adc/adc\_mcux\_lpadc.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/adc/adc_mcux_lpadc.c).
...
These property descriptions apply to “nxp,lpc-lpadc”
...
| Name | Type | Details |
| --- | --- | --- |
| `clk-divider` | `int` | ```text clock divider for the converter ``` |
| `clk-source` | `int` | ```text source to attach the ADC clock to ``` |
| `voltage-ref` | `int` | ```text Voltage reference selection. Corresponds to value of register field CFG[REFSEL] (see chip specific manual). ```  This property is **required**.  Legal values: `0`, `1`, `2`, `3` |
| `calibration-average` | `int` | ```text conversion average number for auto-calibration ```  Legal values: `1`, `2`, `4`, `8`, `16`, `32`, `64`, `128` |
| `no-power-level` | `boolean` | ```text There is no power level selection function. ``` |
| `power-level` | `int` | ```text Power level selection. Corresponds to the value of register field CFG[PWRSEL] (see chip specific manual). ```  Legal values: `0`, `1`, `2`, `3` |
| `offset-value-a` | `int` | ```text Offset value A to use if CONFIG_LPADC_DO_OFFSET_CALIBRATION is false ```  This property is **required**. |
| `offset-value-b` | `int` | ```text Offset value B to use if CONFIG_LPADC_DO_OFFSET_CALIBRATION is false ```  This property is **required**. |
| `nxp,references` | `phandle-array` | ```text References to required regulators which must be enabled for LPADC to function ``` |
...
may apply to the “nxp,lpc-lpadc” compatible.
