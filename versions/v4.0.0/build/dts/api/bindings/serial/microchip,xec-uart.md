---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/build/dts/api/bindings/serial/microchip,xec-uart.html
original_path: build/dts/api/bindings/serial/microchip,xec-uart.html
---
...
[drivers/serial/uart\_mchp\_xec.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/serial/uart_mchp_xec.c).
...
| `girqs` | `array` | ```text UART GIRQ and bit position in EC interrupt aggregator ```  This property is **required**. |
| `pcrs` | `array` | ```text UART Power Clock Reset(PCR) register index and bit position ```  This property is **required**. |
...
| `wakerx-gpios` | `phandle-array` | ```text GPIO configured as UART RX wake source ``` |
...
may apply to the “microchip,xec-uart” compatible.
