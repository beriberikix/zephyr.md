---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/develop/west/manifest.html
original_path: develop/west/manifest.html
---
...
`west.manifest` module, see [west.manifest](west-apis.md#west-apis-manifest). For a more general
introduction and command overview, see [Basics](basics.md#west-basics).

## [Multiple Repository Model](#id11)

West’s view of the repositories in a [west workspace](../../glossary.md#term-west-workspace), and their
history, looks like the following figure (though some parts of this example are
specific to upstream Zephyr’s use of west):

![West multi-repo history](https://docs.zephyrproject.org/4.0.0/_images/west-mr-model.png)
...
file system. Note that west projects [are different from modules](../modules.md#modules-vs-projects).
...
| `revision` | Optional. The Git revision that `west update` should check out. This will be checked out as a detached HEAD by default, to avoid conflicting with local branch names. If not given, the `revision` value from the `defaults` subsection will be used if present.  A project revision can be a branch, tag, or SHA.  The default `revision` is `master` if not otherwise specified.  Using `HEAD~0` [[1]](#f1) as the `revision` will cause west to keep the current state of the project. |
...
| `west-commands` | Optional. If given, a relative path to a YAML file within the project which describes additional west commands provided by that project. This file is named `west-commands.yml` by convention. See [Extensions](extensions.md#west-extensions) for details. |
| `import` | Optional. If `true`, imports projects from manifest files in the given repository into the current manifest. See [Manifest Imports](#west-manifest-import) for details. |
| `groups` | Optional, a list of groups the project belongs to. See [Project Groups](#west-manifest-groups) for details. |
| `submodules` | Optional. You can use this to make `west update` also update [Git submodules](https://git-scm.com/book/en/v2/Git-Tools-Submodules) defined by the project. See [Git Submodules in Projects](#west-manifest-submodules) for details. |
| `userdata` | Optional. The value is an arbitrary YAML value. See [Repository user data](#west-project-userdata). |
...
the location of the corresponding `west-commands.yml` relative to the
repository root.
...
| Key | Description |
| --- | --- |
| `path` | Optional. The path `west init` should clone the manifest repository into, relative to the west workspace topdir.  If not given, the basename of the path component in the manifest repository URL will be used by default. For example, if the URL is `https://git.example.com/project-repo`, the manifest repository would be cloned to the directory `project-repo`. |
| `west-commands` | Optional. This is analogous to the same key in a project sequence element. |
| `import` | Optional. This is also analogous to the `projects` key, but allows importing projects from other files in the manifest repository. See [Manifest Imports](#west-manifest-import). |
...
The pykwalify schema `manifest-schema.yml` in the [west source code
repository](https://github.com/zephyrproject-rtos/west) is used to validate the manifest section.
...
| `"0.8"` | Support for `import: path-prefix:` ([Option 3: Mapping](#west-manifest-import-map)) |
| `"0.9"` | **Use of west v0.9.x is discouraged**.  This schema version is provided to allow users to explicitly request compatibility with west [v0.9.0](release-notes.md#west-0-9-0). However, west [v0.10.0](release-notes.md#west-0-10-0) and later have incompatible behavior for features that were introduced in west v0.9.0. You should ignore version “0.9” if possible. |
| `"0.10"` | Support for:   - `submodules:` in `projects:` ([Git Submodules in Projects](#west-manifest-submodules)) - `manifest: group-filter:`, and `groups:` in `projects:`   ([Project Groups](#west-manifest-groups)) - The `import:` feature now supports `allowlist:` and   `blocklist:`; these are respectively recommended as replacements for   older names as part of a general Zephyr-wide inclusive language   change. The older key names are still supported for backwards   compatibility. ([Manifest Imports](#west-manifest-import),   [Option 3: Mapping](#west-manifest-import-map)) |
| `"0.12"` | Support for `userdata:` in `projects:` ([Repository user data](#west-project-userdata)) |
| `"0.13"` | Support for `self: userdata:` ([Repository user data](#west-project-userdata)) |
| `"1.0"` | Identical to `"0.13"`, but available for use by users that do not wish to use a `"0.x"` version field. |
| `"1.2"` | Support for `description:` in `projects:` ([Projects](#west-manifests-projects)) |
...
See [Project Groups](#west-manifest-groups).

## [Active and Inactive Projects](#id19)
...
[Manifest Imports](#west-manifest-import) in an inactive project will be ignored by west.
...
1. Using the `manifest.project-filter` configuration option. If a project is
   ...
   See the entry for this option in [Built-in Configuration Options](config.md#west-config-index) for details.
...
[above](#west-manifest-files) to place projects into groups, and to
...
[Enabled and Disabled Project Groups](#west-enabled-disabled-groups). There are some basic examples in
[Project Group Examples](#west-project-group-examples). Finally, [Group Filters and Imports](#west-group-filter-imports)
provides a simplified overview of how `group-filter` interacts with the
[Manifest Imports](#west-manifest-import) feature.
...
### [Enabled and Disabled Project Groups](#id22)

All project groups are enabled by default. You can enable or disable groups in
both your manifest file and [Configuration](config.md#west-config).
...
[Group Filters and Imports](#west-group-filter-imports) for more information.
...
### [Group Filters and Imports](#id24)

This section provides a simplified description of how the `manifest:
group-filter:` value behaves when combined with [Manifest Imports](#west-manifest-import).
For complete details, see [Manifest Import Details](#west-manifest-formal).
...
The `unstable` group is disabled in `child/west.yml`, and that is not
overridden in `parent/west.yml`. Therefore, the final `group-filter`
...
The `[-unstable]` group filter in `child/west.yml` is overridden in
`parent/west.yml`, so the `unstable` group is enabled. Since
`project-1` and `project-3` are in the `unstable` group, they are active.

The same `parent/west.yml` file disables the `optional` group, so
`project-2` is inactive.

The final group filter specified by `parent/west.yml` is
`[+unstable,-optional]`.
...
The `-unstable` group filter in `child/west.yml` is overridden in the
...
The final group filter specified by `parent/west.yml` and the
`manifest.group-filter` configuration option is `[+unstable,-optional]`.

## [Git Submodules in Projects](#id25)

You can use the `submodules` keys briefly described [above](#west-manifest-files) to force `west update` to also handle any [Git
submodules](https://git-scm.com/book/en/v2/Git-Tools-Submodules) configured in project’s git repository. The `submodules` key can
...
### [Option 2: List of mappings](#id27)
...
accessible to programs using [West APIs](west-apis.md#west-apis) as the `userdata` attribute of
...
## [Manifest Imports](#id29)

You can use the `import` key briefly described above to include projects from
other manifest files in your `west.yml`. This key can be either a
...
containing your `west.yml`. You can use a “project: … import:” to load
...
other files. For example, a project named `foo` in your `west.yml`
...
- [Boolean](#west-manifest-import-bool)
  :   - [Example 1.1: Downstream of a Zephyr release](#west-manifest-ex1-1)
      - [Example 1.2: “Rolling release” Zephyr downstream](#west-manifest-ex1-2)
      - [Example 1.3: Downstream of a Zephyr release, with module fork](#west-manifest-ex1-3)
- [Relative path](#west-manifest-import-path)
  :   - [Example 2.1: Downstream of a Zephyr release with explicit path](#west-manifest-ex2-1)
      - [Example 2.2: Downstream with directory of manifest files](#west-manifest-ex2-2)
      - [Example 2.3: Continuous Integration overrides](#west-manifest-ex2-3)
- [Mapping with additional configuration](#west-manifest-import-map)
  :   - [Example 3.1: Downstream with name allowlist](#west-manifest-ex3-1)
      - [Example 3.2: Downstream with path allowlist](#west-manifest-ex3-2)
      - [Example 3.3: Downstream with path blocklist](#west-manifest-ex3-3)
      - [Example 3.4: Import into a subdirectory](#west-manifest-ex3-4)
- [Sequence of paths and mappings](#west-manifest-import-seq)
  :   - [Example 4.1: Downstream with sequence of manifest files](#west-manifest-ex4-1)
      - [Example 4.2: Import order illustration](#west-manifest-ex4-2)

A more [formal description](#west-manifest-formal) of how this works is
last, after the examples.

### [Troubleshooting Note](#id30)

If you’re using this feature and find west’s behavior confusing, try
[resolving your manifest](#west-manifest-resolve) to see the final results
after imports are done.

### [Option 1: Boolean](#id31)

This is the easiest way to use `import`.

If `import` is `true` as a `projects` attribute, west imports projects
from the `west.yml` file in that project’s root directory. If it’s
`false` or missing, it has no effect. For example, this manifest would import
`west.yml` from the `p1` git repository at revision `v1.0`:
...
repository’s `west.yml` at revision `v1.14.1` will be cloned into
`my-downstream` as well.

You can add and commit any code to `my-repo` you please at this point,
including your own Zephyr applications, drivers, etc. See [Application Development](../application/index.md#application).

#### Example 1.2: “Rolling release” Zephyr downstream

This is similar to [Example 1.1: Downstream of a Zephyr release](#west-manifest-ex1-1), except we’ll use `revision:
...
This time, whenever you run `west update`, the special [manifest-rev](workspaces.md#west-manifest-rev) branch in the `zephyr` repository will be updated to
...
The contents of `zephyr/west.yml` at the new `manifest-rev` will then
...
repository’s working tree. See [Example 2.2: Downstream with directory of manifest files](#west-manifest-ex2-2) for an example.

#### Example 1.3: Downstream of a Zephyr release, with module fork

This manifest is similar to the one in [Example 1.1: Downstream of a Zephyr release](#west-manifest-ex1-1), except it:

- is a downstream of Zephyr 2.0
- includes a downstream fork of the `modules/hal/nordic`
  [module](../modules.md#modules) which was included in that release
...
`my-repo/west.yml` when defining `hal_nordic` there. The value from
`zephyr/west.yml` is ignored entirely. See [Resolving Manifests](#west-manifest-resolve)
...
- the contents of `project-1/west.yml` at `manifest-rev`, which points
  at tag `v1.0` after running `west update`
- any YAML files in the directory tree `project-2/p2-manifests`
  at the latest commit in the `main` branch, as fetched by `west update`,
  sorted by file name
- YAML files in `submanifests` in your manifest repository,
...
The setting `import: west.yml` means to use the file `west.yml` inside
...
You want to add all the files in `my-repo/submanifests` to the main
manifest file, `my-repo/west.yml`, in addition to projects in
`zephyr/west.yml`. You want to track the latest development code
...
The `.yml` file names are prefixed with numbers in this example to
...
Notice how the manifests in `submanifests` are imported *before*
`my-repo/west.yml` and `zephyr/west.yml`. In general, an `import`
in the `self` section is processed before the manifest files in `projects`
and the main manifest file.

This means projects defined in `my-repo/submanifests` take highest
precedence. For example, if `01-libraries.yml` defines `hal_nordic`,
the project by the same name in `zephyr/west.yml` is simply ignored. As
usual, see [Resolving Manifests](#west-manifest-resolve) for troubleshooting advice.
...
Starting with [Example 2.2: Downstream with directory of manifest files](#west-manifest-ex2-2), the CI scripts add a
file `00-ci.yml` in `my-repo/submanifests`, with these contents:
...
`my-repo/submanifests`. The projects defined in `00-ci.yml` have
higher precedence than other definitions in `my-repo/submanifests`,
because the name `00-ci.yml` comes before the other file names.
...
- `file`: Optional. The name of the manifest file or directory to import.
  This defaults to `west.yml` if not present.
...
- `path-allowlist`: Optional. If present, a path or sequence of project paths
  ...
  implemented with [pathlib](https://docs.python.org/3/library/pathlib.html#pathlib.PurePath.match). Note that this means case sensitivity is
  platform specific.
...
projects, however. We’ll assume the mainline `west.yml` is
...
subdirectory of your [west workspace](../../glossary.md#term-west-workspace).
...
three project repositories in an `external-code` subdirectory, like this:
...
[Example 2.2: Downstream with directory of manifest files](#west-manifest-ex2-2), with a sequence of explicitly named files.
...
1. the listed files in `my-repo/submanifests` are first, in the order
   they occur (e.g. `libraries.yml` comes before
   `applications.yml`, since this is a sequence of files), since the
   `self: import:` is always imported first
2. `my-repo/west.yml` is next (with projects `my-library` etc. as long
   as they weren’t already defined somewhere in `submanifests`)
3. `zephyr/west.yml` is after that, since that’s the first `import` key
   in the `projects` list in `my-repo/west.yml`
4. files in `another-manifest-repo/submanifests` are last (sorted by file
   name), since that’s the final project `import`

### [Manifest Import Details](#id35)
...
because the top-level `west.yml` has already defined a project by that
name.
...
`manifest-rev` in the `baz` project if `baz/west.yml` defines `P`,
...
from `baz/west.yml`, which “should” cause `west update P` to fail with an
...
documentation for the [update.fetch](config.md#west-config-index) configuration
...
### [Resolving Manifests](#id37)

The `--resolve` action outputs a single manifest file equivalent to your
current manifest and all its [imported manifests](#west-manifest-import):
