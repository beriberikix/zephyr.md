---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/develop/test/twister_statuses.html
original_path: develop/test/twister_statuses.html
---
...
`Harness``TestCase``TestSuite``TestInstance`
...
More detailed explanation can be found [here](twister.md#twister-tests-long-version).

## Possible Twister Statuses

Twister Statuses

| In-code | In-text | Description | Suite | Case |
| --- | --- | --- | --- | --- |
| FILTER | filtered | A static or runtime filter has eliminated the test from the list of tests to use. | ✓ | ✓ |
| NOTRUN | not run | The test wasn’t run because it was not runnable in current configuration. It was, however, built correctly. | ✓ | ✓ |
| BLOCK | blocked | The test was not run because of an error or crash in the test suite. | ✕ | ✓ |
| SKIP | skipped | Test was skipped by some other reason than previously delineated. | ✓ | ✓ |
| ERROR | error | The test produced an error in running the test itself. | ✓ | ✓ |
| FAIL | failed | The test produced results different than expected. | ✓ | ✓ |
| PASS | passed | The test produced results as expected. | ✓ | ✓ |
...
| ↓ Case\Suite → | FILTER | ERROR | FAIL | PASS | NOTRUN | SKIP |
| --- | --- | --- | --- | --- | --- | --- |
| FILTER | ✓ | ✕ | ✕ | ✕ | ✕ | ✕ |
| ERROR | ✕ | ✓ | ✕ | ✕ | ✕ | ✕ |
| BLOCK | ✕ | ✓ | ✓ | ✕ | ✕ | ✕ |
