---
version: v4.0.0
source_url: https://docs.zephyrproject.org/4.0.0/kernel/code-relocation.html
original_path: kernel/code-relocation.html
---
...
and file are given to the [scripts/build/gen\_relocate\_app.py](../build/cmake/index.md#gen-relocate-app-py) script in the form
...
The memory region and file are given to the [scripts/build/gen\_relocate\_app.py](../build/cmake/index.md#gen-relocate-app-py) script in the form of a string.
...
Kconfig [`CONFIG_CODE_DATA_RELOCATION`](../kconfig.md#CONFIG_CODE_DATA_RELOCATION "CONFIG_CODE_DATA_RELOCATION") option, when enabled in
...
- Enable [`CONFIG_CODE_DATA_RELOCATION`](../kconfig.md#CONFIG_CODE_DATA_RELOCATION "CONFIG_CODE_DATA_RELOCATION") in the `prj.conf` file
- Inside the `CMakeLists.txt` file in the project, mention
  ...
  function zephyr\_code\_relocate() can be called as many times as required.
...
- if the memory type is appended with \_DATA, \_TEXT, \_RODATA or \_BSS, only the
  ...
  ```
- Multiple regions can also be appended together such as: SRAM2\_DATA\_BSS.
...
A sample showcasing the NOCOPY flag is provided here: [Code relocation nocopy](../samples/application_development/code_relocation_nocopy/README.md#code_relocation_nocopy "Relocate code, data, or bss sections using a custom linker script.").
