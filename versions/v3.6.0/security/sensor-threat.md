---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/security/sensor-threat.html
original_path: security/sensor-threat.html
---
...
![../_images/sensor-model.svg](https://docs.zephyrproject.org/3.6.0/_images/sensor-model.svg)
...
as this seems to be in wide use [[1]](#id23).
...
1. **The bootloader**. This is a small code/data image contained in
   ...
   programmed into the device, early in production [[th-imboot]](#th-imboot).
2. **The application firmware image**. This asset consists of the
   ...
   1. The image shall only be replaced with an authorized image
      [[th-authrepl]](#th-authrepl).
   2. When an authorized replacement image is available, the update
      shall be done in a timely manner [[th-timely-update]](#th-timely-update).
   3. The image update shall be seen as atomic, meaning that when the
      image is run, the flash shall contain either the update image in
      its entirety, or the old image in its entirety
      [[th-atomic-update]](#th-atomic-update).
3. **Root certificate list**. In order to authenticate the cloud service
   ...
   [[th-root-certs]](#th-root-certs), [[th-root-check]](#th-root-check).
4. **Client secrets**. To authenticate the client to the service, the
   ...
   of code necessary shall have access to them. [[th-secret-storage]](#th-secret-storage)
5. **Current date/time**. TLS certificate verification requires
   ...
   certificate to be able to intercept this. [[th-time]](#th-time)
...
7. **Device configuration**. Various configuration data, such as the
   ...
   should be allowed only from authorized parties. [[th-conf]](#th-conf)
8. **Logs**. In order to assist with analysis of security issues, the
   ...
   1. **Firmware image updates**. The system should log the download of
      new images, and when an image is successfully updated.
   2. **Client secret changes**. Changes and new client secrets should be
      logged.
   3. **Changes to the device configuration**.
...
1. **Flash contents**. The flash device contains several regions. The
   ...
   1. **The bootloader**. As described in the Assets section, the
   ...
   2. **The application image**. The application image contains the code
      ...
      image.
   3. **The update image**. This is an area of flash that holds a new
   ...
   4. **Secret storage**. An area of the flash will be used to store
   ...
   5. **Configuration storage**. There shall be an area to store other
      ...
      scrutinized.
   6. **Log storage**. The device may have an area of flash where log
...
3. **Communication with cloud service**. Communication between the
   ...
   service [[th-all-tls]](#th-all-tls). The TLS stack shall be configured to use only cipher suites
   that are generally considered secure [[2]](#id24), including forward
   secrecy. The communication shall be secured by the following:

   1. **Cipher suite selection**. The device shall only allow
      communication with generally agreed secure cipher suites
      [[th-tls-ciphers]](#th-tls-ciphers).
   2. **Server certificate verification**. The server presented by the
      server shall be verified [[th-root-check]](#th-root-check).

      1. **Naming**. The certificate shall name the host and service
         the cloud service server is providing.
         [RFC6125](https://tools.ietf.org/html/rfc6125) describes
      ...
      2. **Path validation**. The device shall verify that the
         ...
         [RFC4158](https://tools.ietf.org/html/rfc4158) describes
      ...
      3. **Validity period**. The validity period of all presented
         certificates shall be checked against the device’s best
         notion of the current time.
   3. **Client authentication**. The client shall authenticate itself to
      ...
      [[th-tls-client-auth]](#th-tls-client-auth).

      1. **TLS client certificates**. The TLS protocol allows the
      ...
      2. **Token-based authentication**. It is also possible for the
         ...
         protocol, such as
         [RFC7519](https://tools.ietf.org/html/rfc7519)‘s JSON Web
   ...
   4. **Random/Entropy source**. Cryptographic communication requires the
      ...
      information on testing a device’s entropy source [[th-entropy]](#th-entropy).
4. **Communication with the time service**. Ideally, the device shall
   ...
   [RFC4330](https://tools.ietf.org/html/rfc4330) and referenced RFCs
...
5. **Device lifecycle**. An IoT device will have a lifecycle from
   ...
   1. **Initial provisioning**. During the initial provisioning stage,
      ...
      [[th-initial-secret]](#th-initial-secret).
   2. **Normal operation**. Normal operation includes the behavior
      described by the rest of this document.
   3. **Re-provisioning**. Sometimes it is necessary to re-provision a
      ...
      [[th-reprovision]](#th-reprovision).
   4. **Destruction**. To prevent the device secret from being used to
      spoof the device, upon decommissioning, the secret for a
      particular device shall be rendered ineffective
      [[th-destruction]](#th-destruction). Possibilities include:

      1. Hardware destruction of the device.
      2. Securely wiping the flash area containing the
         secret [[3]](#id25).
      3. Removing the device identity and certificate from the
         service.
...
Application image shall only be replaced with an
authorized image.

[[th-timely-update](#id4)]

Application updates shall be done in a timely manner.

[[th-atomic-update](#id5)]
...
See [https://www.slideshare.net/kartben/iot-developer-survey-2018](https://www.slideshare.net/kartben/iot-developer-survey-2018). As
...
change.
Organizations such as [https://www.ssllabs.com/](https://www.ssllabs.com/) provide information on
