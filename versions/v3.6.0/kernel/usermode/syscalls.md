---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/kernel/usermode/syscalls.html
...
example of [`k_sem_init()`](../services/synchronization/semaphores.md#c.k_sem_init "k_sem_init"), this API is declared in
...
Inside this header is the body of [`k_sem_init()`](../services/synchronization/semaphores.md#c.k_sem_init "k_sem_init"):
...
[![System Call execution flow](https://docs.zephyrproject.org/3.6.0/_images/syscall_flow.png)](https://docs.zephyrproject.org/3.6.0/_images/syscall_flow.png)
...
- `K_SYSCALL_OBJ()` Checks a memory address to assert that it is
...
- `K_SYSCALL_OBJ_INIT()` is the same as
  `K_SYSCALL_OBJ()`, except that the provided object may be
  uninitialized. This is useful for verifiers of object init functions.
- `K_SYSCALL_OBJ_NEVER_INIT()` is the same as
  `K_SYSCALL_OBJ()`, except that the provided object must be
  uninitialized. This is not used very often, currently only for
  [`k_thread_create()`](../services/threads/index.md#c.k_thread_create "k_thread_create").
- `K_SYSCALL_MEMORY_READ()` validates a memory buffer of a particular
  size. The calling thread must have read permissions on the entire buffer.
- `K_SYSCALL_MEMORY_WRITE()` is the same as
  `K_SYSCALL_MEMORY_READ()` but the calling thread must additionally
  have write permissions.
- `K_SYSCALL_MEMORY_ARRAY_READ()` validates an array whose total size
  ...
  on the total size.
- `K_SYSCALL_MEMORY_ARRAY_WRITE()` is the same as
  `K_SYSCALL_MEMORY_ARRAY_READ()` but the calling thread must
  additionally have write permissions.
- `K_SYSCALL_VERIFY_MSG()` does a runtime check of some boolean
  expression which must evaluate to true otherwise the check will fail.
  A variant `K_SYSCALL_VERIFY` exists which does not take
...
- `K_SYSCALL_DRIVER_OP()` checks at runtime if a driver
...
- `K_SYSCALL_SPECIFIC_DRIVER()` is a runtime check to verify that
...
`K_OOPS()` can be used to induce a kernel oops which will kill the
...
`k_usermode_to_copy()` and `k_usermode_from_copy()` APIs exist for
this purpose.
...
the implementation function, and then used `k_usermode_to_copy()` to fill
...
simply invoke `K_OOPS()` which kills the calling thread. The current
...
2. Any invalid access to memory found by the set of `K_SYSCALL_MEMORY` APIs,
   `k_usermode_from_copy()`, `k_usermode_to_copy()`
   should trigger a `K_OOPS`. This happens when the caller doesn’t have
...
3. Most system calls take kernel object pointers as an argument, checked either
   ...
   manually using `k_object_validate()`. These can fail for a variety
...
6. It is totally forbidden for any kind of kernel mode callback function to
   ...
   must enforce that these are NULL and should invoke `K_OOPS()` if
   not.
...
- [`k_thread_join()`](../services/threads/index.md#c.k_thread_join "k_thread_join") and [`k_thread_abort()`](../services/threads/index.md#c.k_thread_abort "k_thread_abort") are no-ops if
...
- [`k_thread_create()`](../services/threads/index.md#c.k_thread_create "k_thread_create") invokes `K_OOPS()` for parameter
  checks, due to a great deal of existing code ignoring the return value.
  This will also be addressed by #23030.
- [`k_thread_abort()`](../services/threads/index.md#c.k_thread_abort "k_thread_abort") invokes `K_OOPS()` if an essential
  thread is aborted, as the function has no return value.
- Various system calls related to logging invoke `K_OOPS()`
