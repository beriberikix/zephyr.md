---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/kernel/services/interrupts.html
original_path: kernel/services/interrupts.html
---
...
An *interrupt service routine* (ISR) is a function that executes
...
An IDT or a vector table is used
...
provides the [`k_is_in_isr()`](#c.k_is_in_isr) API to allow the routine to
alter its behavior depending on whether it is executing as part of
a thread or as part of an ISR.

### [Multi-level Interrupt Handling](#id2)
...
If nested interrupt controllers are supported, [`CONFIG_MULTI_LEVEL_INTERRUPTS`](../../kconfig.md#CONFIG_MULTI_LEVEL_INTERRUPTS "CONFIG_MULTI_LEVEL_INTERRUPTS")
should be enabled, and [`CONFIG_2ND_LEVEL_INTERRUPTS`](../../kconfig.md#CONFIG_2ND_LEVEL_INTERRUPTS "CONFIG_2ND_LEVEL_INTERRUPTS") and
[`CONFIG_3RD_LEVEL_INTERRUPTS`](../../kconfig.md#CONFIG_3RD_LEVEL_INTERRUPTS "CONFIG_3RD_LEVEL_INTERRUPTS") configured as well, based on the
...
occur. Thread B will not run until the next [reschedule point](scheduling/index.md#scheduling-v2) reached after releasing the IRQ lock.
...
[`CONFIG_ZERO_LATENCY_IRQS`](../../kconfig.md#CONFIG_ZERO_LATENCY_IRQS "CONFIG_ZERO_LATENCY_IRQS") to be enabled. In addition to that, the
flag `IRQ_ZERO_LATENCY` must be passed to [`IRQ_CONNECT`](#c.IRQ_CONNECT) or
[`IRQ_DIRECT_CONNECT`](#c.IRQ_DIRECT_CONNECT) macros to configure the particular interrupt
with zero latency.
...
- An ISR can instruct the system workqueue thread to execute a work item.
  (See [Workqueue Threads](threads/workqueue.md#workqueues-v2).)
...
the shared\_irq interrupt controller), which doesn’t scale very well.

To solve this problem, one may use shared interrupts, which can be enabled using
[`CONFIG_SHARED_INTERRUPTS`](../../kconfig.md#CONFIG_SHARED_INTERRUPTS "CONFIG_SHARED_INTERRUPTS"). Whenever an attempt to register
a second ISR/argument pair on the same interrupt line is made (using
[`IRQ_CONNECT`](#c.IRQ_CONNECT) or [`irq_connect_dynamic()`](#c.irq_connect_dynamic)), the interrupt line will
...
controlled by [`CONFIG_SHARED_IRQ_MAX_NUM_CLIENTS`](../../kconfig.md#CONFIG_SHARED_IRQ_MAX_NUM_CLIENTS "CONFIG_SHARED_IRQ_MAX_NUM_CLIENTS").

Interrupt sharing is transparent to the user. As such, the user may register
interrupts using [`IRQ_CONNECT`](#c.IRQ_CONNECT) and [`irq_connect_dynamic()`](#c.irq_connect_dynamic) as
...
allow users to dynamically disconnect ISRs using [`irq_disconnect_dynamic()`](#c.irq_disconnect_dynamic).
...
Please note that enabling [`CONFIG_SHARED_INTERRUPTS`](../../kconfig.md#CONFIG_SHARED_INTERRUPTS "CONFIG_SHARED_INTERRUPTS") will
result in a non-negligible increase in the binary size. Use with caution.

## [Implementation](#id6)

### [Defining a regular ISR](#id7)

An ISR is defined at runtime by calling [`IRQ_CONNECT`](#c.IRQ_CONNECT). It must
then be enabled by calling [`irq_enable()`](#c.irq_enable).

Important

IRQ\_CONNECT() is not a C function and does some inline assembly magic
...
Since the [`IRQ_CONNECT`](#c.IRQ_CONNECT) macro requires that all its parameters be
known at build time, in some cases this may not be acceptable. It is also
possible to install interrupts at runtime with
[`irq_connect_dynamic()`](#c.irq_connect_dynamic). It is used in exactly the same way as
[`IRQ_CONNECT`](#c.IRQ_CONNECT):
...
Dynamic interrupts require the [`CONFIG_DYNAMIC_INTERRUPTS`](../../kconfig.md#CONFIG_DYNAMIC_INTERRUPTS "CONFIG_DYNAMIC_INTERRUPTS") option to
be enabled. Removing or re-configuring a dynamic interrupt is currently
unsupported.

### [Defining a ‘direct’ ISR](#id8)
...
[`IRQ_DIRECT_CONNECT`](#c.IRQ_DIRECT_CONNECT). These direct interrupts have some special
implementation requirements and a reduced feature set; see the definition
of [`IRQ_DIRECT_CONNECT`](#c.IRQ_DIRECT_CONNECT) for details.
...
The same restrictions regarding [`IRQ_CONNECT`](#c.IRQ_CONNECT) described in [Defining a regular ISR](#defining-a-regular-isr)
are applicable here. If [`CONFIG_SHARED_INTERRUPTS`](../../kconfig.md#CONFIG_SHARED_INTERRUPTS "CONFIG_SHARED_INTERRUPTS") is disabled, the above
...
If [`CONFIG_SHARED_IRQ_MAX_NUM_CLIENTS`](../../kconfig.md#CONFIG_SHARED_IRQ_MAX_NUM_CLIENTS "CONFIG_SHARED_IRQ_MAX_NUM_CLIENTS") is set to a value lower than 2
(current number of clients), a build error will be generated.

If dynamic interrupts are enabled, [`irq_connect_dynamic()`](#c.irq_connect_dynamic) will allow sharing interrupts
during runtime. Exceeding the configured maximum number of allowed clients will result in
a failed assertion.

### [Dynamically disconnecting an ISR](#id10)
...
The [`irq_disconnect_dynamic()`](#c.irq_disconnect_dynamic) call will result in interrupt 24 becoming
unshared, meaning the system will act as if the first [`IRQ_CONNECT`](#c.IRQ_CONNECT)
call never happened. This behaviour is only allowed if
[`CONFIG_DYNAMIC_INTERRUPTS`](../../kconfig.md#CONFIG_DYNAMIC_INTERRUPTS "CONFIG_DYNAMIC_INTERRUPTS") is enabled, otherwise a linker
error will be generated.

### [Implementation Details](#id11)
...
covered in the [x86 Details](#x86-details) section below.

The invocation of [`IRQ_CONNECT`](#c.IRQ_CONNECT) will declare an instance of
struct \_isr\_list wich is placed in a special .intList section.
...
directly in the file where [`IRQ_CONNECT`](#c.IRQ_CONNECT) is used.
...
Any invocation of [`IRQ_CONNECT`](#c.IRQ_CONNECT) will declare an instance of
struct \_isr\_list which is placed in a special .intList section:
...
This data consisting of the header and instances of struct \_isr\_list inside
`${ZEPHYR_PREBUILT_EXECUTABLE}`.elf is then used by the
gen\_isr\_tables.py script to generate a C file defining a vector table and
...
[`IRQ_CONNECT`](#c.IRQ_CONNECT) also has a runtime component which programs the desired
...
A vector table is generated when [`CONFIG_GEN_IRQ_VECTOR_TABLE`](../../kconfig.md#CONFIG_GEN_IRQ_VECTOR_TABLE "CONFIG_GEN_IRQ_VECTOR_TABLE") is
...
1. For ‘direct’ interrupts declared with [`IRQ_DIRECT_CONNECT`](#c.IRQ_DIRECT_CONNECT), the
   handler function will be placed here.
2. For regular interrupts declared with [`IRQ_CONNECT`](#c.IRQ_CONNECT), the address
   ...
   software ISR table.
3. For interrupt lines that are not configured at all, the address of the
...
which case the [`CONFIG_GEN_IRQ_VECTOR_TABLE`](../../kconfig.md#CONFIG_GEN_IRQ_VECTOR_TABLE "CONFIG_GEN_IRQ_VECTOR_TABLE") option should be
disabled.
...
CONFIG\_GEN\_IRQ\_START\_VECTOR needs to be set to properly offset the indices
in the table.

##### SW ISR Table

This is an array of struct \_isr\_table\_entry:
...
This is an array of struct z\_shared\_isr\_table\_entry:
...
lines. Whenever an interrupt line becomes shared, `z_shared_isr()` will
replace the currently registered ISR in \_sw\_isr\_table. This special ISR will
...
interrupt vectors are created locally in place of invocation of [`IRQ_CONNECT`](#c.IRQ_CONNECT) macro.
Then automatically generated linker scripts are used to place it in the right place in the memory.

This option requires enabling by the choose of `ISR_TABLES_LOCAL_DECLARATION`.
If this configuration is supported by the used architecture and toolchaing the
`ISR_TABLES_LOCAL_DECLARATION_SUPPORTED` is set.
See defails of this option for the information about currently supported configurations.

Any invocation of [`IRQ_CONNECT`](#c.IRQ_CONNECT) or IRQ\_DIRECT\_CONNECT will declare an instance of struct
\_isr\_list\_sname which is placde in a special .intList section:
...
Beside of the \_isr\_list\_sname the [`IRQ_CONNECT`](#c.IRQ_CONNECT) macro generates an entry
...
This array is placed in a section with the name saved in \_isr\_list\_sname structure.

The values created by [`IRQ_DIRECT_CONNECT`](#c.IRQ_DIRECT_CONNECT) macro depends on the architecture.
...
Simillar like for [`IRQ_CONNECT`](#c.IRQ_CONNECT), the created variable or function is placed
in a section, saved in \_isr\_list\_sname section.

##### Files generated by the script

The interrupt tables generator script creates 3 files:
isr\_tables.c, isr\_tables\_swi.ld, and isr\_tables\_vt.ld.

The isr\_tables.c will contain all the structures for interrupts, direct interrupts and
...
Then two linker files are used. The isr\_tables\_vt.ld file is included in place
where the interrupt vectors are required to be placed in the selected architecture.
The isr\_tables\_swi.ld file describes the placement of the software interrupt table
...
[arch/x86/gen\_idt.py](../../build/cmake/index.md#gen-idt-py) tool uses the .intList section to create it. However, on APIC-based
...
48-63, and so forth. When the [arch/x86/gen\_idt.py](../../build/cmake/index.md#gen-idt-py) tool is constructing the IDT, when it
...
indexed by IRQ line is not used. Instead, the [`IRQ_CONNECT`](#c.IRQ_CONNECT) call
creates a small assembly language function which calls the common interrupt
code in `_interrupt_enter()` with the ISR and parameter as arguments.
It is the address of this assembly interrupt stub which gets placed in the IDT.
For interrupts declared with [`IRQ_DIRECT_CONNECT`](#c.IRQ_DIRECT_CONNECT) the parameterless
...
runtime what vector is associated with an IRQ line. [arch/x86/gen\_idt.py](../../build/cmake/index.md#gen-idt-py) additionally
creates an \_irq\_to\_interrupt\_vector array which maps an IRQ line to its
configured vector in the IDT. This is used at runtime by [`IRQ_CONNECT`](#c.IRQ_CONNECT)
...
by the [`CONFIG_X86_DYNAMIC_IRQ_STUBS`](../../kconfig.md#CONFIG_X86_DYNAMIC_IRQ_STUBS "CONFIG_X86_DYNAMIC_IRQ_STUBS") option. Each stub pushes an
...
default 8 in Kconfig.multilevel by setting [`CONFIG_1ST_LEVEL_INTERRUPT_BITS`](../../kconfig.md#CONFIG_1ST_LEVEL_INTERRUPT_BITS "CONFIG_1ST_LEVEL_INTERRUPT_BITS")
for the first level, [`CONFIG_2ND_LEVEL_INTERRUPT_BITS`](../../kconfig.md#CONFIG_2ND_LEVEL_INTERRUPT_BITS "CONFIG_2ND_LEVEL_INTERRUPT_BITS") for the second level and
[`CONFIG_3RD_LEVEL_INTERRUPT_BITS`](../../kconfig.md#CONFIG_3RD_LEVEL_INTERRUPT_BITS "CONFIG_3RD_LEVEL_INTERRUPT_BITS") for the third level. These masks control the
...
this can be found in `irq_multilevel.h`
...
should be handed off to a thread. See [Offloading ISR Work](#offloading-isr-work) for
a description of various techniques that can be used in an application.

## [Configuration Options](#id13)
...
:   Defines
    ...
    IRQ\_CONNECT(irq\_p, priority\_p, isr\_p, isr\_param\_p, flags\_p)
    ...
    :   Initialize a ‘direct’ interrupt handler.
        ...
        - Scheduling decisions are now optional, controlled by the return value of ISRs implemented with the [ISR\_DIRECT\_DECLARE()](#group__isr__apis_1gaf86866cd07fd37f381d98866f8874ebf) macro
        - The call into the OS to exit power management idle state is now optional. Normal interrupts always do this before the ISR is run, but when it runs is now controlled by the placement of a [ISR\_DIRECT\_PM()](#group__isr__apis_1ga3c1327551dfca7818975e3fbf1470227) macro, or omitted entirely.
    ...
    ISR\_DIRECT\_DECLARE(name)
    :   Helper macro to declare a direct interrupt service routine.

        This will declare the function in a proper way and automatically include the [ISR\_DIRECT\_FOOTER()](#group__isr__apis_1ga31581157c9dbacf935f0e6a8dd456335) and [ISR\_DIRECT\_HEADER()](#group__isr__apis_1ga1ab99dbeb50b228001e1fca808cbaeea) macros. The function should return nonzero status if a scheduling decision should potentially be made. See [ISR\_DIRECT\_FOOTER()](#group__isr__apis_1ga31581157c9dbacf935f0e6a8dd456335) for more details on the scheduling decision.
    ...
    :   Lock interrupts.
        ...
        This routine disables all interrupts on the CPU. It returns an unsigned integer “lock-out key”, which is an architecture-dependent indicator of whether interrupts were locked prior to the call. The lock-out key must be passed to [irq\_unlock()](#group__isr__apis_1ga646045943b3b2a130738bcc48867bf57) to re-enable interrupts.

        This routine can be called recursively, as long as the caller keeps track of each lock-out key that is generated. Interrupts are re-enabled by passing each of the keys to [irq\_unlock()](#group__isr__apis_1ga646045943b3b2a130738bcc48867bf57) in the reverse order they were acquired. (That is, each call to [irq\_lock()](#group__isr__apis_1ga19fdde73c3b02fcca6cf1d1e67631228) must be balanced by a corresponding call to [irq\_unlock()](#group__isr__apis_1ga646045943b3b2a130738bcc48867bf57).)
    ...
    :   Unlock interrupts.
        ...
        This routine reverses the effect of a previous call to [irq\_lock()](#group__isr__apis_1ga19fdde73c3b02fcca6cf1d1e67631228) using the associated lock-out key. The caller must call the routine once for each time it called [irq\_lock()](#group__isr__apis_1ga19fdde73c3b02fcca6cf1d1e67631228), supplying the keys in the reverse order they were acquired, before interrupts are enabled.
        ...
        Parameters:
        :   - **key** – Lock-out key generated by [irq\_lock()](#group__isr__apis_1ga19fdde73c3b02fcca6cf1d1e67631228).
    ...
    static inline int irq\_connect\_dynamic(unsigned int irq, unsigned int priority, void (\*routine)(const void \*parameter), const void \*parameter, uint32\_t flags)
    :   Configure a dynamic interrupt.

        Use this instead of [IRQ\_CONNECT()](#group__isr__apis_1ga131739d1faf501a15590053817aba984) if arguments cannot be known at build time.
    ...
    static inline int irq\_disconnect\_dynamic(unsigned int irq, unsigned int priority, void (\*routine)(const void \*parameter), const void \*parameter, uint32\_t flags)
    ...
    static inline bool k\_is\_pre\_kernel(void)
