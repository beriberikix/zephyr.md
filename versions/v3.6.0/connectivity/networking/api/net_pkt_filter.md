---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/connectivity/networking/api/net_pkt_filter.html
...
*group* net\_pkt\_filter
:   Network Packet Filter API.

    Defines

    npf\_insert\_send\_rule(rule)

    npf\_insert\_recv\_rule(rule)

    npf\_append\_send\_rule(rule)

    npf\_append\_recv\_rule(rule)

    npf\_remove\_send\_rule(rule)

    npf\_remove\_recv\_rule(rule)

    npf\_remove\_all\_send\_rules()

    npf\_remove\_all\_recv\_rules()
...
:   Defines
    ...
    :   Statically define a “source address match” packet filter condition.
        ...
        Parameters:
        :   - **\_name** – Name of the condition
            - **\_addr\_array** – Array of `struct net_eth_addr` items to test against
