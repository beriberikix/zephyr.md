---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/develop/test/twister.html
...
shows for every test how the test is run (qemu, native\_sim, etc.) or
...
1. The format of the test identifier shall be a string without any spaces or
   special characters (allowed characters: alphanumeric and [\_=]) consisting
   of multiple sections delimited with a dot (.).
2. Each test identifier shall start with a section followed by a subsection
   separated by a dot. For example, a test that covers semaphores in the kernel
   shall start with `kernel.semaphore`.
3. All test identifiers within a testcase.yaml file need to be unique. For
...
4. Depending on the nature of the test, an identifier can consist of at least
   two sections:
...
>
> robot\_test\_path: <robot file path> (default empty)
...
:   [Snippets](../../build/snippets/index.md#snippets) are supported in twister for test cases that
...
(`twister_discard.csv`) can help show why particular test cases were
skipped.
...
- `timeout` option in each test case. See [here](#twister-test-case-timeout) for more
  details.
...
In this example we are using a reel\_board and an nrf52840dk\_nrf52840:
...
![../../_images/fixtures.svg](https://docs.zephyrproject.org/3.6.0/_images/fixtures.svg)
...
- Option to specify your own list of default platforms overriding what
  upstream defines.
- Ability to override build\_on\_all options used in some testcases.
...
- override\_default\_platforms: override default key a platform sets in board
  ...
  False by default.
- increased\_platform\_scope: This option is set to True by default, when
...
- default\_platforms: A list of additional default platforms to add. This list
  can either be used to replace the existing default platforms or can extend it
  depending on the value of override\_default\_platforms.
