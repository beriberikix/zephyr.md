---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/develop/west/extensions.html
original_path: develop/west/extensions.html
---
...
[build, flash, and debug](build-flash-debug.md#west-build-flash-debug) and the
[ones described here](zephyr-cmds.md#west-zephyr-ext-cmds) , are extensions. That’s why
...
See `zephyr/scripts/west-commands.yml` and the
...
[configuration](config.md#west-config) option to `false`. To set this
...
If you want to, you can then re-enable them in a particular [west
workspace](../../glossary.md#term-west-workspace) with:
...
3. Make sure the `west-commands.yml` file is referenced in the
   [west manifest](../../glossary.md#term-west-manifest).
...
Requires” information on the [west PyPI page](https://pypi.org/project/west/) for details on the currently
supported versions of Python). You can put it in anywhere in any project
tracked by your [west manifest](../../glossary.md#term-west-manifest), or the manifest repository itself.
...
details on the west APIs you can use, see [West APIs](west-apis.md#west-apis).
...
### Step 2: Add or Update Your `west-commands.yml`

You now need to add a `west-commands.yml` file to your project which
...
This is because west won’t import `my_west_extension.py` until the user
...
multiple files, your `west-commands.yml` will look something like this:
...
- `my_west_extension.py` defines extension `my-command-name`
...
See the file `west-commands-schema.yml` in the [west repository](https://github.com/zephyrproject-rtos/west/) for a
schema describing the contents of a `west-commands.yml`.

### Step 3: Update Your Manifest

Finally, you need to specify the location of the `west-commands.yml` you
...
Where `path/to/west-commands.yml` is relative to the root of the project.
Note that the name `west-commands.yml`, while encouraged, is just a
