---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/develop/west/built-in.html
original_path: develop/west/built-in.html
---
...
The new workspace is created in the given `directory`, creating a new
`.west` inside this directory. You can give the manifest URL using
...
would clone the upstream official zephyr repository into `zp/zephyr`,
and check out the `v1.14.0` release. This command creates
`zp/.west`, and set the `manifest.path` [configuration option](config.md#west-config) to `zephyr` to record the location of the manifest
...
This creates `.west` **next to** `directory` in the file
...
`manifest.file` using [west config](config.md#west-config-cmd) after running `west init`.
...
`west.yml`, and updates each project specified there.
If your manifest uses [project groups](manifest.md#west-manifest-groups), then
...
1. Initializes a local Git repository for the project in the workspace, if
   it does not already exist
2. Inspects the project’s `revision` field in the manifest, and fetches
   it from the remote if it is not already available locally
3. Sets the project’s [manifest-rev](workspaces.md#west-manifest-rev) branch to the
   commit specified by the revision in the previous step
4. Checks out `manifest-rev` in the local working copy as a [detached
   HEAD](https://git-scm.com/docs/git-checkout#_detached_head)
5. If the manifest file specifies a [submodules](manifest.md#west-manifest-submodules) key for the project, recursively updates
...
or set the `update.fetch` [configuration option](config.md#west-config) to
...
See [Project Groups](manifest.md#west-manifest-groups) for details on the project group feature.

The `west update` command behaves as if the `--group-filter` option’s
value were appended to the `manifest.group-filter`
[configuration option](config.md#west-config-index).
...
`update.sync-submodules` [Configuration](config.md#west-config) option is false.
...
- `west manifest`: manage the manifest file. See [Manifest Command](manifest.md#west-manifest-cmd).
...
- `west config`: get or set [configuration options](config.md#west-config)
...
- `west help`: get help about a command, or print information about all
  commands in the workspace, including [Extensions](extensions.md#west-extensions)
