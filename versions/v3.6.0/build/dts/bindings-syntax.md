---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/build/dts/bindings-syntax.html
original_path: build/dts/bindings-syntax.html
---
...
files are YAML files. A [simple example](bindings-intro.md#dt-bindings-simple-example) was
...
[Introduction to Devicetree Bindings](bindings-intro.md#dt-binding-compat). It should look like this in a binding file:
...
binding is used. The [on-bus:](#dt-bindings-on-bus) key can be used to
refine the search.
...
[dts/bindings/vendor-prefixes.txt](https://github.com/zephyrproject-rtos/zephyr/blob/main/dts/bindings/vendor-prefixes.txt) for a list of accepted vendor
...
[`gpio-leds`](api/bindings/led/gpio-leds.md#std-dtcompatible-gpio-leds) compatible which is commonly used to describe board
...
source code from these macros in [Devicetree access from C/C++](api-usage.md#dt-from-c). Generally speaking, the
...
standard properties, like [reg](intro-syntax-structure.md#dt-important-props), whose meaning is
defined by the devicetree specification. This happens regardless of whether the
node has a matching binding or not.

### [Property entry syntax](#id5)
...
### [Example property definitions](#id6)
...
available. See [Writing property values](intro-syntax-structure.md#dt-writing-property-values) for more details about writing
values of each type in a DTS file. See [Phandles](phandles.md#dt-phandles) for more information
about the `phandle*` type properties.

| Type | Description | Example in DTS |
| --- | --- | --- |
| `string` | exactly one string | `status = "disabled";` |
...
[Test Runner (Twister)](../../develop/test/twister.md#twister-script) for upstream pull requests.)
...
[Rules for default values](bindings-upstream.md#dt-bindings-default-rules).

See [Example property definitions](#dt-bindings-example-properties) for examples. Putting `default:` on
any property type besides those used in [Example property definitions](#dt-bindings-example-properties)
will raise an error.
...
is raised. See [Example property definitions](#dt-bindings-example-properties) for examples.
...
bus-sensitive logic ignores `manufacturer,sensor-i2c.yaml` when searching
for a binding for `sensor@0`.

## [Specifier cell names (\*-cells)](#id17)
...
[phandle-array properties](phandles.md#dt-phandle-arrays).
...
like [`DT_PWMS_CHANNEL_BY_NAME`](api/api.md#c.DT_PWMS_CHANNEL_BY_NAME "DT_PWMS_CHANNEL_BY_NAME").
...
If any file named `foo.yaml` is found (see
[Where bindings are located](bindings-intro.md#dt-where-bindings-are-located) for the search process), it will be
...
a [property definition](#dt-bindings-properties) for which the included
...
The file [base.yaml ](https://github.com/zephyrproject-rtos/zephyr/blob/main/dts/bindings/base/base.yaml) contains
definitions for many common properties. When writing a new binding, it is a
good idea to check if `base.yaml` already defines some of the needed
properties, and include it if it does.

Note that you can make a property defined in base.yaml obligatory like this,
taking [reg](intro-syntax-structure.md#dt-important-props) as an example:
...
This includes the files `foo.yaml` and `bar.yaml`. (You can
