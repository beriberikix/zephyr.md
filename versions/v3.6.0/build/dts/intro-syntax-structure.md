---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/build/dts/intro-syntax-structure.html
original_path: build/dts/intro-syntax-structure.html
---
...
See [Writing property values](#dt-writing-property-values) below for more example property values.

## [Devicetrees reflect hardware](#id5)
...
![representation of a board with three I2C peripherals](https://docs.zephyrproject.org/3.6.0/_images/zephyr_dt_i2c_high_level.png)
...
![../../_images/zephyr_dt_i2c_example.png](https://docs.zephyrproject.org/3.6.0/_images/zephyr_dt_i2c_example.png)
...
:   The name of the hardware device the node represents.
    ...
    [dts/bindings/vendor-prefixes.txt](https://github.com/zephyrproject-rtos/zephyr/blob/main/dts/bindings/vendor-prefixes.txt) contains a list of commonly
    ...
    [bindings](bindings.md#dt-bindings) for the node. Device drivers use
...
:   Information about interrupts generated by the device, encoded as an array
    ...
    [Devicetree Specification release v0.3](https://www.devicetree.org/specifications/) for more details.
...
which is distinct from the standard [node label](#dt-node-labels).
...
## [Writing property values](#id9)

This section describes how to write property values in DTS format. The property
types in the table below are described in detail in [Devicetree bindings](bindings.md#dt-bindings).
...
- The values in the `phandle`, `phandles`, and `phandle-array` types are
  described further in [Phandles](phandles.md#dt-phandles)
...
- Property values refer to other nodes in the devicetree by their *phandles*.
  You can write a phandle using `&foo`, where `foo` is a [node label](#dt-node-labels). Here is an example devicetree fragment:
  ...
  1. The `device@0` node’s phandle, which is written here as `&foo` since
  ...
  [Devicetree access from C/C++](api-usage.md#dt-from-c).
...
## [Aliases and chosen nodes](#id10)

There are two additional ways beyond [node labels](#dt-node-labels) to
...
example, [Blinky](../../samples/basic/blinky/README.md#blinky "Blink an LED forever using the GPIO API.") uses this to abstract the LED to blink via the
`led0` alias.

The `/chosen` node’s properties are used to configure system- or
subsystem-wide values. See [Chosen nodes](api/api.md#devicetree-chosen-nodes) for more information.
