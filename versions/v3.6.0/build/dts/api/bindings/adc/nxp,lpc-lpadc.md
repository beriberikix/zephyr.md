---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/build/dts/api/bindings/adc/nxp,lpc-lpadc.html
...
| `clk-source` | `int` | ```text source to attach the ADC clock to ```  This property is **required**. |
...
| `power-level` | `int` | ```text Power level selection. Corresponds to the value of register field CFG[PWRSEL] (see chip specific manual). ```  This property is **required**.  Legal values: `0`, `1`, `2`, `3` |
...
| `nxp,reference-supply` | `phandles` | ```text References to required regulators which must be enabled for LPADC to function ``` |
