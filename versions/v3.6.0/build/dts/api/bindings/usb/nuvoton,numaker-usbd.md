---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/build/dts/api/bindings/usb/nuvoton,numaker-usbd.html
original_path: build/dts/api/bindings/usb/nuvoton,numaker-usbd.html
---
...
| `dma-buffer-size` | `int` | ```text Size of DMA buffer in bytes ```  This property is **required**. |
| `disallow-iso-in-out-same-number` | `boolean` | ```text Some soc series don't allow Isochronous IN/OUT endpoints to be assigned the same numbers, for example, 0x82 (for Isochronous IN) and 0x02 (for Isochronous OUT) are disallowed. ``` |
...
may apply to the “nuvoton,numaker-usbd” compatible.
