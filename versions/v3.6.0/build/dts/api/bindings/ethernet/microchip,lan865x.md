---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/build/dts/api/bindings/ethernet/microchip,lan865x.html
original_path: build/dts/api/bindings/ethernet/microchip,lan865x.html
---
...
| Name | Type | Details |
| --- | --- | --- |
| `tx-cut-through-mode` | `boolean` | ```text Enable TX cut through mode ``` |
| `rx-cut-through-mode` | `boolean` | ```text Enable RX cut through mode ``` |
| `plca-enable` | `boolean` | ```text Enable or disable PLCA support ``` |
| `plca-node-id` | `int` | ```text Specify the PLCA node ID number ``` |
| `plca-node-count` | `int` | ```text Specify the PLCA node count ``` |
| `plca-burst-count` | `int` | ```text Specify the PLCA burst count ``` |
| `plca-burst-timer` | `int` | ```text Specify the PLCA burst timer value ``` |
| `plca-to-timer` | `int` | ```text Specify the PLCA to timer value ``` |
| `int-gpios` | `phandle-array` | ```text The interrupt pin of LAN865X is active low. If connected directly the MCU pin should be configured as active low. ```  This property is **required**. |
| `rst-gpios` | `phandle-array` | ```text The reset pin of LAN865X. ```  This property is **required**. |
...
may apply to the “microchip,lan865x” compatible.
