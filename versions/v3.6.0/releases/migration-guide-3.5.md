---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/releases/migration-guide-3.5.html
...
- The kernel [`k_mem_slab_free()`](../kernel/memory_management/slabs.md#c.k_mem_slab_free "k_mem_slab_free") function has changed its signature, now
  ...
  the slab.
- [`CONTAINER_OF`](../kernel/util/index.md#c.CONTAINER_OF "CONTAINER_OF") now performs type checking, this was very commonly
  misused to obtain user structure from [`k_work`](../kernel/services/threads/workqueue.md#c.k_work "k_work") pointers without
  passing from [`k_work_delayable`](../kernel/services/threads/workqueue.md#c.k_work_delayable "k_work_delayable"). This would now result in a build
  error and have to be done correctly using
  [`k_work_delayable_from_work()`](../kernel/services/threads/workqueue.md#c.k_work_delayable_from_work "k_work_delayable_from_work").
...
- STM32 Ethernet driver was misusing [`hwinfo_get_device_id()`](../hardware/peripherals/hwinfo.md#c.hwinfo_get_device_id "hwinfo_get_device_id") to generate
...
- The CAN controller timing API functions [`can_set_timing()`](../hardware/peripherals/can/controller.md#c.can_set_timing "can_set_timing") and [`can_set_timing_data()`](../hardware/peripherals/can/controller.md#c.can_set_timing_data "can_set_timing_data")
  ...
  the `sjw` field in [`can_timing`](../hardware/peripherals/can/controller.md#c.can_timing "can_timing"). To aid in this, the [`can_calc_timing()`](../hardware/peripherals/can/controller.md#c.can_calc_timing "can_calc_timing") and
  [`can_calc_timing_data()`](../hardware/peripherals/can/controller.md#c.can_calc_timing_data "can_calc_timing_data") functions now automatically calculate a suitable SJW. The
  calculated SJW can be overwritten by the caller if needed. The CAN controller API functions
  [`can_set_bitrate()`](../hardware/peripherals/can/controller.md#c.can_set_bitrate "can_set_bitrate") and [`can_set_bitrate_data()`](../hardware/peripherals/can/controller.md#c.can_set_bitrate_data "can_set_bitrate_data") now also automatically calculate a
  suitable SJW, but their SJW cannot be overwritten by the caller.
- The CAN ISO-TP message configuration in [`isotp_msg_id`](../connectivity/canbus/isotp.md#c.isotp_msg_id "isotp_msg_id") is changed to use the following
  flags instead of bit fields:

  - [`ISOTP_MSG_EXT_ADDR`](../connectivity/canbus/isotp.md#c.ISOTP_MSG_EXT_ADDR "ISOTP_MSG_EXT_ADDR") to enable ISO-TP extended addressing
  - [`ISOTP_MSG_FIXED_ADDR`](../connectivity/canbus/isotp.md#c.ISOTP_MSG_FIXED_ADDR "ISOTP_MSG_FIXED_ADDR") to enable ISO-TP fixed addressing
  - [`ISOTP_MSG_IDE`](../connectivity/canbus/isotp.md#c.ISOTP_MSG_IDE "ISOTP_MSG_IDE") to use extended (29-bit) CAN IDs

  The two new flags [`ISOTP_MSG_FDF`](../connectivity/canbus/isotp.md#c.ISOTP_MSG_FDF "ISOTP_MSG_FDF") and [`ISOTP_MSG_BRS`](../connectivity/canbus/isotp.md#c.ISOTP_MSG_BRS "ISOTP_MSG_BRS") were added for CAN FD
  mode.
...
- Multiple platforms no longer support powering the system off using
  [`pm_state_force()`](../services/pm/api/index.md#c.pm_state_force "pm_state_force"). The new [`sys_poweroff()`](../services/poweroff.md#c.sys_poweroff "sys_poweroff") API must be used.
...
- The [`CONFIG_BOOTLOADER_SRAM_SIZE`](../kconfig.md#CONFIG_BOOTLOADER_SRAM_SIZE "CONFIG_BOOTLOADER_SRAM_SIZE") default value is now `0` (was
...
- The `accept()` callback’s signature in [`bt_l2cap_server`](../connectivity/bluetooth/api/l2cap.md#c.bt_l2cap_server "bt_l2cap_server") has
  changed to `int (*accept)(struct bt_conn *conn, struct bt_l2cap_server
  *server, struct bt_l2cap_chan **chan)`,
  adding a new `server` parameter pointing to the [`bt_l2cap_server`](../connectivity/bluetooth/api/l2cap.md#c.bt_l2cap_server "bt_l2cap_server")
...
- MCUmgr SMP version 2 error codes entry has changed due to a collision with an
  ...
  `smp_add_cmd_err()` should be used instead, `MGMT_CB_ERROR_RET` is
  now deprecated and [`MGMT_CB_ERROR_ERR`](../services/device_mgmt/mcumgr_callbacks.md#c.mgmt_cb_return.MGMT_CB_ERROR_ERR "MGMT_CB_ERROR_ERR") should be used instead.
...
- MCUmgr SMP version 2 error translation (to legacy MCUmgr error code) is now
  ...
  pointer of [`mgmt_group`](../services/device_mgmt/mcumgr.md#c.mgmt_group "mgmt_group") when registering a group. See
...
- Nordic nRF based boards using [`CONFIG_GPIO_AS_PINRESET`](../kconfig.md#CONFIG_GPIO_AS_PINRESET "CONFIG_GPIO_AS_PINRESET")
  ...
  ```
- The [`CONFIG_MODEM_GSM_PPP`](../kconfig.md#CONFIG_MODEM_GSM_PPP "CONFIG_MODEM_GSM_PPP") modem driver is obsolete.
  Instead the new [`CONFIG_MODEM_CELLULAR`](../kconfig.md#CONFIG_MODEM_CELLULAR "CONFIG_MODEM_CELLULAR") driver should be used.
  As part of this [`CONFIG_GSM_MUX`](../kconfig.md#CONFIG_GSM_MUX "CONFIG_GSM_MUX") and [`CONFIG_UART_MUX`](../kconfig.md#CONFIG_UART_MUX "CONFIG_UART_MUX") are being
