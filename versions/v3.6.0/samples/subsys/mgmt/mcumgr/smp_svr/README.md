---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/samples/subsys/mgmt/mcumgr/smp_svr/README.html
...
`mcumgr` installed. Follow the instructions in the [Command-line Tool](../../../../../services/device_mgmt/mcumgr.md#mcumgr-cli) section
...
Detailed instructions can be found in the [MCUboot](../../../../../services/device_mgmt/dfu.md#mcuboot) documentation page.
...
uploaded and run on a target. To sign images, the MCUboot tool `imgtool` can be used.
...
The above command creates an image file called `zephyr.signed.bin` in the
build directory.

For more information on image signing and `west sign`, see the [Signing Binaries](../../../../../develop/west/sign.md#west-sign)
documentation.

## Flashing the sample image

Upload the `zephyr.signed.bin` file from the previous to image slot-0 of your
board. See [Flash map](../../../../../services/storage/flash_map/flash_map.md#flash-map-api) for details on flash partitioning.
...
The `mcumgr` command-line tool requires a connection string in order
...
the `--conntype <type>` and `--connstring=<string>` `mcumgr` parameters.
...
- Build an MCUboot enabled application, see [Building the sample application](#smp-svr-sample-build)
- Sign the application image, see [Signing the sample image](#smp-svr-sample-sign)
...
image and getting the hash by using the `image list` command of `mcumgr`.
...
`mcumgr`. This is useful with FS log backend, when files are stored in
non-volatile memory. Build and flash both MCUboot and smp\_svr applications and
then use `mcumgr` with `download` command, e.g.:
