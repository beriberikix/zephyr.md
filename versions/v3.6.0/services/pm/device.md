---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/services/pm/device.html
...
> - [System Power Management](#pm-device-system-pm)
...
power states quickly when `pm_system_suspend()` gets called. This is
...
APIs **must** check whether they can do so with [`k_can_yield()`](../../kernel/services/threads/index.md#c.k_can_yield "k_can_yield").
...
[Device Runtime Power Management](#pm-device-runtime-pm) is the **preferred**
...
[`CONFIG_PM_DEVICE_RUNTIME_EXCLUSIVE`](../../kconfig.md#CONFIG_PM_DEVICE_RUNTIME_EXCLUSIVE "CONFIG_PM_DEVICE_RUNTIME_EXCLUSIVE") is set to `y` (that is
the default value when [`CONFIG_PM_DEVICE_RUNTIME`](../../kconfig.md#CONFIG_PM_DEVICE_RUNTIME "CONFIG_PM_DEVICE_RUNTIME") is enabled)
...
[`pm_device_state`](api/index.md#c.pm_device_state "pm_device_state"). This type is used to track power states of
...
to handle device actions([`pm_device_action`](api/index.md#c.pm_device_action "pm_device_action")) which change device state.

Each [`pm_device_action`](api/index.md#c.pm_device_action "pm_device_action") have a direct an unambiguous relationship with
a [`pm_device_state`](api/index.md#c.pm_device_state "pm_device_state").
...
Use [`PM_DEVICE_DEFINE`](api/index.md#c.PM_DEVICE_DEFINE "PM_DEVICE_DEFINE") or [`PM_DEVICE_DT_DEFINE`](api/index.md#c.PM_DEVICE_DT_DEFINE "PM_DEVICE_DT_DEFINE") to define the
...
Drivers can use [`PM_DEVICE_GET`](api/index.md#c.PM_DEVICE_GET "PM_DEVICE_GET") or
[`PM_DEVICE_DT_GET`](api/index.md#c.PM_DEVICE_DT_GET "PM_DEVICE_DT_GET") to get a pointer to this state. These
pointers should be passed to `DEVICE_DEFINE` or `DEVICE_DT_DEFINE`
to initialize the power management field in each [`device`](../../kernel/drivers/index.md#c.device "device").
...
subsystem can suspend devices, as described in [System Power Management](#pm-device-system-pm). This
...
When [`pm_device_busy_set()`](api/index.md#c.pm_device_busy_set "pm_device_busy_set") is called, the device is marked as busy and
...
[`pm_device_busy_clear()`](api/index.md#c.pm_device_busy_clear "pm_device_busy_clear").
...
[`pm_device_wakeup_enable()`](api/index.md#c.pm_device_wakeup_enable "pm_device_wakeup_enable").
...
later calling [`pm_device_wakeup_enable()`](api/index.md#c.pm_device_wakeup_enable "pm_device_wakeup_enable").
...
using it. For more details, see [Power Domain](power_domain.md#pm-power-domain).
