---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/services/sensing/index.html
...
devices can use the Zephyr [Sensors](../../hardware/peripherals/sensor.md#sensor-api) API directly.
...
![Unified Zephyr sensing architecture.](https://docs.zephyrproject.org/3.6.0/_images/sensing_solution.png)
...
![Sensing subsystem API organization.](https://docs.zephyrproject.org/3.6.0/_images/sensing_api_org.png)
...
![Sensor Configuration Flow (App set report interval to hinge angel sensor example).](https://docs.zephyrproject.org/3.6.0/_images/sensor_config_flow.png)

- Sensor Data Flow

![Sensor Data Flow (App receive hinge angel data through data event callback example).](https://docs.zephyrproject.org/3.6.0/_images/sensor_data_flow.png)
...
![Sensor Reporting Topology.](https://docs.zephyrproject.org/3.6.0/_images/sensor_top.png)
...
*group* sensing\_sensor
:   Sensing Sensor API .

    Defines

    SENSING\_SENSOR\_INFO\_NAME(node, idx)

    SENSING\_SENSOR\_INFO\_DEFINE(node, idx)

    SENSING\_CONNECTIONS\_NAME(node)

    SENSING\_SENSOR\_SOURCE\_NAME(idx, node)

    SENSING\_SENSOR\_SOURCE\_EXTERN(idx, node)

    SENSING\_CONNECTION\_INITIALIZER(source\_name, cb\_list\_ptr)

    SENSING\_CONNECTION\_DEFINE(idx, node, cb\_list\_ptr)

    SENSING\_CONNECTIONS\_DEFINE(node, num, cb\_list\_ptr)

    SENSING\_SUBMIT\_CFG\_NAME(node, idx)

    SENSING\_SENSOR\_IODEV\_NAME(node, idx)

    SENSING\_SENSOR\_IODEV\_DEFINE(node, idx)

    SENSING\_SENSOR\_NAME(node, idx)

    SENSING\_SENSOR\_DEFINE(node, prop, idx, reg\_ptr, cb\_list\_ptr)
    ...
    :   Like [SENSOR\_DEVICE\_DT\_DEFINE()](../../hardware/peripherals/sensor.md#group__sensor__interface_1gaa67ca630e3931a0c3821ba438c86690c) with sensing specifics.
    ...
    :   *Values:*
        ...
        enumerator EVENT\_CONFIG\_READY

    enum [anonymous]
    :   *Values:*

        enumerator SENSOR\_LATER\_CFG\_BIT
    ...
    struct sensing\_submit\_config
    :   *#include <sensing\_sensor.h>*
