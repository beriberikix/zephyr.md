---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/services/zbus/index.html
...
[![zbus usage overview](https://docs.zephyrproject.org/3.6.0/_images/zbus_overview.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_overview.svg)
...
[![ZBus anatomy](https://docs.zephyrproject.org/3.6.0/_images/zbus_anatomy.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_anatomy.svg)
...
[![ZBus observers type](https://docs.zephyrproject.org/3.6.0/_images/zbus_type_of_observers.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_type_of_observers.svg)
...
[`ZBUS_CHAN_DEFINE`](#c.ZBUS_CHAN_DEFINE) or [`ZBUS_CHAN_ADD_OBS`](#c.ZBUS_CHAN_ADD_OBS). There are also runtime observers,
...
[![ZBus observation mask.](https://docs.zephyrproject.org/3.6.0/_images/zbus_observation_mask.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_observation_mask.svg)
...
[![ZBus sensor-based application](https://docs.zephyrproject.org/3.6.0/_images/zbus_operations.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_operations.svg)
...
- The event dispatcher logic executes the listeners, sends a copy of the message to the message
  ...
  quick access to the constant message reference directly (via the [`zbus_chan_const_msg()`](#c.zbus_chan_const_msg)
...
[![ZBus example scenario](https://docs.zephyrproject.org/3.6.0/_images/zbus_publishing_process_example_scenario1.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_publishing_process_example_scenario1.svg)
...
[![ZBus publish processing detail](https://docs.zephyrproject.org/3.6.0/_images/zbus_publishing_process_example.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_publishing_process_example.svg)
...
| d, e | The VDED executes L1 and L2 in the respective sequence. Inside the listeners, usually, there is a call to the [`zbus_chan_const_msg()`](#c.zbus_chan_const_msg) function, which provides a direct constant reference to channel A’s message. It is quick, and no copy is needed here. |
...
[![ZBus publish processing detail](https://docs.zephyrproject.org/3.6.0/_images/zbus_publishing_process_example2.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_publishing_process_example2.svg)
...
[![ZBus publishing process details using priority boost.](https://docs.zephyrproject.org/3.6.0/_images/zbus_publishing_process_example_HLP.svg)](https://docs.zephyrproject.org/3.6.0/_images/zbus_publishing_process_example_HLP.svg)
...
On the above code, the [`zbus_obs_attach_to_thread()`](#c.zbus_obs_attach_to_thread) will set the `s1` observer with
priority two as the thread has that priority. It is possible to reverse that by detaching the
observer using the [`zbus_obs_detach_from_thread()`](#c.zbus_obs_detach_from_thread). Only enabled observers and observations
...
ZBus uses [include/zephyr/net/buf.h](https://github.com/zephyrproject-rtos/zephyr/blob/main/include/zephyr/net/buf.h) (network buffers) to exchange data with message
...
1. Observers defined in a channel using the [`ZBUS_CHAN_DEFINE`](#c.ZBUS_CHAN_DEFINE) (following the definition
   sequence);
2. Observers defined using the [`ZBUS_CHAN_ADD_OBS`](#c.ZBUS_CHAN_ADD_OBS) based on the sequence priority
   (parameter of the macro);
3. The latest is the runtime observers in the addition sequence using the
   [`zbus_chan_add_obs()`](#c.zbus_chan_add_obs).
...
It is possible to add static observers to a channel using the [`ZBUS_CHAN_ADD_OBS`](#c.ZBUS_CHAN_ADD_OBS). We call
...
Messages are published to a channel in zbus by calling [`zbus_chan_pub()`](#c.zbus_chan_pub). For example, the
...
Only use this function inside an ISR with a [`K_NO_WAIT`](../../kernel/services/timing/clocks.md#c.K_NO_WAIT "K_NO_WAIT") timeout.

### [Reading from a channel](#id21)

Messages are read from a channel in zbus by calling [`zbus_chan_read()`](#c.zbus_chan_read). So, for example, the
...
Choose the timeout of [`zbus_chan_read()`](#c.zbus_chan_read) after receiving a notification from
[`zbus_sub_wait()`](#c.zbus_sub_wait) carefully because the channel will always be unavailable during the VDED
...
It is possible to force zbus to notify a channel’s observers by calling [`zbus_chan_notify()`](#c.zbus_chan_notify).
...
declare them by calling [`ZBUS_CHAN_DECLARE`](#c.ZBUS_CHAN_DECLARE) and [`ZBUS_OBS_DECLARE`](#c.ZBUS_OBS_DECLARE). In other
...
observers, for which there are supporting APIs like [`zbus_iterate_over_channels()`](#c.zbus_iterate_over_channels),
[`zbus_iterate_over_channels_with_user_data()`](#c.zbus_iterate_over_channels_with_user_data), [`zbus_iterate_over_observers()`](#c.zbus_iterate_over_observers) and
[`zbus_iterate_over_observers_with_user_data()`](#c.zbus_iterate_over_observers_with_user_data). This feature enables developers to call a
procedure over all declared channels, where the procedure parameter is a [`zbus_channel`](#c.zbus_channel).
...
[`zbus_chan_const_msg()`](#c.zbus_chan_const_msg) because the channel passed as an argument to the listener function is
...
To take more control over channels, two functions were added [`zbus_chan_claim()`](#c.zbus_chan_claim) and
[`zbus_chan_finish()`](#c.zbus_chan_finish). With these functions, it is possible to access the channel’s metadata
...
- [`CONFIG_ZBUS_CHANNELS_SYS_INIT_PRIORITY`](../../kconfig.md#CONFIG_ZBUS_CHANNELS_SYS_INIT_PRIORITY "CONFIG_ZBUS_CHANNELS_SYS_INIT_PRIORITY") determine the `SYS_INIT`
...
[Confirmed channel](../../samples/subsys/zbus/confirmed_channel/README.md#zbus-confirmed-channel "Use confirmed zbus channels to ensure all subscribers consume a message.")
...
[Message subscriber](../../samples/subsys/zbus/msg_subscriber/README.md#zbus-msg-subscriber "Use zbus message subscribers to listen to messages published to channels.")
...
[zbus Priority Boost](../../samples/subsys/zbus/priority_boost/README.md#zbus-priority-boost "Illustrates zbus priority boost feature with a priority inversion scenario.")
...
:   Zbus API.
    ...
    ZBUS\_CHAN\_ADD\_OBS\_WITH\_MASK(\_chan, \_obs, \_masked, \_prio)
    ...
    ZBUS\_CHAN\_ADD\_OBS(\_chan, \_obs, \_prio)
    :   Add a static channel observervation.
    ...
    ZBUS\_CHAN\_DEFINE(\_name, \_type, \_validator, \_user\_data, \_observers, \_init\_val)
    :   Zbus channel definition.
        ...
        struct [zbus\_channel](#structzbus__channel)
    ...
    ZBUS\_MSG\_INIT(\_val, ...)
    :   Initialize a message.

        This macro initializes a message by passing the values to initialize the message struct or union.

        Parameters:
        :   - **\_val** – **[in]** Variadic with the initial values. `ZBUS_INIT(0)` means `{0}`, as ZBUS\_INIT(.a=10, .b=30) means `{.a=10, .b=30}`.

    ZBUS\_SUBSCRIBER\_DEFINE\_WITH\_ENABLE(\_name, \_queue\_size, \_enable)
    :   Define and initialize a subscriber.

        This macro defines an observer of subscriber type. It defines a message queue where the subscriber will receive the notification asynchronously, and initialize the `struct [zbus_observer](#structzbus__observer)` defining the subscriber.

        Parameters:
        :   - **\_name** – **[in]** The subscriber’s name.
            - **\_queue\_size** – **[in]** The notification queue’s size.
            - **\_enable** – **[in]** The subscriber initial enable state.

    ZBUS\_SUBSCRIBER\_DEFINE(\_name, \_queue\_size)
    :   Define and initialize a subscriber.

        This macro defines an observer of subscriber type. It defines a message queue where the subscriber will receive the notification asynchronously, and initialize the `struct [zbus_observer](#structzbus__observer)` defining the subscriber. The subscribers are defined in the enabled state with this macro.

        Parameters:
        :   - **\_name** – **[in]** The subscriber’s name.
            - **\_queue\_size** – **[in]** The notification queue’s size.

    ZBUS\_LISTENER\_DEFINE\_WITH\_ENABLE(\_name, \_cb, \_enable)
    :   Define and initialize a listener.

        This macro defines an observer of listener type. This macro establishes the callback where the listener will be notified synchronously, and initialize the `struct [zbus_observer](#structzbus__observer)` defining the listener.
    ...
    ZBUS\_LISTENER\_DEFINE(\_name, \_cb)
    :   Define and initialize a listener.

        This macro defines an observer of listener type. This macro establishes the callback where the listener will be notified synchronously and initialize the `struct [zbus_observer](#structzbus__observer)` defining the listener. The listeners are defined in the enabled state with this macro.

        Parameters:
        :   - **\_name** – **[in]** The listener’s name.
            - **\_cb** – **[in]** The callback function.

    ZBUS\_MSG\_SUBSCRIBER\_DEFINE\_WITH\_ENABLE(\_name, \_enable)
    :   Define and initialize a message subscriber.

        This macro defines an observer of [ZBUS\_OBSERVER\_SUBSCRIBER\_TYPE](#group__zbus__apis_1gga88941281d7bdd24f3cfbb53e57711d8fa22c784a85353545e8dca2ce7a0ec81b3) type. It defines a FIFO where the subscriber will receive the message asynchronously and initialize the [zbus\_observer](#structzbus__observer) defining the subscriber.

        Parameters:
        :   - **\_name** – **[in]** The subscriber’s name.
            - **\_enable** – **[in]** The subscriber’s initial state.

    ZBUS\_MSG\_SUBSCRIBER\_DEFINE(\_name)
    :   Define and initialize an enabled message subscriber.

        This macro defines an observer of message subscriber type. It defines a FIFO where the subscriber will receive the message asynchronously and initialize the [zbus\_observer](#structzbus__observer) defining the subscriber. The message subscribers are defined in the enabled state with this macro.
    ...
    :   Type used to represent an observer type.
        ...
        *Values:*
        ...
        enumerator ZBUS\_OBSERVER\_LISTENER\_TYPE

        enumerator ZBUS\_OBSERVER\_SUBSCRIBER\_TYPE

        enumerator ZBUS\_OBSERVER\_MSG\_SUBSCRIBER\_TYPE

    Functions

    int zbus\_chan\_pub(const struct [zbus\_channel](#c.zbus_channel) \*chan, const void \*msg, [k\_timeout\_t](../../kernel/services/timing/clocks.md#c.k_timeout_t "k_timeout_t") timeout)
    :   Publish to a channel.
        ...
        :   - **0** – Channel published.
            ...
            - **-EFAULT** – A parameter is incorrect, the notification could not be sent to one or more observer, or the function context is invalid (inside an ISR). The function only returns this value when the CONFIG\_ZBUS\_ASSERT\_MOCK is enabled.

    int zbus\_chan\_read(const struct [zbus\_channel](#c.zbus_channel) \*chan, void \*msg, [k\_timeout\_t](../../kernel/services/timing/clocks.md#c.k_timeout_t "k_timeout_t") timeout)
    :   Read a channel.
        ...
        :   - **chan** – **[in]** The channel’s reference.
            ...
            - **timeout** – **[in]** Waiting period to read the channel, or one of the special values K\_NO\_WAIT and K\_FOREVER.

        Return values:
        :   - **0** – Channel read.
            - **-EBUSY** – The channel is busy.
            - **-EAGAIN** – Waiting period timed out.
            - **-EFAULT** – A parameter is incorrect, or the function context is invalid (inside an ISR). The function only returns this value when the CONFIG\_ZBUS\_ASSERT\_MOCK is enabled.

    int zbus\_chan\_claim(const struct [zbus\_channel](#c.zbus_channel) \*chan, [k\_timeout\_t](../../kernel/services/timing/clocks.md#c.k_timeout_t "k_timeout_t") timeout)
    :   Claim a channel.
        ...
        Parameters:
        :   - **chan** – **[in]** The channel’s reference.
            - **timeout** – **[in]** Waiting period to claim the channel, or one of the special values K\_NO\_WAIT and K\_FOREVER.
    ...
    int zbus\_chan\_finish(const struct [zbus\_channel](#c.zbus_channel) \*chan)
    ...
    int zbus\_chan\_notify(const struct [zbus\_channel](#c.zbus_channel) \*chan, [k\_timeout\_t](../../kernel/services/timing/clocks.md#c.k_timeout_t "k_timeout_t") timeout)
    ...
    static inline const char \*zbus\_chan\_name(const struct [zbus\_channel](#c.zbus_channel) \*chan)
    ...
    static inline void \*zbus\_chan\_msg(const struct [zbus\_channel](#c.zbus_channel) \*chan)
    ...
    static inline const void \*zbus\_chan\_const\_msg(const struct [zbus\_channel](#c.zbus_channel) \*chan)
    ...
    static inline uint16\_t zbus\_chan\_msg\_size(const struct [zbus\_channel](#c.zbus_channel) \*chan)
    ...
    static inline void \*zbus\_chan\_user\_data(const struct [zbus\_channel](#c.zbus_channel) \*chan)
    ...
    int zbus\_chan\_add\_obs(const struct [zbus\_channel](#c.zbus_channel) \*chan, const struct [zbus\_observer](#c.zbus_observer) \*obs, [k\_timeout\_t](../../kernel/services/timing/clocks.md#c.k_timeout_t "k_timeout_t") timeout)
    ...
    int zbus\_chan\_rm\_obs(const struct [zbus\_channel](#c.zbus_channel) \*chan, const struct [zbus\_observer](#c.zbus_observer) \*obs, [k\_timeout\_t](../../kernel/services/timing/clocks.md#c.k_timeout_t "k_timeout_t") timeout)
    ...
    int zbus\_obs\_set\_enable(struct [zbus\_observer](#c.zbus_observer) \*obs, bool enabled)
    ...
    static inline int zbus\_obs\_is\_enabled(struct [zbus\_observer](#c.zbus_observer) \*obs, bool \*enable)
    ...
    int zbus\_obs\_set\_chan\_notification\_mask(const struct [zbus\_observer](#c.zbus_observer) \*obs, const struct [zbus\_channel](#c.zbus_channel) \*chan, bool masked)
    ...
    int zbus\_obs\_is\_chan\_notification\_masked(const struct [zbus\_observer](#c.zbus_observer) \*obs, const struct [zbus\_channel](#c.zbus_channel) \*chan, bool \*masked)
    ...
    static inline const char \*zbus\_obs\_name(const struct [zbus\_observer](#c.zbus_observer) \*obs)
    ...
    int zbus\_obs\_attach\_to\_thread(const struct [zbus\_observer](#c.zbus_observer) \*obs)
    ...
    int zbus\_obs\_detach\_from\_thread(const struct [zbus\_observer](#c.zbus_observer) \*obs)
    ...
    int zbus\_sub\_wait(const struct [zbus\_observer](#c.zbus_observer) \*sub, const struct [zbus\_channel](#c.zbus_channel) \*\*chan, [k\_timeout\_t](../../kernel/services/timing/clocks.md#c.k_timeout_t "k_timeout_t") timeout)
    :   Wait for a channel notification.
        ...
        :   - **sub** – **[in]** The subscriber’s reference.
            ...
            - **timeout** – **[in]** Waiting period for a notification arrival, or one of the special values K\_NO\_WAIT and K\_FOREVER.
    ...
    int zbus\_sub\_wait\_msg(const struct [zbus\_observer](#c.zbus_observer) \*sub, const struct [zbus\_channel](#c.zbus_channel) \*\*chan, void \*msg, [k\_timeout\_t](../../kernel/services/timing/clocks.md#c.k_timeout_t "k_timeout_t") timeout)
    :   Wait for a channel message.
        ...
        :   - **0** – Message received.
            - **-EINVAL** – The observer is not a subscriber.
            - **-ENOMSG** – Could not retrieve the [net\_buf](../../connectivity/networking/api/net_buf.md#structnet__buf) from the subscriber FIFO.
            - **-EILSEQ** – Received an invalid channel reference.
            - **-EFAULT** – A parameter is incorrect, or the function context is invalid (inside an ISR). The function only returns this value when the  [`CONFIG_ZBUS_ASSERT_MOCK`](../../kconfig.md#CONFIG_ZBUS_ASSERT_MOCK "CONFIG_ZBUS_ASSERT_MOCK") is enabled.

    bool zbus\_iterate\_over\_channels(bool (\*iterator\_func)(const struct [zbus\_channel](#c.zbus_channel) \*chan))
    :   Iterate over channels.
        ...
        Parameters:
        :   - **iterator\_func** – **[in]** The function that will be execute on each iteration.

        Return values:
        :   - **true** – Iterator executed for all channels.
            - **false** – Iterator could not be executed. Some iterate returned false.

    bool zbus\_iterate\_over\_channels\_with\_user\_data(bool (\*iterator\_func)(const struct [zbus\_channel](#c.zbus_channel) \*chan, void \*user\_data), void \*user\_data)
    :   Iterate over channels with user data.
        ...
        Parameters:
        :   - **iterator\_func** – **[in]** The function that will be execute on each iteration.
            - **user\_data** – **[in]** The user data that can be passed in the function.

        Return values:
        :   - **true** – Iterator executed for all channels.
            - **false** – Iterator could not be executed. Some iterate returned false.

    bool zbus\_iterate\_over\_observers(bool (\*iterator\_func)(const struct [zbus\_observer](#c.zbus_observer) \*obs))
    ...
    bool zbus\_iterate\_over\_observers\_with\_user\_data(bool (\*iterator\_func)(const struct [zbus\_observer](#c.zbus_observer) \*obs, void \*user\_data), void \*user\_data)
    ...
    struct zbus\_channel\_data
    :   *#include <zbus.h>*

        Type used to represent a channel mutable data.

        Every channel has a [zbus\_channel\_data](#structzbus__channel__data) structure associated.
    ...
    struct zbus\_channel
    :   *#include <zbus.h>*

        Type used to represent a channel.

        Every channel has a [zbus\_channel](#structzbus__channel) structure associated used to control the channel access and usage.
        ...
        struct [zbus\_channel\_data](#c.zbus_channel_data) \*const data
        :   Mutable channel data struct.

    struct zbus\_observer\_data
    :   *#include <zbus.h>*
    ...
    :   *#include <zbus.h>*
        ...
        [zbus\_obs\_set\_enable](#group__zbus__apis_1ga6b44a887c82cbceff3136bbd9e0aa91c) function to properly change the observer’s enabled field.
        ...
        enum [zbus\_observer\_type](#c.zbus_observer_type) type
        :   Type indication.

        struct [zbus\_observer\_data](#c.zbus_observer_data) \*const data
        :   Mutable observer data struct.

        struct [k\_msgq](../../kernel/services/data_passing/message_queues.md#c.k_msgq "k_msgq") \*const queue
        :   Observer message queue.

            It turns the observer into a subscriber.

        void (\*const callback)(const struct [zbus\_channel](#c.zbus_channel) \*chan)
        ...
        :   Observer message FIFO.
            ...
            It turns the observer into a message subscriber. It only exists if the  [`CONFIG_ZBUS_MSG_SUBSCRIBER`](../../kconfig.md#CONFIG_ZBUS_MSG_SUBSCRIBER "CONFIG_ZBUS_MSG_SUBSCRIBER") is enabled.
