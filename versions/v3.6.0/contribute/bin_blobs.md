---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/contribute/bin_blobs.html
original_path: contribute/bin_blobs.html
---
...
[upstream (vanilla) Zephyr](https://github.com/zephyrproject-rtos/zephyr).
...
Blobs are fetched from official third-party sources by the [west blobs](../develop/west/zephyr-cmds.md#west-blobs) command.

The blobs themselves must be specified in the [module.yml](../develop/modules.md#modules-bin-blobs) files included in separate Zephyr [module repositories](../develop/modules.md#modules) maintained by their respective vendors. This means that in order to
...
corresponding `module.yml` file. A specification for a blob must contain:

- An abstract description of the blob itself
- Version information
- A reference to vendor-provided documentation
- The blob’s [type](#bin-blobs-types), which must be one of the allowed types
...
See the [corresponding section](../develop/modules.md#modules-bin-blobs) for a more formal
definition of the fields.

The [west blobs](../develop/west/zephyr-cmds.md#west-blobs) command can be used to list metadata of
...
tainting originates in the [Linux kernel](https://www.kernel.org/doc/html/latest/admin-guide/tainted-kernels.html) and,
...
removed from `zephyr/west.yml` at any time due to lack of responsiveness or
...
[module update procedure](../develop/modules.md#modules-changes).
...
1. Make sure to read through the [Binary Blobs](#bin-blobs) section in
   detail, so that you are informed of the criteria used by the TSC in order to
   approve or reject a request
2. Use the [New Binary Blobs Issue](https://github.com/zephyrproject-rtos/zephyr/issues/new?assignees=&labels=RFC&template=008_bin-blobs.md&title=) to open an issue
