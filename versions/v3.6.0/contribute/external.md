---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/contribute/external.html
original_path: contribute/external.html
---
...
[Contributing External Tooling](#external-tooling) section at the end of the page.
...
context and approved by the [Zephyr governing board](https://www.zephyrproject.org/governance/), as described in the
[Zephyr project charter](https://www.zephyrproject.org/wp-content/uploads/sites/38/2020/09/CLEAN-LF-Zephyr-Charter-20200624-effective-20200901.pdf). The board will automatically reject licenses that
have not been approved by the [Open Source Initiative (OSI)](https://opensource.org/licenses/alphabetical). See the
[Submission and review process](#external-src-process) section for more details.

By carefully reviewing potential contributions and also enforcing a [Developer Certification of Origin (DCO)](guidelines.md#dco)
...
- The code is formatted according to the Zephyr [Coding Style](guidelines.md#coding-style)
- The code adheres to the project’s [Coding Guidelines](coding_guidelines/index.md#coding-guidelines)
...
- If the source is not Apache 2.0 licensed,
  an entry is added to the [licensing page](../LICENSING.md#zephyr-licensing).
...
repository, and then include it under the form of a [module](../develop/modules.md#modules).
...
1. Make sure to read through the [Contributing External Components](#external-contributions) section in
   detail, so that you are informed of the criteria used by the TSC and board in
   order to approve or reject a request
2. Use the [New External Source Code Issue](https://github.com/zephyrproject-rtos/zephyr/issues/new?assignees=&labels=RFC&template=007_ext-source.md&title=) to open an issue
3. Fill out all required sections, making sure you provide enough detail for the
...
4. Wait for feedback from the TSC, respond to any additional questions added as
...
1. The TSC chair will forward the link to the GitHub issue created during the
...
2. The Zephyr governing board has two weeks to review and ask questions:
   ...
   - If a governing board member raises an objection that cannot be resolved
     ...
     objections
3. On approval of the Zephyr TSC and governing board the submission process is
   complete

The flowchart below shows an overview of the process:

![../_images/ext-src-flowchart.svg](https://docs.zephyrproject.org/3.6.0/_images/ext-src-flowchart.svg)
...
[Open Source Initiative (OSI)](https://opensource.org/licenses/alphabetical).

Just like with regular external components, tooling that is imported from
another project can be integrated either in the main tree or as a [west
project](../develop/west/basics.md#west-workspace). Note that in this case the corresponding west project
will not be a [module](../develop/modules.md#modules), because tooling does not make use of the
Zephyr build system and does not need to be processed by it. Please see
[Modules vs west projects](../develop/modules.md#modules-vs-projects) for additional information on the differences.
...
[this section](../develop/west/manifest.md#west-manifest-groups).
...
the [main manifest file](https://github.com/zephyrproject-rtos/zephyr/blob/main/west.yml) requires TSC approval. This includes, but is not
limited to:
