---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/boards/xtensa/intel_adsp_cavs25/doc/intel_adsp_generic.html
original_path: boards/xtensa/intel_adsp_cavs25/doc/intel_adsp_generic.html
---
...
([SOF](https://thesofproject.github.io/latest/index.html)) project. SOF can be built with either Zephyr or Cadence’s proprietary
Xtensa OS (XTOS) and run on a variety of Intel and non-Intel platforms.

The Intel [UP Xtreme](https://up-board.org/up-xtreme/) product line has the publicly
available reference boards for Zephyr’s Intel ADSP support. This guide uses the
[UP Xtreme i11-0001 series](https://up-shop.org/up-xtreme-i11-boards-0001-series.html) ([Intel ADSP CAVS 2.5](index.md#intel-adsp-cavs25)) board as an example.
...
recommended you follow the [SOF instructions](https://thesofproject.github.io/latest/getting_started/build-guide/build-with-zephyr.html) to build and run SOF on Zephyr.

UP Xtreme users can refer to the [UP Community wiki](https://github.com/up-board/up-community/wiki/Ubuntu) for help installing a
...
Follow the instructions in the rimage `README.md` to build the tool on
...
[Cadence](https://www.cadence.com/en_US/home/tools/ip/tensilica-ip.html) instead. The following instructions assume you have purchased and
...
- For local, interactive use prefer `rimage.extra-args` in west config, see
  `west sign -h`. The WEST\_CONFIG\_LOCAL environment variable can point at a
...
- For board-specific needs you can define WEST\_SIGN\_OPTS in
...
[soc/xtensa/intel\_adsp/tools/remote-fw-service.py](https://github.com/zephyrproject-rtos/zephyr/blob/main/soc/xtensa/intel_adsp/tools/remote-fw-service.py) will receive
the binary sent over the network by West and invoke
[soc/xtensa/intel\_adsp/tools/cavstool.py](https://github.com/zephyrproject-rtos/zephyr/blob/main/soc/xtensa/intel_adsp/tools/cavstool.py) (referred to as the
...
`remote-fw-service.py` uses ports 9999 and 10000 on the remote host to
communicate. It forwards logs collected by `cavstool.py` on port 9999
...
it sends requests using the [soc/xtensa/intel\_adsp/tools/cavstool\_client.py](https://github.com/zephyrproject-rtos/zephyr/blob/main/soc/xtensa/intel_adsp/tools/cavstool_client.py)
...
`cavstool_client.py` to `remote-fw-service.py` is forwarded through
an intermediate host, you may need to tell `cavstool_client.py` to connect
...
Refer to [Test Runner (Twister)](../../../../develop/test/twister.md#twister-script) for more information on hardware maps.
...
recommended to take advantage of Zephyr’s built-in [Core Dump](../../../../services/debugging/coredump.md#coredump) and
[Logging](../../../../services/logging/index.md#logging-api) features.
...
`remote-fw-service.py` on the board, you may have another instance of the
...
with `remote-fw-service.py` but hang with no output afterwards,
there are two possibilities: either `remote-fw-service.py` failed
to communicate, or `cavstool.py` failed to flash. Log into
the remote host and check the output of `remote-fw-service.py`.
...
the ADSP or `remote-fw-service.py`. If you are unable to identify a
