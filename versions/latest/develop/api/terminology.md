---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/develop/api/terminology.html
...
suspend, wait, or invoke [`k_yield()`](../../kernel/services/threads/index.md#c.k_yield "k_yield") before it can complete
...
timeout, to which [`K_NO_WAIT`](../../kernel/services/timing/clocks.md#c.K_NO_WAIT "K_NO_WAIT") can be passed. The semantics of
...
[`k_sem_take()`](../../kernel/services/synchronization/semaphores.md#c.k_sem_take "k_sem_take") to be invoked from ISRs, since it is not
...
use by any API that is expected to be called in [`DEVICE_DEFINE()`](../../kernel/drivers/index.md#c.DEVICE_DEFINE "DEVICE_DEFINE")
or `SYS_INIT()` calls that may be invoked with `PRE_KERNEL_1`
or `PRE_KERNEL_2` initialization levels.

Generally a function that is **pre-kernel-ok** checks
[`k_is_pre_kernel()`](../../kernel/services/interrupts.md#c.k_is_pre_kernel "k_is_pre_kernel") when determining whether it can fulfill its
required behavior. In many cases it would also check
[`k_is_in_isr()`](../../kernel/services/interrupts.md#c.k_is_in_isr "k_is_in_isr") so it can be **isr-ok** as well.
...
example is [`spi_transceive_async()`](../../hardware/peripherals/spi.md#c.spi_transceive_async "spi_transceive_async").
