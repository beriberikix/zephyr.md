---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/services/device_mgmt/smp_protocol.html
...
| `OP` | [`mcumgr_op_t`](mcumgr.md#c.mcumgr_op_t "mcumgr_op_t"), determines whether information is written to a device or requested from it and whether a packet contains request to an SMP server or response from it. |
| `Flags` | Reserved for flags; there are no flags defined yet, the field should be set to 0 |
| `Data Length` | Length of the `Data` field |
| `Group ID` | [`mcumgr_group_t`](mcumgr.md#c.mcumgr_group_t "mcumgr_group_t"), see [Management Group ID’s](#mcumgr-smp-protocol-group-ids) for further details. |
