---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/services/smf/index.html
...
when [`CONFIG_SMF_ANCESTOR_SUPPORT`](../../kconfig.md#CONFIG_SMF_ANCESTOR_SUPPORT "CONFIG_SMF_ANCESTOR_SUPPORT") is enabled.
...
**NOTE:** While the state machine is running, smf\_set\_state should only be
called from the Entry and Run functions. Calling smf\_set\_state from the Exit
...
smf\_run\_state if it returns a non-zero value. The function has the following
...
:   - Ancestor entry actions are executed before the sibling entry actions. For
    ...
    - Ancestor exit actions are executed after the sibling exit actions. For
      example, the s1\_exit function is called before the parent\_exit function
      is called.
    - The parent\_run function only executes if the child\_run function returns
      without transitioning to another state, ie. calling smf\_set\_state.
