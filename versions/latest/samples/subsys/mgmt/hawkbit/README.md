---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/samples/subsys/mgmt/hawkbit/README.html
...
- The Zephyr port of `Hawkbit` is configured to run on a
  [Freedom-K64F](../../../../boards/arm/frdm_k64f/doc/index.md#frdm-k64f) MCU by default. The application should
...
`Hawkbit` can be built for the frdm\_k64f as follows:
...
`zephyr.signed.confirmed.bin` in the build directory. It’s important for
...
Upload the `zephyr.signed.confirmed.bin` file from Step 5 to image slot-0
of your board.
...
you should see the frdm\_k64f show up in the Targets pane. It’s time to
...
`zephyr.signed.bin` in the build directory.
...
`Hawkbit https` can be built for the frdm\_k64f as follows:
