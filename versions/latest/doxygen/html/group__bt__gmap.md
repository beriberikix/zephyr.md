---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/doxygen/html/group__bt__gmap.html
...
| Enumerator | |
| --- | --- |
| BT\_GMAP\_UGT\_FEAT\_SOURCE | Source support.  Requires `CONFIG_BT_ASCS_ASE_SRC_COUNT` > 0 |
| BT\_GMAP\_UGT\_FEAT\_80KBPS\_SOURCE | 80 kbps source support  Requires BT\_GMAP\_UGT\_FEAT\_SOURCE to be set as well |
| BT\_GMAP\_UGT\_FEAT\_SINK | Sink support.  Requires `CONFIG_BT_ASCS_ASE_SNK_COUNT` > 0 |
...
| BT\_GMAP\_UGT\_FEAT\_MULTISINK | Support for receiving at least two audio channels, each in a separate CIS.  Requires `CONFIG_BT_ASCS_ASE_SNK_COUNT` > 1 and `CONFIG_BT_ASCS_MAX_ACTIVE_ASES` > 1, and BT\_GMAP\_UGT\_FEAT\_SINK to be set as well |
| BT\_GMAP\_UGT\_FEAT\_MULTISOURCE | Support for sending at least two audio channels, each in a separate CIS.  Requires `CONFIG_BT_ASCS_ASE_SRC_COUNT` > 1 and `CONFIG_BT_ASCS_MAX_ACTIVE_ASES` > 1, and BT\_GMAP\_UGT\_FEAT\_SOURCE to be set as well |
