---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/doxygen/html/structieee802154__radio__api.html
...
|  | Stop the device. |
| int(\* | [continuous\_carrier](#a5b9b45e274fd6a8777cd96280b1a1b9e) )(const struct [device](structdevice.md) \*dev) |
...
All drivers SHALL support two externally observable interface operational states: "UP" and "DOWN". Drivers MAY additionally support a "TESTING" interface state (see [continuous\_carrier()](#a5b9b45e274fd6a8777cd96280b1a1b9e)).

The following rules apply:

- An interface is considered "UP" when it is able to transmit and receive packets, "DOWN" otherwise (see precise definitions of the corresponding ifOperStatus values in RFC 2863, section 3.1.14, [net\_if\_oper\_state](group__net__if.md#gadfe9d90f24046cd9bd4bbee096e747b9 "net_if_oper_state") and the [continuous\_carrier()](#a5b9b45e274fd6a8777cd96280b1a1b9e) exception below). A device that has its receiver temporarily disabled during "UP" state due to an active receive window configuration is still considered "UP".
...
- Drivers that implement the optional [continuous\_carrier()](#a5b9b45e274fd6a8777cd96280b1a1b9e) operation will be considered to be in the RFC 2863 "testing" ifOperStatus state if that operation returns zero. This state is active until either [start()](#a0e4d57dc3a53299ddedf2140e172103d) or [stop()](#a75cfcfe2b9e69c2e4af53f00c8294b36) is called. If [continuous\_carrier()](#a5b9b45e274fd6a8777cd96280b1a1b9e) returns a non-zero value then the previous state is assumed by upper layers.
...
## [◆ ](#a5b9b45e274fd6a8777cd96280b1a1b9e)continuous\_carrier

| int(\* ieee802154\_radio\_api::continuous\_carrier) (const struct [device](structdevice.md) \*dev) |
| --- |

Start continuous carrier wave transmission.

The method blocks until the interface has started to emit a continuous carrier. To leave this mode, [start()](#a0e4d57dc3a53299ddedf2140e172103d) or [stop()](#a75cfcfe2b9e69c2e4af53f00c8294b36) should be called, which will put the driver back into the "UP" or "DOWN" states, respectively.
...
Return values
:   | 0 | continuous carrier wave transmission started |
    | --- | --- |
    | -EALREADY | The driver was already in "TESTING" state and emitting a continuous carrier. |
    | -EIO | not started |
...
Return values
:   | 0 | The frame was successfully sent or scheduled. If the driver supports ACK offloading and the frame requested acknowlegment (AR bit set), this means that the packet was successfully acknowledged by its peer. |
    | --- | --- |
    ...
    | -ENOMSG | The frame was not confirmed by an ACK packet (TX ACK offloading feature only). |
