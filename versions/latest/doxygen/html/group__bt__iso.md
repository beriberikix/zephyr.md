---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/doxygen/html/group__bt__iso.html
...
Connect ISO channels. The ISO channels must have been initialized in a CIG first by calling bt\_iso\_cig\_create.
...
This function will also setup the ISO data path based on the `path` parameter of the [bt\_iso\_chan\_io\_qos](structbt__iso__chan__io__qos.md "ISO Channel IO QoS structure.") for each channel.
...
Regarding to input parameter, to get details see reference description to [bt\_iso\_chan\_connect()](#ga98953a261f3699b62cd19ab4977e0b4c) API above.
...
Send data from buffer to the channel. If credits are not available, buf will be queued and sent as and when credits are received from peer. Regarding to first input parameter, to get details see reference description to [bt\_iso\_chan\_connect()](#ga98953a261f3699b62cd19ab4977e0b4c) API above.
...
Once a CIG is created, the channels supplied in the `param` can be connected using bt\_iso\_chan\_connect.
...
Register ISO server, each new connection is authorized using the [accept()](group__bsd__sockets.md#ga33e6ea428ff537ed7a4763ce91b7d9bb "POSIX wrapper for zsock_accept.") callback which in case of success shall allocate the channel structure to be used by the new connection.
