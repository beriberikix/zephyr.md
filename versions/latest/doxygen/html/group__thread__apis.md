---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/doxygen/html/group__thread__apis.html
...
| void | [k\_reschedule](#gad82575e576cd08906fbc68fe36be48bd) (void) |
...
| static void | [k\_thread\_start](#gac539268e0b45c600315a6567ec27f965) ([k\_tid\_t](kernel_2thread_8h.md#a6379f5a1f19ffbc262a6877c4f6e3647) thread) |
...
## [◆ ](#gad82575e576cd08906fbc68fe36be48bd)k\_reschedule()
...
Returns
:   Zero if the requested time has elapsed or the time left to sleep rounded up to the nearest millisecond (e.g. if the thread was awoken by the [k\_wakeup](#ga9275a019c8ff3c7fe49a81f8c078157e) call). Will be clamped to INT\_MAX in the case where the remaining time is unrepresentable in an [int32\_t](stdint_8h.md#a0c18914b3041c2f583aba76f418399c2).
...
Stack objects passed to this function may be statically allocated with either of these macros in order to be portable:
...
Alternatively, the stack may be dynamically allocated using [k\_thread\_stack\_alloc](#gafe00cc70bac8a47ba6dda21bde508614).
...
This routine reverses the thread suspension from [k\_thread\_suspend()](#ga66cf8682fb65870eceb5e57d667a8d4e) and allows the kernel scheduler to make *thread* the current thread when it is next eligible for that role.
...
Dynamically allocate a thread stack either from a pool of thread stacks of size `CONFIG_DYNAMIC_THREAD_POOL_SIZE`, or from the system heap. Order is determined by the `CONFIG_DYNAMIC_THREAD_PREFER_ALLOC` and `CONFIG_DYNAMIC_THREAD_PREFER_POOL` options. Thread stacks from the pool are of maximum size `CONFIG_DYNAMIC_THREAD_STACK_SIZE`.

Note
:   When no longer required, thread stacks allocated with [k\_thread\_stack\_alloc()](#gafe00cc70bac8a47ba6dda21bde508614) must be freed with [k\_thread\_stack\_free](#ga95560cb85f6656b981a9a50ff2cd70b7) to avoid leaking memory.
...
- [K\_USER](#gacb5340339892f22301e02697c6039ccc) allocate a userspace thread (requires `CONFIG_USERSPACE`)
...
## [◆ ](#gac539268e0b45c600315a6567ec27f965)k\_thread\_start()

| | void k\_thread\_start | ( | [k\_tid\_t](kernel_2thread_8h.md#a6379f5a1f19ffbc262a6877c4f6e3647) | *thread* | ) |  | | --- | --- | --- | --- | --- | --- | | inlinestatic |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
Note
:   This is a legacy API for compatibility. Modern Zephyr threads are initialized in the "sleeping" state and do not need special handling for "start".
...
This routine prevents the kernel scheduler from making *thread* the current thread. All other internal operations on *thread* are still performed; for example, kernel objects it is waiting on are still handed to it. Thread suspension does not impact any timeout upon which the thread may be waiting (such as a timeout from a call to [k\_sem\_take()](group__semaphore__apis.md#gac71e2383c1920dddc45a561cacfef090 "Take a semaphore.") or [k\_sleep()](#ga48d4b041790454da4d68ac8711f29657)). Thus if the timeout expires while the thread is suspended, it is still suspended until [k\_thread\_resume()](#ga117b26f8569ec3045ead1fad1851663d) is called.
...
This function is unlikely to work as expected without kernel tuning. In particular, because the lower bound on the duration of a sleep is the duration of a tick, `CONFIG_SYS_CLOCK_TICKS_PER_SEC` must be adjusted to achieve the resolution desired. The implications of doing this must be understood before attempting to use [k\_usleep()](#gaeac56bb072ce295b9fdc372ab8cee67e). Use with caution.
