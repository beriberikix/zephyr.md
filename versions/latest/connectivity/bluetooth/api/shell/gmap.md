---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/connectivity/bluetooth/api/shell/gmap.html
original_path: connectivity/bluetooth/api/shell/gmap.html
---
...
When the Bluetooth stack has been initialized (`bt init`), the GMAS can be registered by
by calling `gmap init`. It is also strongly suggested to enable BAP via `bap init`.
...
The `set_role` command can be used to change the role at runtime, assuming that the device
