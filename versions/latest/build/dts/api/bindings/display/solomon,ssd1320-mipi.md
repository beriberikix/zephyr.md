---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/display/solomon,ssd1320-mipi.html
original_path: build/dts/api/bindings/display/solomon,ssd1320-mipi.html
---

# solomon,ssd1320 (on mipi-dbi bus)
...
```text
SSD1320 160x160 dot-matrix display controller on MIPI_DBI or SPI bus
```
...
| `multiplex-ratio` | `int` | ```text Multiplex ratio from 15MUX to 159MUX. Typically same value as height - 1. ```  This property is **required**. |
| `remap-value` | `int` | ```text Remap register Has multiple configurations (see each bit setting in the datasheet) - A[1] must be 1 - Sequential (0) or Alternative (Odd/Even) SEGs (A[4]) - Disable or Enable SEG left/right remap (A[5]) A[4] is needed when display has striping. ```  This property is **required**. |
...
| `internal-iref` | `int` | ```text Internal or external iref. 0x0 is External, 0x10 Internal. Most displays use Internal. ```  Default value: `16` |
| `precharge-voltage` | `int` | ```text Set precharge voltage (4:0) from 0.10 x VCC to 0.5113 x VCC (0x1F). 0x1E is the reset default. ```  Default value: `30` |
| `vcomh-voltage` | `int` | ```text Set COM deselect voltage (5:3) from 0.72 x VCC (0x0) to 0.84 x VCC (0x30) 0x20 is the reset default. ```  Default value: `32` |
| `inversion-on` | `boolean` | ```text Turn on display color inverting ``` |
| `greyscale-enhancement` | `boolean` | ```text Enable low greyscale enhancement, partially undocumented. ``` |
| `inv-seg` | `boolean` | ```text Invert segment order ``` |
| `inv-com` | `boolean` | ```text Invert COM order ``` |
...
may apply to the “solomon,ssd1320” compatible.
