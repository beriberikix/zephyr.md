---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/display/solomon,ssd1331.html
original_path: build/dts/api/bindings/display/solomon,ssd1331.html
---
...
| Name | Type | Details |
| --- | --- | --- |
| `display-offset` | `int` | ```text Vertical offset by com from 0 ~ 63. Typically 64 - height. ```  This property is **required**. |
| `start-line` | `int` | ```text Start line of display RAM to be displayed by selecting a value from 0 to 63. Typically 0. ```  This property is **required**. |
| `multiplex-ratio` | `int` | ```text Multiplex ratio from 16MUX to 64MUX, ranges from 15 to 63. Typically same value as height - 1. ```  This property is **required**. |
| `phase-length` | `int` | ```text Phase 2 Period (7:4) and Phase 1 Period (3:0). ```  This property is **required**. |
| `oscillator-freq` | `int` | ```text Front clock divider (3:0) / oscillator frequency (7:4). If you get weird scanlines, increase oscillator frequency and play with phase length values, for example set this to 0xF0. Note this increases power consumption. ```  This property is **required**. |
| `precharge-time-a` | `int` | ```text Set precharge time for segments A. Typically around 0x70. ```  This property is **required**. |
| `precharge-time-b` | `int` | ```text Set precharge time for segments B. Typically around 0x70. ```  This property is **required**. |
| `precharge-time-c` | `int` | ```text Set precharge time for segments C. Typically around 0x70. ```  This property is **required**. |
| `precharge-voltage` | `int` | ```text Set precharge voltage. A[5:1], from 0 (0.10 x VCC) to 0x3E (0.5 x VCC) ```  This property is **required**. |
| `vcomh-voltage` | `int` | ```text Set COM deselect voltage. A[5:1]. 0.44, 0.52, 0.61, 0.71, 0.83 x VCC. ```  This property is **required**.  Legal values: `0`, `16`, `32`, `48`, `62` |
| `current-att` | `int` | ```text Set Master current attenuation. A[3:0], from 1/16 to 16/16. ```  This property is **required**. |
| `remap-value` | `int` | ```text Remap register Has multiple configurations (see each bit setting in the datasheet) - A[0] HTiled or VTiled. Must be 0 for this driver. - A[1] Invert segment order - A[2] Invert ABC segment order - A[3] Left-Right Swap COMs - A[4] Invert COM order - A[5] Stripped odd/even display - A[7:6] Set color format. Must be 0x1 for this driver. ```  This property is **required**. |
| `power-save` | `boolean` | ```text Enable Power Save mode ``` |
...
may apply to the “solomon,ssd1331” compatible.
