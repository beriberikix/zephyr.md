---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/display/sitronix,st7567-spi.html
original_path: build/dts/api/bindings/display/sitronix,st7567-spi.html
---

# sitronix,st7567 (on spi bus)
...
```text
Sitronix ST7567 dot-matrix LCD controller on SPI bus
...
| `column-offset` | `int` | ```text 8-bits start column address ```  This property is **required**. |
| `line-offset` | `int` | ```text 6-bits start line offset ```  This property is **required**. |
| `regulation-ratio` | `int` | ```text Regulation resistor ratio (3-bits RR2 RR1 RR0) ```  This property is **required**. |
| `segment-invdir` | `boolean` | ```text Scan direction is from last SEG output to first SEG output ``` |
| `com-invdir` | `boolean` | ```text Scan direction is from last COM output to first COM output ``` |
| `inversion-on` | `boolean` | ```text Invert display ``` |
| `bias` | `boolean` | ```text Bias setting (0 for 1/9, or 1 for 1/7) ``` |
| `reset-gpios` | `phandle-array` | ```text RESET pin. The RESET pin of ST7567 is active low. If connected directly, the MCU pin should be configured as active low. ``` |
...
may apply to the “sitronix,st7567” compatible.
