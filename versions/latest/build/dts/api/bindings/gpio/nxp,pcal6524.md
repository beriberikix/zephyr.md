---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/gpio/nxp,pcal6524.html
original_path: build/dts/api/bindings/gpio/nxp,pcal6524.html
---

# nxp,pcal6524 (on i2c bus)
...
[drivers/gpio/gpio\_pca\_series.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/gpio/gpio_pca_series.c).

## Description

```text
NXP PCAL6524 I2C GPIO expander node
```
...
These property descriptions apply to “nxp,pcal6524”
...
| Name | Type | Details |
| --- | --- | --- |
| `ngpios` | `int` | ```text This property indicates the number of in-use slots of available slots for GPIOs. The typical example is something like this: the hardware register is 32 bits wide, but only 18 of the bits have a physical counterpart. The driver is generally written so that all 32 bits can be used, but the IP block is reused in a lot of designs, some using all 32 bits, some using 18 and some using 12. In this case, setting "ngpios = <18>;" informs the driver that only the first 18 GPIOs, at local offset 0 .. 17, are in use.  For cases in which there might be holes in the slot range, this value should be the max slot number+1. ```  This property is **required**.  Default value: `32`  Constant value: `24` |
| `reset-gpios` | `phandle-array` | ```text Reset GPIO pin (active-low) Left blank if the device does not have reset pin or the pin is not connected in your application. ``` |
| `int-gpios` | `phandle-array` | ```text Interrupt GPIO pin (active-low open-drain) Left blank if the pin is not connected in your application. ``` |
...
may apply to the “nxp,pcal6524” compatible.
