---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/adc/microchip,mcp356xr.html
original_path: build/dts/api/bindings/adc/microchip,mcp356xr.html
---

# microchip,mcp356xr (on spi bus)
...
[drivers/adc/adc\_mcp356xr.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/adc/adc_mcp356xr.c).
...
These property descriptions apply to “microchip,mcp356xr”
...
| `address` | `int` | ```text ADC address, must be between 0 and 4. In most cases address is "1". ```  This property is **required**.  Legal values: `0`, `1`, `2`, `3`, `4` |
| `analog-clock-prescaler` | `int` | ```text Prescaler to use to derive the analog (AMCKL) from the master clock (MCLK). Valid values are: - MCP356XR_ANALOG_CLOCK_NO_DIV - MCP356XR_ANALOG_CLOCK_DIV_2 - MCP356XR_ANALOG_CLOCK_DIV_4 - MCP356XR_ANALOG_CLOCK_DIV_8 ```  This property is **required**.  Legal values: `0`, `1`, `2`, `3` |
| `boost-current-bias` | `int` | ```text Delta-sigma modulator current biasing configuration. Valid values are: - MCP356XR_BOOST_CURRENT_BIAS_DIV_2 - MCP356XR_BOOST_CURRENT_BIAS_MUL_0_66 - MCP356XR_BOOST_CURRENT_BIAS_MUL_1 - MCP356XR_BOOST_CURRENT_BIAS_MUL_2 ```  This property is **required**.  Legal values: `0`, `1`, `2`, `3` |
| `vref-settle-time-ms` | `int` | ```text Time in milliseconds to wait for after the selected voltage reference has changed in order to stabilize itself. This is necessary if a mix of channels with different reference voltages is used. ``` |
| `irq-gpios` | `phandle-array` | ```text GPIO connected to the MCP356XR's IRQ pin ``` |
| `irq-pin-drive-open-drain` | `boolean` | ```text Drives the MCP356XR's IRQ pin in an open drain fashion ``` |
| `enable-adc-offset-cancellation` | `boolean` | ```text Enable ADC induced offset cancellation ``` |
| `enable-internal-vref-offset-cancellation` | `boolean` | ```text Enable internal voltage reference induced offset cancellation ``` |
| `use-internal-clock` | `boolean` | ```text Use the internal ADC clock instead of providing it via the MCLK pin ``` |
| `enable-analog-clock-output` | `boolean` | ```text Output AMCLK on the MCLK pin only effective when use-internal-clock is set ``` |
...
may apply to the “microchip,mcp356xr” compatible.
