---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/mspi/snps,designware-ssi.html
original_path: build/dts/api/bindings/mspi/snps,designware-ssi.html
---
...
[drivers/mspi/mspi\_dw.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/mspi/mspi_dw.c).
...
```text
Synopsys DesignWare Synchronous Serial Interface (SSI) node
```
...
| Name | Type | Details |
| --- | --- | --- |
| `aux-reg-enable` | `boolean` | ```text Activates auxiliary register access that is needed on some platforms. ``` |
| `fifo-depth` | `int` | ```text Number of items that can be stored in the TX FIFO. Range: 8-256. If the RX FIFO depth is not specified separately in the rx-fifo-depth property, this value specifies depth of both TX and RX FIFOs. ```  This property is **required**. |
| `rx-fifo-depth` | `int` | ```text Number of items that can be stored in the RX FIFO. Range: 8-256. ``` |
| `tx-fifo-threshold` | `int` | ```text Number of entries in the TX FIFO above which the TX transfer is started. Maximum value is the TX FIFO depth - 1. ``` |
| `rx-fifo-threshold` | `int` | ```text Number of entries in the RX FIFO above which the controller gets an RX interrupt. Maximum value is the RX FIFO depth - 1. ``` |
...
may apply to the “snps,designware-ssi” compatible.
