---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/hdlc_rcp_if/spi,hdlc-rcp-if.html
original_path: build/dts/api/bindings/hdlc_rcp_if/spi,hdlc-rcp-if.html
---

# spi,hdlc-rcp-if (on spi bus)

Vendor: [Unknown vendor](../../bindings.md#dt-unknown-vendor)

Note

An implementation of a driver matching this compatible is available in
[drivers/hdlc\_rcp\_if/hdlc\_rcp\_if\_spi.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/hdlc_rcp_if/hdlc_rcp_if_spi.c).

## Description

```text
SPI HDLC RCP interface node
```
...
| Name | Type | Details |
| --- | --- | --- |
| `int-gpios` | `phandle-array` | ```text Interrupt GPIO. Used by the RCP to signal data is available. Active low. ```  This property is **required**. |
| `reset-gpios` | `phandle-array` | ```text Reset GPIO. Used to assert a hardware reset for the RCP. Active low. ``` |
| `reset-assert-time` | `int` | ```text The time (in ms) to keep the RCP radio reset active. ```  Default value: `30` |
| `reset-delay` | `int` | ```text The delay (in ms) after reset de-assertion. ``` |
| `cs-delay` | `int` | ```text The delay (in ms) for the RCP radio chip select. ``` |
...
may apply to the “spi,hdlc-rcp-if” compatible.
