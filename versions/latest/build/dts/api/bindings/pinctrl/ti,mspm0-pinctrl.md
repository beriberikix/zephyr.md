---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/pinctrl/ti,mspm0-pinctrl.html
original_path: build/dts/api/bindings/pinctrl/ti,mspm0-pinctrl.html
---
...
[drivers/pinctrl/pinctrl\_mspm0.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/pinctrl/pinctrl_mspm0.c).
...
These property descriptions apply to “ti,mspm0-pinctrl”
...
may apply to the “ti,mspm0-pinctrl” compatible.
...
| Name | Type | Details |
| --- | --- | --- |
| `pinmux` | `int` | ```text MSPM0 pin's configuration (IO pin, IO function). ```  This property is **required**. |
| `drive-strength` | `int` | ```text The drive strength controls the maximum output drive strength sunk or sourced by an I/O pin.   6: max 6 mA (SoC default)   20: max 20 mA on high-drive capable IOs only (HDIO). ```  Default value: `6`  Legal values: `6`, `20` |
| `ti,invert` | `boolean` | ```text Enables inversion of the input or output using the internal inversion capability of the GPIO ``` |
| `ti,hysteresis` | `boolean` | ```text Enables the hysteresis control for access to CMOS logic (on open-drain capable pins) ``` |
