---
version: v4.1.0
source_url: https://docs.zephyrproject.org/4.1.0/build/dts/api/bindings/spi/nxp,dspi.html
original_path: build/dts/api/bindings/spi/nxp,dspi.html
---
...
[drivers/spi/spi\_mcux\_dspi.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/spi/spi_mcux_dspi.c).
...
| Name | Type | Details |
| --- | --- | --- |
| `pcs-sck-delay` | `int` | ```text Delay in nanoseconds from the chip select assert to the first clock edge. If not set, the minimum supported delay is used. ``` |
| `sck-pcs-delay` | `int` | ```text Delay in nanoseconds from the last clock edge to the chip select deassert. If not set, the minimum supported delay is used. ``` |
| `transfer-delay` | `int` | ```text Delay in nanoseconds from the chip select deassert to the next chip select assert. If not set, the minimum supported delay is used. ``` |
...
| `nxp,rx-tx-chn-share` | `boolean` | ```text If the edma channel shared with tx and rx ``` |
| `ctar` | `int` | ```text ctar register selection range form 0-1 for master mode, 0 for slave mode ``` |
| `sample-point` | `int` | ```text Controls when the DSPI master samples SIN in the Modified Transfer Format. This field is valid only when the CPHA bit in the CTAR register is 0. ``` |
| `continuous-sck` | `boolean` | ```text continuous SCK enable. Note that the continuous SCK is only supported for CPHA = 1. ``` |
| `rx-fifo-overwrite` | `boolean` | ```text receive FIFO overflow overwrite enable. If ROOE = 0, the incoming data is ignored and the data from the transfer that generated the overflow is also ignored. If ROOE = 1, the incoming data is shifted to the shift register. ``` |
| `modified-timing-format` | `boolean` | ```text Enables a modified transfer format to be used if true. ``` |
| `tx-fifo-size` | `int` | ```text tx fifo size ``` |
| `rx-fifo-size` | `int` | ```text rx fifo size ``` |
...
may apply to the “nxp,dspi” compatible.
