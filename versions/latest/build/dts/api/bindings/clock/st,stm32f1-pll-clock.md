---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/build/dts/api/bindings/clock/st,stm32f1-pll-clock.html
original_path: build/dts/api/bindings/clock/st,stm32f1-pll-clock.html
---
...
| `mul` | `int` | ```text Main PLL multiplication factor for VCO Valid range: 2 - 16 ```  This property is **required**. |
| `xtpre` | `boolean` | ```text Optional HSE divider for PLL entry ``` |
| `usbpre` | `boolean` | ```text Optional PLL output divisor to generate a 48MHz USB clock. When set, PLL clock is not divided. Otherwise, PLL output clock is divided by 1.5. ``` |
