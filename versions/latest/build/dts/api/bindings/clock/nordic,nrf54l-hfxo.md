---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/clock/nordic,nrf54l-hfxo.html
original_path: build/dts/api/bindings/clock/nordic,nrf54l-hfxo.html
---
...
```text
Nordic nRF high-frequency crystal oscillator (nRF54L series)
```
...
| `startup-time-us` | `int` | ```text Startup time in microseconds.  The value can be obtained by first measuring the time between TASKS_XOSTART and EVENTS_XOTUNED. Then multiply the value by 2 to account for temperature and supply variations.  Note that the startup time will be longer usual on the first power-up and as such should not be used to determine startup-time-us. ```  This property is **required**. |
...
may apply to the “nordic,nrf54l-hfxo” compatible.
