---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/regulator/nordic,npm1304-regulator.html
original_path: build/dts/api/bindings/regulator/nordic,npm1304-regulator.html
---

# nordic,npm1304-regulator
...
[drivers/regulator/regulator\_npm13xx.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/regulator/regulator_npm13xx.c).
...
These property descriptions apply to “nordic,npm1304-regulator”
...
may apply to the “nordic,npm1304-regulator” compatible.
...
| `enable-gpio-config` | `array` | ```text Regulator enable controlled by specified GPIO pin <idx flags>. When set regulator must be enabled/disabled using set_dvs_mode. ``` |
| `pwm-gpio-config` | `array` | ```text Regulator enable controlled by specified GPIO pin <idx flags>. When set regulator must be enabled/disabled using set_dvs_mode. ``` |
| `retention-gpio-config` | `array` | ```text Retention mode controlled by specified GPIO pin <idx flags>. ``` |
| `soft-start-microamp` | `int` | ```text Soft start current limit in microamps. ```  Legal values: `10000`, `20000`, `35000`, `50000` |
| `active-discharge` | `boolean` | ```text Enable active-discharge on the BUCK/LDO/LDSW output when disabled. ``` |
| `nordic,anomaly38-disable-workaround` | `boolean` | ```text Disable the SW workaround for nPM13xx anomaly #38. When nPM13xx is in ULP mode, LDO is supplied from VSYS and then LDO is enabled, it can take long time until the LDO output has reached its target voltage. To avoid this, an i2c read is performed shortly after an LDO is enabled. See nPM13xx Errata manual for more details. ``` |
