---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/pwm/renesas,rx-mtu-pwm.html
original_path: build/dts/api/bindings/pwm/renesas,rx-mtu-pwm.html
---
...
| Name | Type | Details |
| --- | --- | --- |
| `prescaler` | `int` | ```text Valid values are in the range 0-7 but specifically depend on the device (see RX user's manual). Common settings are 0-3 for dividers PCKL/1, PCLK/4, PCLK/16 and PCLK/64. ``` |
| `#pwm-cells` | `int` | ```text Number of items to expect in a PWM   - channel of the timer used for PWM   - period to set in ns   - flags : combination of standard flags like PWM_POLARITY_NORMAL ```  This property is **required**.  Constant value: `3` |
...
may apply to the “renesas,rx-mtu-pwm” compatible.
