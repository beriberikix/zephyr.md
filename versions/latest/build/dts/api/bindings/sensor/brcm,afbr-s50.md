---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/sensor/brcm,afbr-s50.html
original_path: build/dts/api/bindings/sensor/brcm,afbr-s50.html
---

# brcm,afbr-s50 (on spi bus)

Vendor: [Broadcom Corporation](../../bindings.md#dt-vendor-brcm)

Note

An implementation of a driver matching this compatible is available in
[drivers/sensor/broadcom/afbr\_s50/afbr\_s50.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/sensor/broadcom/afbr_s50/afbr_s50.c).
...
| Name | Type | Details |
| --- | --- | --- |
| `int-gpios` | `phandle-array` | ```text The INT signal default configuration is active-low. The property value should ensure the flags properly describe the signal that is presented to the driver. ```  This property is **required**. |
| `measurement-mode` | `int` | ```text Specify Measurement Mode setting. ```  This property is **required**.  Legal values: `1`, `2`, `5`, `6`, `9`, `129`, `130`, `133`, `134`, `137` |
| `dual-freq-mode` | `int` | ```text Specify Dual Frequency Mode setting. ```  This property is **required**.  Legal values: `0`, `1`, `2` |
| `odr` | `int` | ```text Specify the default output data rate expressed in samples per second (Hz). Default configuration used from all sample-codes provided by vendor. Please note the range is limited with Dual Frequency Mode enabled (up to 100 Hz). ```  Default value: `10` |
| `spi-mosi-gpios` | `phandle-array` | ```text SPI MOSI GPIO pin. Used by the driver in order to use the bus in GPIO mode, which is required to interact with the internal EEPROM the AFBR contains (mandatory). This property is required since the SPI bus is configured and used through pinctrl, which is not directly mapped to a GPIO port/pin. ```  This property is **required**. |
| `spi-miso-gpios` | `phandle-array` | ```text SPI MISO GPIO pin. Used by the driver in order to use the bus in GPIO mode, which is required to interact with the internal EEPROM the AFBR contains (mandatory). This property is required since the SPI bus is configured and used through pinctrl, which is not directly mapped to a GPIO port/pin. ```  This property is **required**. |
| `spi-sck-gpios` | `phandle-array` | ```text SPI CLK GPIO pin. Used by the driver in order to use the bus in GPIO mode, which is required to interact with the internal EEPROM the AFBR contains (mandatory). This property is required since the SPI bus is configured and used through pinctrl, which is not directly mapped to a GPIO port/pin. ```  This property is **required**. |
...
may apply to the “brcm,afbr-s50” compatible.
