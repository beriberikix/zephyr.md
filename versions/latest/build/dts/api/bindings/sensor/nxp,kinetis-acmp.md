---
version: v3.6.0
source_url: https://raw.githubusercontent.com/zephyrproject-rtos/zephyr/3.6.0/doc/build/dts/api/bindings/sensor/nxp,kinetis-acmp.html
original_path: build/dts/api/bindings/sensor/nxp,kinetis-acmp.html
---
...
```text
NXP Kinetis Analog Comparator (ACMP)
```
...
| Name | Type | Details |
| --- | --- | --- |
| `nxp,enable-output-pin` | `boolean` | ```text Make the comparator output (CMP0) available on a packaged pin. ``` |
| `nxp,use-unfiltered-output` | `boolean` | ```text Use the unfiltered comparator output for CMP0. ``` |
| `nxp,high-speed-mode` | `boolean` | ```text Enable high speed comparison mode. ``` |
| `nxp,enable-sample` | `boolean` | ```text Enable external sample signal as clock input. ``` |
| `nxp,filter-count` | `int` | ```text Filter sample count (0 to 7). ``` |
| `nxp,filter-period` | `int` | ```text Filter sample period in bus clock cycles (0 to 255). ``` |
| `nxp,window-mode` | `boolean` | ```text Enable windowing mode. ``` |
