---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/sensor/nordic,npm1304-charger.html
original_path: build/dts/api/bindings/sensor/nordic,npm1304-charger.html
---

# nordic,npm1304-charger
...
[drivers/sensor/nordic/npm13xx\_charger/npm13xx\_charger.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/sensor/nordic/npm13xx_charger/npm13xx_charger.c).

## Description

```text
nPM1304 PMIC Charger
```
...
| Name | Type | Details |
| --- | --- | --- |
| `term-microvolt` | `int` | ```text Charge termination voltage in uV. Available range is 3.6 V to 3.65 V and 4.0 V to 4.65 V in 50 mV steps. ```  This property is **required**. |
| `term-warm-microvolt` | `int` | ```text Charge termination voltage when warm in uV. Available range is 3.6 V to 3.65 V and 4.0 V to 4.65 V in 50 mV steps. If omitted, the term-microvolt value will be used. ``` |
| `current-microamp` | `int` | ```text Charge current in uA. Available range is 4 mA to 100 mA in 0.5 mA steps. ```  This property is **required**. |
| `dischg-limit-microamp` | `int` | ```text Discharge current limit in uA. Fixed to 125 mA on nPM1304. ```  Default value: `125000`  Constant value: `125000` |
| `term-current-percent` | `int` | ```text Termination current, as a percentage of current-microamp. Charge completes when the charge current falls below this value. If omitted the device default of 10% is used. ```  Default value: `10`  Legal values: `10`, `5` |
| `vbus-limit-microamp` | `int` | ```text Vbus current limit in uA. Available range is 100 mA to 1500 mA in 100 mA steps. ```  This property is **required**. |
...
may apply to the “nordic,npm1304-charger” compatible.
