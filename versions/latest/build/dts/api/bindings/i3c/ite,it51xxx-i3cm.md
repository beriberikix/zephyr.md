---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/i3c/ite,it51xxx-i3cm.html
original_path: build/dts/api/bindings/i3c/ite,it51xxx-i3cm.html
---
...
[drivers/i3c/i3cm\_it51xxx.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/i3c/i3cm_it51xxx.c).
...
| Name | Type | Details |
| --- | --- | --- |
| `io-channel` | `int` | ```text The it51xxx chip features four i3c io channels and two i3c cotroller (i3cm) engines. This property allows the i3cm engine to select the desired i3c io channel. To ensure proper i3c functionality, make sure that multiple controllers (including the i3cm controller and i3cs target engines) are not assigned the same io channel. ```  This property is **required**.  Legal values: `0`, `1`, `2`, `3` |
| `i3c-pp-duty-cycle` | `int` | ```text The duty cycle of push-pull frequency(unit in percentage). ```  Default value: `50` |
| `i3c-od-scl-hz` | `int` | ```text The open-drain frequency for the i3c controller. ```  This property is **required**. |
| `i3c-scl-hddat` | `int` | ```text i3c data hold time(0 by default). The time is calculated as: t_hddat = (i3c-scl-hddat + 1) * 20.8ns. The range of i3c-scl-hddat is [63:0]. ``` |
| `i3c-scl-tcas` | `int` | ```text i3c clock after start condition(1 by default). The time is calculated as: t_cas = (i3c-scl-tcas + 1) * 20.8ns. The maximum value is 255. ``` |
| `i3c-scl-tcbs` | `int` | ```text i3c clock before stop condition(0 by default). The time is calculated as: t_cbs = (i3c-scl-tcbs + 1) * 20.8ns. The maximum value is 255. ``` |
| `i3c-scl-tcasr` | `int` | ```text i3c clock after repeated start condition(1 by default). The time is calculated as: t_casr = (i3c-scl-tcasr + 1) * 20.8ns. The maximum value is 255. ``` |
| `i3c-scl-tcbsr` | `int` | ```text i3c clock before repeated start condition(0 by default). The time is calculated as: t_cbsr = (i3c-scl-tcbsr + 1) * 20.8ns. The maximum value is 255. ``` |
| `i2c-scl-hddat` | `int` | ```text i2c data hold time(0 by default). The time is calculated as: t_hddat = (i2c-scl-hddat + 1) * 20.8ns. The range of i3c-scl-hddat is [65535:0]. ``` |
...
may apply to the “ite,it51xxx-i3cm” compatible.
