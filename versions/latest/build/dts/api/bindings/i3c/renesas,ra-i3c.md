---
version: v4.2.0
source_url: https://docs.zephyrproject.org/4.2.0/build/dts/api/bindings/i3c/renesas,ra-i3c.html
original_path: build/dts/api/bindings/i3c/renesas,ra-i3c.html
---
...
[drivers/i3c/i3c\_renesas\_ra.c](https://github.com/zephyrproject-rtos/zephyr/blob/main/drivers/i3c/i3c_renesas_ra.c).
...
may apply to the “renesas,ra-i3c” compatible.
...
| `clock-names` | `string-array` | ```text Clocks must be given corresponding names so that the shim driver can recognize them. "pclk": peripheral clock source "tclk": transfer clock source ```  This property is **required**.  Legal values: `'pclk'`, `'tclk'` |
...
| `interrupt-names` | `string-array` | ```text Interrupts must be given corresponding names so that the shim driver can recognize them. "resp": Command response interrupt "rx": Rx data buffer full interrupt "tx": Tx data buffer empty interrupt "rcv": Receive status buffer full interrupt "ibi": IBI data interrupt "eei": Error, timeout and HDR exit interrupt ```  This property is **required**.  Legal values: `'resp'`, `'rx'`, `'tx'`, `'rcv'`, `'ibi'`, `'eei'` |
